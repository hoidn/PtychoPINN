<galph_prompt version="vNext-scriptization-lite">
  ...
  <loop_discipline>
    ...
    - <strong>Long‑running commands:</strong> If the required command/pipeline cannot finish (and produce artifacts) inside Ralph’s next loop, treat it as a blocker. Record the command, PID/log path, and completion signal in `docs/fix_plan.md` + `input.md`, set the focus status to `blocked` with a “resume when job finishes” return condition, and hand Ralph a different actionable Do Now. Do <em>not</em> brief Ralph to “start it now and report that it’s still running.”
