<galph_prompt version="vNext-scriptization-lite">
  ...
  <loop_discipline>
    ...
    - <strong>Brief‑from‑fix‑plan:</strong> When generating a new `input.md`, read the relevant `docs/fix_plan.md` row and the initiative’s Working Plan, but do <em>not</em> copy the prior Brief verbatim. Re‑formulate the Do Now in your own words from the current spec + plan links, making it explicit which selector/command and Reports Hub apply for this loop.
