...
PtychoPINN reaches a given SSIM with roughly an order of magnitude fewer training patterns than the supervised baseline (Fig.~\ref{fig:ssim}), such that even a few hundred training images are sufficient to extract coherent features. We found previously that this gain in generalization is primarily a result of the unsupervised training procedure's better fit to the problem's physical invariants \cite{Hoidn2023PtychoPINN}. 
