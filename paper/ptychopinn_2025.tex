...
To address this challenge, machine learning (ML) has been explored to accelerate reconstruction. Supervised convolutional networks, for instance, can achieve significant speedups\cite{Cherukara2020PtychoNN}. However, their practical application is limited by poor generalization and resolution. \cite{Maiden2012Annealing,Zhang2013Position,Seifert2023PoissonGaussian, [ptychonn nat com 'machine learning at the edge' natcom paper as example of the impracticality of these approaches]}. Moreover, these methods impose the onerous burden of generating large, curated datasets of ground-truth reconstructions for training. Our previous work, PtychoPINN~\cite{Hoidn2023PtychoPINN}, showed that a Physics-Informed Neural Network (PINN) could achieve rapid, self-supervised reconstruction. However, its effectiveness was demonstrated only on synthetic data with idealized probes and regular scan patterns. 
% In parallel, maximum-likelihood formulations grounded in photon counting provide a principled objective for low-dose data~\cite{Thibault2012NJPML}. 
...
TODO need a section introducing the inverse problem and mentioning that solutions must satisfy two sets of constraints: real-space (overlap or shrink-wrap) and reciprocal space (intensity matching). In some cases (Fresnel CDI) the first constraint can be dropped. The diffraction-space constraint, on the other hand, is always necessary. Laying it out this way makes it easier to account for the failure of previous methods that stumbled onto overlap-free reconstruction but were fundamentally flawed because they lacked diffraction-space constraints. 
...
Semi-synthetic datasets derived from the conventionally-reconstructed object and probe are experimentally realistic and allow quantitiative ground truth-comparisons of the three reconstruction methods. Here, PtychoPINN achieves an SSIM of $0.962$ in phase reconstruction compared to $0.912$ for the supervised-CNN baseline and \emph{TODO: fill in the iterative reconstruction SSIM value}. 

Evaluation of both models subsequent to training on datasets of varying size (Figure \ref{fig:ssim}) demonstrates the models' relative degrees of in-distribution generalization. (todo language: a training set size of 512 images is a stringent test of in-distribution generalization. Supervised (PtychoNN) fails to converge. PtychoPINN gives a capable reconstruction. MS SSIM values are represented in Figure \ref{fig:ssim} and should be represented in a table so that we can cite numerical values here)
...
Focused probe configurations wherein the [probe wavefront is spherically curved] make single-shot reconstruction possible, in principle. Conventional sovers don't support this (COMMENT does the comparison to conventional solvers belong in discussion only, not here?) but one can make use of this in PtychoPINN by simply configuring $C_g=0$. Figure~\ref{fig:overlap_free} shows this [hybrid vs idealized with and wo overlap, wo overlap is only a bit worse].

\subsection{Data Efficiency}

Figure~\ref{fig:ssim_training_size} plots reconstruction quality versus training set size. PtychoPINN exhibits superior reconstruction quality at approximately one-tenth the training set size compared to PtychoNN. [TODO it should be explained, maybe in a footnote, that this factor of 10 comes from averaging the horizontal distance between the two curves in fig:ssim\_training\_size. "The horizontal shift between the two curves in Figure~\ref{fig:ssim_training_size} quantifies this advantage: at any given SSIM threshold, PtychoPINN requires an order of magnitude less training data than the supervised baseline."]. This demonstrates our self-supervised framework's significantly more favorable in-distribution generalization from limited data. 

\subsubsection{Sparse Sampling rubustness}

Figure~\ref{fig:sparse_sampling} presents a three-way comparison of PtychoPINN, the supervised ML baseline (PtychoNN), and conventional rPIE on a very small dataset comprising a subsample of 512 diffraction images from the Siemens star test pattern [TODO again, this is a vertical train test split so the ml models were trained on just 256 patterns. this should be mentioned]. This qualitative test reveals the failure modes of different reconstruction methods when faced with extremely small, sparsely sampled datasets. At this sampling density, conventional rPIE is insufficiently constrained in real space and fails to converge entirely, producing only noise. The supervised PtychoNN baseline also fails, generating extremely blurred images. [TODO language, flow] In contrast, PtychoPINN successfully reconstructs both amplitude and phase with clearly resolved features, demonstrating robust generalization from extremely sparse data. 

\subsection{Out-of-distribution Generalization}

[TODO: abrupt, flow] We trained models on APS data and tested on LCLS measurements without retraining. Notably, the underlying illumination profiles are completely different [TODO refer to insets in fig:cross\_facility or a separate figure]. The resulting reconstructions provide a qualitative, but stringent, test of out of distribution generalization. [TODO I'll extend the fig by adding an in-distribution leg (train on LCLS, recon on LCLS. so it'll be top row in distribution, bottom row out of distribution, left column ptychopinn, center column ptychonn, right column ground truth]. 
The supervised PtychoNN baseline, trained on APS data, fails completely on LCLS data (Figure~\ref{fig:fivepanel}), producing %TODO: describe failure mode, e.g., "only noise" or "unrecognizable features"
. This out-of-distribution robustness stems from physics-informed training.
...
Table~\ref{tab:dose_comparison} compares Poisson NLL (Negative Log Likelihood) versus MAE loss across photon doses. At $10^3$ photons/pattern, NLL maintains SSIM = %TODO: value
while MAE drops to %TODO: value
. Figure~\ref{fig:lowcounts} shows the NLL-trained model preserves features that MAE loses to noise.

\subsection{Computational Performance}

PtychoPINN processes 2000 diffraction patterns in $1$ second on an NVIDIA RTX 3090, versus [TODO: find typical times for ePIE / rPIE and similar algorithms on GPU and insert supporting cite(s)], a [TODO speedup] speedup. 
...
In Fresnel geometry, probe curvature acts as coded illumination that provides phase diversity sufficient for $C_g{=}1$. With the diffraction constraint and dual‑resolution decoding, we obtain stable single‑shot reconstructions with modest quality loss relative to overlapped scans (Fig.~\ref{fig:overlap_free}). This unifies ptychography and single‑shot CDI in one self‑supervised solver: overlap is a design resource, not an algorithmic requirement.

\subsection*{Data efficiency and inductive bias}
PtychoPINN reaches a given SSIM with roughly an order of magnitude fewer training patterns than the supervised baseline (Figs.~\ref{fig:ssim}, \ref{fig:ssim_training_size}). From an ML perspective, incorporating the forward physics reduces the hypothesis space to measurement‑consistent functions, lowering sample complexity. From a physics perspective, the model exploits redundancy already present in the measurements (Fourier magnitudes and known probe) rather than relearning it from labels. This holds even with a few hundred training images.
...
\emph{TODO: decide how much to emphasize comparison to ML baseline. This was already covered pretty well in the first paper. On the other hand it's good to emphasize the relative generalizability and self-supervised training, since this is what makes ptychopinn  practical to use where previous ml-based solvers were not.}

\emph{cites and discussion: papers recommended by Junjin Deng. We expect adoption at APS / velociprobe, which might influence the project's technical direction. }
