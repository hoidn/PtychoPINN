...
In Fresnel geometry, probe curvature acts as structured illumination that provides phase diversity sufficient for reconstruction without overlap constraints. We obtain stable single-shot reconstructions with modest quality loss relative to overlapped scans (Fig.~\ref{fig:recon_2x2}). This unifies ptychography and single-shot CDI in one self-supervised solver, making overlaps an option instead of an algorithmic requirement. 

\subsection*{Data efficiency and inductive bias}
PtychoPINN reaches a given SSIM with roughly an order of magnitude fewer training patterns than the supervised baseline (Fig.~\ref{fig:ssim}), such that even a few hundred training images is sufficient to extract coherent features (todo language?). We found previously that this gain in generalization is primarily a result of the unsupervised training procedure's better fit to the problem's physical invariants \cite{Hoidn2023PtychoPINN}. 

\subsection*{Practical performance}
At $\sim10^4$ photons/frame, Poisson NLL preserves high-$q$ detail relative to MAE (Fig.~\ref{fig:dose}, Fig.~\ref{fig:lowcounts}). Inference runs at $\sim$2000 patterns/s on an RTX 3090, and stable reconstructions are obtained with a few hundred--low-thousands frames (Fig.~\ref{fig:ssim}). Overlap-free operation ($C_g{=}1$) yields recognizable structure with modest fidelity loss compared to overlapped scans (Fig.~\ref{fig:recon_2x2}, Table~\ref{tab:sim_lines_metrics}).
