...
| `architecture` | `Literal['cnn', 'ffno', 'fno', 'hybrid', 'stable_hybrid', 'fno_vanilla', 'neuralop_uno', 'hybrid_resnet', 'spectral_resnet_bottleneck_net']` | `'cnn'` | The generator architecture for PINN models. Used by the generator registry to select the network backbone. 'cnn' is the default U-Net based generator. PyTorch-only options include: 'ffno' (constant-resolution factorized Fourier-flow baseline), 'fno' (Cascaded FNO→CNN), 'hybrid' (Hybrid U‑NO), 'stable_hybrid' (InstanceNorm-stabilized Hybrid), 'fno_vanilla' (constant‑resolution FNO), 'neuralop_uno' (external NeuralOperator U-NO adapter for the locked Lines128 CDI path only: `N=128`, `gridsize=1`, `C=1`, `real_imag`, requires `neuraloperator==2.0.0`), 'hybrid_resnet' (FNO encoder + CycleGAN ResNet‑6 decoder), and 'spectral_resnet_bottleneck_net' (shared encoder/decoder shell with a spectral ResNet bottleneck). See `docs/architecture_torch.md` §4.1. |
