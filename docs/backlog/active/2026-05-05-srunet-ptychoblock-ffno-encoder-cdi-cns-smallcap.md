---
priority: 29
plan_path: docs/plans/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-05-srunet-ptychoblock-ffno-encoder-cdi-cns-smallcap/execution_plan.md
check_commands:
  - |
    python - <<'PY'
    ...
    if missing:
        raise SystemExit(f"missing SRU-Net PtychoBlock->FFNO encoder inputs: {missing}")
    print("SRU-Net PtychoBlock->FFNO encoder inputs present")
    PY
...
# Backlog Item: SRU-Net PtychoBlock-To-FFNO Encoder Ablation
...
- Implement and evaluate a narrow SRU-Net encoder variant that replaces the
  ...
  - an FFNO-style factorized spectral stack at the encoded resolution.
...
- Implement one explicit architecture/profile family, for example
  `hybrid_resnet_ptychoblock_ffno_encoder`, with a clearer equivalent name
...
- Treat `PtychoBlock` as a shape-preserving local-plus-spectral update. If the
  implementation needs spatial reduction, pair each of the two `PtychoBlock`
  stages with the existing SRU-Net downsample layer rather than inventing a new
  downsampling primitive.
- Use the existing FFNO-close components where practical, such as
  `SharedFactorizedFfnoBottleneck` / `FactorizedFfnoBlock`, but wire them as
  the post-downsample encoder stack rather than as a replacement for the SRU-Net
  bottleneck.
...
- Reuse completed baseline rows by lineage:
  - Lines128: `pinn_hybrid_resnet`, `pinn_hybrid_resnet_encoder_spectral_only`,
    `pinn_ffno`, and `pinn_hybrid_resnet_ffno_bottleneck` where available;
...
- Use the official `2d_cfd_cns` PDEBench dataset and the existing task-local
  CNS runner.
- Use `history_len=2` unless the implementation plan proves that the current
  matched-condition CNS authority has moved to a single completed
  `history_len=5` headline table before this item runs.
...
- The causal question is whether replacing the current SRU-Net encoder with a
  `2x(PtychoBlock + downsample) -> FFNO` encoder improves the same downstream
...
- Do not conflate this with the completed spectral-only encoder ablation. That
  ...
  different FFNO-style post-downsample encoder stack.
...
- A concise durable summary:
  `docs/plans/NEURIPS-HYBRID-RESNET-2026/srunet_ptychoblock_ffno_encoder_cdi_cns_smallcap_summary.md`
- Updates to:
