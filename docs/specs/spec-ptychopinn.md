...
- spec-ptycho-tensor-correspondence.md — Canonical TF↔PyTorch tensor/shape correspondence (grouped data, positions, probe layouts, padded-size, reassembly, complex representation).
- spec-ptycho-config-bridge.md — Normative mapping between TensorFlow dataclass configs and PyTorch config singletons (field transformations, defaults, `update_legacy_dict` flow, validation).
