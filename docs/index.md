...
**Description:** Append-only architecture row extension over the completed BRDT four-row preflight. Adds a single factorized Fourier operator (FFNO) row under the same dataset (`brdt128_decision_support_preflight`), operator authority, `born_init_image` input, 2048/256/256 split, fixed-sample IDs, normalization, and supervised+Born training contract (20 epochs, Adam @ 2e-4, batch 16, seed 42). Reuses `ptycho_torch.generators.ffno_bottleneck.SharedFactorizedFfnoBottleneck` behind a task-local `_BRDTFFNO` wrapper without touching the CDI generator registry. Decision-support read: FFNO at `parameter_count=36,674` lands between FNO vanilla and Hybrid ResNet on every blocking metric (image_relative_l2_phys 0.342 vs FNO 0.992 / Hybrid 0.319; PSNR 29.13 vs 19.89 / 29.74; SSIM 0.942 vs 0.678 / 0.947) — competitive with Hybrid ResNet at ~25% the parameter count, but does NOT displace it. Bundle is `decision_support_append_only`; the baseline four-row bundle is preserved read-only and the combined five-row view is published by lineage rather than overwrite. The `defer_after_preflight` decision in `brdt_preflight_summary.md` stands.
...
**Description:** Summary authority for backlog item `2026-05-05-brdt-supervised-born-40ep-paper-evidence`. Fresh same-contract BRDT 40-epoch rerun of exactly `hybrid_resnet` and `ffno` with Adam @ `2e-4`, batch `16`, seed `42`, and `ReduceLROnPlateau(factor=0.5, patience=2, threshold=0.0, min_lr=1e-5)`. Publishes per-row `history.{json,csv}`, `convergence_audit.{json,csv}`, `paper_evidence_gate.json`, runtime/dataset/split provenance, and the sample-`255` compare/error/source-array bundle under `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-05-brdt-supervised-born-40ep-paper-evidence/`. The paper-evidence gate did **not** pass: claim boundary stays at `decision_support_convergence_followup` and `failed_gate_checks=["git_provenance","host_provenance"]` after honest reconstruction of the overwritten `runtime_provenance.json` from `invocation.json`. The bundle remains valid same-contract decision-support context but does not authorize manuscript paper-evidence claims and does not replace CDI `lines128` or PDEBench CNS.
