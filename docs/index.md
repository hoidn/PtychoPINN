...
See also: [Prompts Catalog](../prompts/index.md) for orchestration/agent prompt files.
...
|--------|---------|-----|-----------|
| **MODULE-SINGLETON-001** (RESOLVED 2026-01-07) | *(historical)* Model expected shape (B,N,N,1) but data had (B,N,N,4) after changing gridsize | Lazy `__getattr__` loading (`ptycho/model.py:867-890`) now prevents import-time model creation; for custom workflows use `create_compiled_model()` (compiled, ready to train) or `create_model_with_gridsize()` (uncompiled) | [Finding: MODULE-SINGLETON-001](findings.md) |
...
### [CLI Workflow Guide (train/infer/compare)](WORKFLOW_GUIDE.md)
...
### [NeurIPS Steering Document (as-of 2026-04-28)](steering.md)
...
**Description:** Single-seed expanded-object CDI six-row benchmark on the frozen `natural_patches128_fixedprobe_v1` dataset. The bundle is currently `benchmark_incomplete_recovered_non_authoritative`: the original tmux launcher exited non-zero, four torch rows still report `failed/1` invocations, and the latest `--mode recollate` republication preserved those original envelopes and original-execution commits without promoting them. Use the recovered bundle for diagnostic context only; a clean `--mode benchmark` rerun whose tracked PID exits `0` end-to-end is required before the natural-patch authority can be cited as paper-grade. Separate expanded-object evidence only; does not replace the `lines128` table authority.
...
### [NeurIPS CDI Lines128 No-Refiner FFNO Table Refresh Summary](plans/NEURIPS-HYBRID-RESNET-2026/cdi_lines128_no_refiner_ffno_table_refresh_summary.md)
**Description:** Paper-local CDI refresh that swaps the active `FFNO + PINN` and `FFNO + supervised` manuscript rows, phase-zoom figure input, model-config table, efficiency table, and discovery surfaces to the corrected four-block no-refiner reruns while preserving the older `fno_cnn_blocks=2` rows as explicit proxy lineage only.
...
### [NeurIPS BRDT Task Adapters Summary](plans/NEURIPS-HYBRID-RESNET-2026/brdt_task_adapters.md)
**Description:** Locks the task-local BRDT data loader, Born-init-image derivation, model-adapter, supervised-plus-Born-consistency loss, and train/eval entrypoints sufficient for the later bounded four-row preflight. Reuses existing model bodies (U-Net, FNO vanilla via `neuralop`, Hybrid ResNet/SRU-Net) only through ordinary real-channel adapters under `scripts/studies/born_rytov_dt/`; deliberately not registered in the CDI generator registry. Encodes the first shared input contract (`born_init_image` only, direct sinogram rejected), the row metadata schema (`model`, `training`, `input_mode`, `dataset_id`, `operator_version`, `row_status`), and the unnormalize-before-physics guard via the locked `dataset_contract` helpers. Sanity execution under `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-04-29-brdt-task-adapters/fast_dev_run/` (adapter contract schema `brdt_adapter_contract_v1`). Adapter readiness only; does not authorize BRDT manuscript evidence.
...
### [NeurIPS BRDT Sinogram-Input 40-Epoch Paper Evidence Summary](plans/NEURIPS-HYBRID-RESNET-2026/brdt_sinogram_input_40ep_paper_evidence_summary.md)
**Description:** Summary authority for backlog item `2026-05-07-brdt-sinogram-input-40ep-paper-evidence` at `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-07-brdt-sinogram-input-40ep-paper-evidence/`. Records the current learned sinogram-input paired BRDT `40`-epoch rerun under the locked supervised+Born `2048 / 256 / 256` contract, the `sru_net` + corrected no-refiner `ffno` metrics and convergence read, sample-`255` visual provenance, tracked-PID exit proof, and the additive-secondary gate result under `paper_evidence_brdt_additive`.
**Keywords:** neurips, brdt, sinogram-input, 40-epoch, additive, paper-evidence, sru-net, ffno, sample-255
**Use this when:** Citing the current BRDT manuscript-side additive authority, checking the measured-sinogram learned-input contract, or locating the live `2026-05-07` visual/source-array bundle and gate/provenance artifacts.
...
**Use this when:** Checking the authoritative pure-BRDT-FFNO row, verifying the corrected no-refiner contract, or tracing how the corrected row differs from the historical local-refiner proxy.

### [NeurIPS BRDT Corrected FFNO 40-Epoch Rerun Summary](plans/NEURIPS-HYBRID-RESNET-2026/brdt_corrected_ffno_40ep_rerun_summary.md)
**Description:** Summary authority for backlog item `2026-05-06-brdt-corrected-ffno-40ep-rerun` at `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-06-brdt-corrected-ffno-40ep-rerun/`. Records the corrected paired BRDT `40`-epoch rerun under the historical `born_init_image` supervised+Born `2048 / 256 / 256` contract, the same-contract lineage against the four-row preflight plus corrected pure-FFNO prerequisite, the fresh `hybrid_resnet` + corrected no-refiner `ffno` metrics and convergence deltas, sample-`255` visual provenance, and the additive-secondary gate result under `paper_evidence_brdt_additive`. Current sinogram-input claims now route through `brdt_sinogram_input_40ep_paper_evidence_summary.md`.
**Keywords:** neurips, brdt, ffno, corrected, 40-epoch, additive, paper-evidence, sample-255
**Use this when:** Citing the preserved `born_init_image` paired BRDT `40`-epoch authority, checking the passed historical additive gate, or locating the clean Hybrid-ResNet-versus-pure-FFNO born-image-input bundle and its paper-facing visual/source-array artifacts.

### [NeurIPS BRDT FFNO Row Extension Summary](plans/NEURIPS-HYBRID-RESNET-2026/brdt_ffno_row_extension_summary.md)
**Description:** Append-only architecture row extension over the completed BRDT four-row preflight. Adds a single historical FFNO-local-refiner proxy row under the same dataset (`brdt128_decision_support_preflight`), operator authority, `born_init_image` input, 2048/256/256 split, fixed-sample IDs, normalization, and supervised+Born training contract (20 epochs, Adam @ 2e-4, batch 16, seed 42). Post-hoc caveat: this artifact was produced before the BRDT FFNO adapter was corrected to remove post-bottleneck CNN refiners, so its metrics are not a pure FFNO-paper-stack result. Current code uses `SpatialLifter -> SharedFactorizedFfnoBottleneck -> 1x1` and rejects `cnn_blocks`; a pure-BRDT-FFNO comparison requires a fresh rerun. The `defer_after_preflight` decision in `brdt_preflight_summary.md` stands.
**Keywords:** neurips, brdt, born, ffno, factorized-fourier-operator, candidate, decision-support, append-only, row-extension
**Use this when:** Citing the preserved historical FFNO-local-refiner proxy, consuming the read-only-lineage combined-bundle artifacts under `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-04-brdt-ffno-row-extension/`, or checking why the proxy row did not change the BRDT deferred-candidate-lane recommendation.
...
**Description:** Summary authority for backlog item `2026-05-05-brdt-supervised-born-40ep-paper-evidence`. Fresh same-contract BRDT 40-epoch rerun of exactly `hybrid_resnet` and the historical `ffno` local-refiner proxy with Adam @ `2e-4`, batch `16`, seed `42`, and `ReduceLROnPlateau(factor=0.5, patience=2, threshold=0.0, min_lr=1e-5)` under the historical `born_init_image` contract. Post-hoc caveat: the `ffno` row was produced before the BRDT FFNO adapter was corrected to remove post-bottleneck CNN refiners, so its metrics are not a pure FFNO-paper-stack result. The corrected pure-FFNO `20`-epoch authority now lives in `brdt_corrected_ffno_row_rerun_summary.md`, and current sinogram-input claims now route through `brdt_sinogram_input_40ep_paper_evidence_summary.md`. The paper-evidence gate did **not** pass: claim boundary stays at `decision_support_convergence_followup` and `failed_gate_checks=["git_provenance","host_provenance"]` after honest reconstruction of the overwritten `runtime_provenance.json` from `invocation.json`.
**Keywords:** neurips, brdt, born, 40-epoch, decision-support, decision_support_convergence_followup, runtime-provenance-reconstruction, sample-255, ffno, hybrid-resnet
**Use this when:** Citing the historical `born_init_image` BRDT 40-epoch decision-support bundle, checking the 20-epoch to 40-epoch deltas and convergence read, or auditing why the bundle's gate failed after `runtime_provenance.json` was reconstructed from `invocation.json`.
...
### [NeurIPS WaveBench Shared-Encoder Supervised Summary](plans/NEURIPS-HYBRID-RESNET-2026/wavebench_shared_encoder_supervised_summary.md)
**Description:** Durable repo-local supervised shared-encoder benchmark for the optional WaveBench inverse-source lane on `time_varying/is/thick_lines_gaussian_lens`. Locks the shared anisotropic measurement encoder, the five-row body roster (`cnn`, `hybrid_resnet`, `spectral_resnet_bottleneck_net`, `fno`, `ffno`), the L1/Adam/plateau training recipe, the `C=32` and `C=64` latent-channel comparison, and the row-status vocabulary distinguishing benchmark `completed` from `smoke_pass` and explicit row-level blockers. Records the locked row contract, table-ready metrics, fixed-sample reconstruction figures, and the explicit candidate-lane claim boundary that keeps this item separate from manuscript evidence and from native WaveBench reference rows.
**Keywords:** neurips, wavebench, inverse-source, shared-encoder, hybrid_resnet, ffno, fno, supervised, candidate
**Use this when:** Locating the shared-encoder row contract or table-ready metrics, comparing repo-local body architectures under one fair encoder/split/recipe contract, picking up the `C=32` versus `C=64` latent-width sensitivity rows, or confirming why this benchmark is candidate-lane only rather than manuscript or native-reference evidence.
...
### [PDEBench CNS Paper 2048cap Extension Summary](plans/NEURIPS-HYBRID-RESNET-2026/pdebench_cns_paper_2048cap_extension_summary.md)
**Description:** Durable summary for the same-contract `2048 / 256 / 256`, `history_len=2`, 40-epoch CNS authority bundle. Records the upgrade-ready audit outcome, the reused vs rerun row sources, the helper-tracked replacement `unet_strong` rerun that superseded the earlier reconstructed evidence, and the preserved capped decision-support claim boundary while keeping the older 512cap bundle as historical provenance.
**Keywords:** neurips, hybrid_resnet, pdebench, cns, 2048cap, authority, capped, decision-support
**Use this when:** Locating the current capped CNS authority bundle, checking whether all four headline rows were recovered under the same contract, or tracing how the preserved 512cap fallback relates to the promoted 2048cap authority.
...
**Description:** Durable decision-support summary for the bounded CDI-only bridge study between the existing `lines128` Hybrid/spectral anchors and three FFNO-adjacent architecture changes, including the frozen study matrix, the recovered row-local recon-path bugfix, the copy-on-write authoritative-anchor repair, the fresh DS1, linear-decoder, and FFNO-bottleneck row outcomes, the refreshed closeout integration-proof archive, and the explicit no-promotion boundary.
**Keywords:** neurips, cdi, lines128, hybrid_resnet, spectral_resnet, ffno, parameter-space, bridge, decision-support
**Use this when:** Checking whether any same-contract CDI bridge row improved on the current anchors, locating the six-row bridge-study artifact root, or auditing the row-local recovery, refreshed closeout verification, and no-promotion decision.
...
### Project Status
**Description:** `PROJECT_STATUS.md` was a dangling symlink (target removed) and has been deleted. Current status lives in `docs/plans/` (one plan per initiative) and the [Knowledge Base Ledger](findings.md).  
**Keywords:** project-status, tracking, initiatives, plans, findings  
**Use this when:** Need to see current project status and active development work — check `docs/plans/` for the active initiative plan and `findings.md` for known issues/conventions.
...
#### TF↔Torch correspondence: see [PtychoPINN Tensor Correspondence](specs/spec-ptycho-tensor-correspondence.md) in Specifications below
**Description:** Canonical, code-cited table of tensor shape/axis conventions across backends (grouped data, positions, probe layouts, padded-size formulas, reassembly, complex representation), replacing the duplicated shape table formerly in `architecture.md`.
**Keywords:** tensor-correspondence, tf-torch, shapes, probe, reassembly, padded-size
**Use this when:** Comparing or porting a tensor convention between the TensorFlow and PyTorch backends.
...
For sampling and grouping behavior, start with [GridSize & n_groups Guide](GRIDSIZE_N_GROUPS_GUIDE.md), [Commands Reference](COMMANDS_REFERENCE.md), and the data contracts above. For PtyChi/Tike comparison inputs, use the comparison spec and reconstruction scripts below.

### Decisions

- [ADR-0007: Remove Timestamped "Report Hubs"](adr/ADR-0007-remove-hubs.md) — deprecates report-hub directories in favor of per-initiative summaries and the `.artifacts/` convention.

See also: Specifications (normative contracts: `docs/specs/spec-ptycho-*`).
...
#### [PtychoPINN Tensor Correspondence (TensorFlow ↔ PyTorch)](specs/spec-ptycho-tensor-correspondence.md)
**Description:** Canonical, code-cited correspondence table of tensor shapes and axis conventions across backends: grouped diffraction/positions, probe layouts, padded-size formulas (with a documented divergence), reassembly and offset-sign convention, generator real/imag→complex conversion, and complex representation. Each row is marked Documented / Divergence / Previously-undocumented.  
**Keywords:** tensor-correspondence, tf-torch, shapes, probe-layouts, padded-size, reassembly, divergence  
**Use this when:** Comparing, porting, or auditing a tensor shape/axis convention between the TensorFlow and PyTorch backends.
...
**Superseded copy:** [`specs/overlap_metrics.md`](../specs/overlap_metrics.md) is superseded (2025-11-11, ACCEPTANCE-001) by this shard; retained for history only.
...
- **Evaluating models**: [Compare Models Spec](../specs/compare_models_spec.md) → [Studies Guide](../scripts/studies/README.md)

### By User Type
- **New Users**: [README](../README.md) → [CLI Workflow Guide](WORKFLOW_GUIDE.md) → [Training](../scripts/training/README.md)
- **Developers**: [Developer Guide](DEVELOPER_GUIDE.md) → [Testing Guide](TESTING_GUIDE.md) → [Architecture](architecture.md)
- **Researchers**: [Studies Guide](../scripts/studies/README.md) → [Model Comparison](../scripts/studies/README.md)
...
6. Record initiative status in its `docs/plans/` entry (or `docs/plans/<initiative>/summary.md`) rather than `PROJECT_STATUS.md` (removed)
...
**Tracking Backlog:** [Loader Fallback Hardening](backlog/paused/2026-02-11-tf-loader-keras3-fallback.md)
...
### [POISSON_LOSS_TF_TORCH_MISMATCH: TF/PyTorch Poisson Loss Parity](bugs/POISSON_LOSS_TF_TORCH_MISMATCH.md) - OPEN
**Description:** TensorFlow and PyTorch Poisson loss implementations are not equivalent in value or scale (TF drops the `log(y!)` constant term; Torch includes it and additionally normalizes by `mean(x)`), complicating cross-backend loss/gradient comparisons.
**Status:** Open (reported 2026-01-27)

### [FNO_INPUT_TRANSFORM_IGNORED: PyTorch FNO Input Transform Dropped](bugs/FNO_INPUT_TRANSFORM_IGNORED.md) - OPEN
**Description:** `fno_input_transform` (e.g. `log1p`, `sqrt`) is silently dropped when `create_training_payload()` builds the PyTorch `ModelConfig`, so FNO/Hybrid Lightning runs always use the default `"none"` transform regardless of the configured value.
**Status:** Open (reported 2026-01-29)

### [Object-Big Coords-Relative Regression](bugs/2026-02-05-object-big-coords-relative-regression.md) - RESOLVED
**Description:** PyTorch `object_big=True` treated scan coordinates as absolute instead of the TF-contract relative offsets, degrading reassembly/extraction and metrics once `config_factory` began defaulting `object_big=True`.
**Status:** Resolved 2026-02-05
**Fix:** `save_split_npz()` stamps `coords_type` in dataset metadata; the torch runner selects `coords_relative` explicitly and the Lightning dataloader guards against absolute coordinates when `object_big=True`.

### [Poisson/MAE Unit Mismatch in PyTorch Losses](bugs/POISSON_MAE_UNIT_MISMATCH_PYTORCH.md) - RESOLVED
**Description:** PyTorch Poisson/MAE losses mixed amplitude and intensity units — the Poisson rate did not square the observed input, and MAE compared `pred**2` to raw amplitude — contradicting the documented contract and TensorFlow behavior.
