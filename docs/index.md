...
**Description:** Summary authority for backlog item `2026-05-06-brdt-corrected-ffno-40ep-rerun` at `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-06-brdt-corrected-ffno-40ep-rerun/`. Records the corrected paired BRDT `40`-epoch rerun under the locked supervised+Born `2048 / 256 / 256` contract, the same-contract lineage against the four-row preflight plus corrected pure-FFNO prerequisite, the fresh `hybrid_resnet` + corrected no-refiner `ffno` metrics and convergence deltas, sample-`255` visual provenance, and the additive-secondary gate result under `paper_evidence_brdt_additive`.
**Keywords:** neurips, brdt, ffno, corrected, 40-epoch, additive, paper-evidence, sample-255
**Use this when:** Citing the current paired BRDT `40`-epoch authority, checking whether the corrected additive gate passed, or locating the clean Hybrid-ResNet-versus-pure-FFNO bundle and its paper-facing visual/source-array artifacts.
...
**Use this when:** Citing the BRDT 40-epoch decision-support bundle, checking the 20-epoch to 40-epoch deltas and convergence read, or auditing why the bundle's gate failed after `runtime_provenance.json` was reconstructed from `invocation.json`.
