...
- The project standard is to run tests via pytest selectors:
  ...
  pytest tests/ -q
  ```
- Whole-tree caveat: `tests/study/test_dose_overlap_*.py` import the top-level `studies.` package, which is gitignored (`.gitignore:167`). On a fresh clone without a local `studies/` checkout, collection of those modules is skipped via `pytest.importorskip`; where `studies/` is present locally, the tests still run.
...
- **Examples:**
  ...
  - `tests/torch/test_padded_size_no_jitter.py` - Ensures Torch padded size ignores max position jitter
  - `tests/torch/test_reassemble_patches_position_real_c1.py` - Validates C=1 reassembly normalization behavior in the Torch helper
...
- **Key selectors:**
  ...
  # Collect all reporting helper tests
...
When adding new command-line parameters (like `--n_subsample`):
...
class TestSubsamplingParameter(unittest.TestCase):
    def test_parameter_parsing(self):
        """Test --n_subsample parameter is correctly parsed."""
        args = parse_args(['--n_subsample', '1000'])
        self.assertEqual(args.n_subsample, 1000)
    
    def test_backward_compatibility(self):
        """Test behavior when --n_subsample not specified."""
        args = parse_args([])
