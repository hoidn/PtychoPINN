...
### `lines128-probe-channel-conditioning-ablation` (completed; append-only CDI conditioning evidence)

- Purpose: test whether appending the fixed Run1084 probe real/imag channels to
  the learned-model input improves the locked `lines128` CDI Hybrid ResNet and
  corrected pure-FFNO rows without changing the dataset/probe/optimizer/epoch
  contract.
- Governing design:
  `docs/plans/2026-04-20-neurips-hybrid-resnet-submission-design.md`
- Execution plan:
  `docs/plans/NEURIPS-HYBRID-RESNET-2026/backlog/2026-05-07-cdi-lines128-probe-channel-conditioning-ablation/execution_plan.md`
- Summary authority:
...
- Current read:
  probe-channel conditioning did not yield a promotable improvement on the
  locked CDI contract; the conditioned Hybrid row regressed on phase-side
  metrics, and the conditioned pure-FFNO row showed mixed amplitude movement
  but severe phase collapse.
- Boundary: append-only input-conditioning evidence only; it does not replace
...
- Paper efficiency table: `docs/plans/NEURIPS-HYBRID-RESNET-2026/paper_efficiency_table_summary.md` plus `docs/plans/NEURIPS-HYBRID-RESNET-2026/tables/paper_efficiency_table.{json,csv,tex}`; this groups parameter counts, provenance-backed runtime fields, hardware labels, and explicit throughput values for Synthetic CDI, PDEBench CNS, and user-approved secondary BRDT rows without ranking across benchmarks.
...
- CNS matched-condition table refresh summary: `docs/plans/NEURIPS-HYBRID-RESNET-2026/pdebench_cns_matched_condition_table_refresh_summary.md`; this is the discoverable source for the current manuscript CNS headline-table decision (matched `history_len=5`, `512 / 64 / 64`, `40`-epoch capped contract), the deterministic lane-selection audit, the emitted item-local matched-condition payload bundle, the unchanged `bounded_capped_decision_support_only` claim boundary, and the explicit note that the larger-cap `history_len=2`, `2048 / 256 / 256` bundle is preserved as bounded larger-cap context only.
- CNS U-NO matched-condition row extension summary: `docs/plans/NEURIPS-HYBRID-RESNET-2026/pdebench_cns_uno_row_extension_summary.md`; this is the discoverable source for the single same-contract `neuralop_uno_cns_base` rerun, the derived five-row `plus_uno` payload bundle, the paper-local `tables/pdebench_cns_matched_condition_metrics_plus_uno.{json,csv,tex}` assets, and the recommendation to keep the current four-row matched-condition headline table unchanged while using the five-row derivative as adjacent append-only context only.
...
python scripts/studies/grid_lines_compare_wrapper.py \
  --N 64 \
  --gridsize 1 \
  --output-dir outputs/grid_lines_n64_compare_padex_lr2e4_plateau_e20_seed13_pinn_only_retry1 \
