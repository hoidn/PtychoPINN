...
The inference bundle (`wts.h5.zip`) contains two models (`autoencoder`, `diffraction_to_obj`); external inference SHOULD invoke `diffraction_to_obj`. The authoritative external I/O shape contract is `specs/ptychodus_api_spec.md` §4.4.1.

Every other cross-backend tensor convention (positions/offsets, probe layouts, padded-size formulas, reassembly, complex representation) is consolidated in **<doc-ref type="contract">docs/specs/spec-ptycho-tensor-correspondence.md</doc-ref>** — do not duplicate shape facts here; update that shard instead.
