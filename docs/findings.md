...
| STABLE-LS-001 | 2026-01-29 | pytorch, stable_hybrid, layerscale, training-collapse | LayerScale (init=1e-3) in `StablePtychoBlock` solved the zero-gamma stagnation (STABLE-GAMMA-001) — norm weights now train to healthy values (mean~0.82-1.0). However, the stable arm still produces collapsed amplitude reconstructions (std=0, SSIM=0.277). Training trajectory shows early convergence (best val_loss=0.024 at epoch ~4, near control's 0.014) followed by collapse to val_loss=0.179 by epoch 50. The failure mode shifted from "can't learn" (architecture) to "learns then collapses" (training dynamics). Next investigation: LR warmup/cosine schedule, reduced LR, or optimizer tuning. | [Link](plans/active/FNO-STABILITY-OVERHAUL-001/reports/2026-01-29T230000Z/stage_a_arm_stable_layerscale_stats.json) | Active |
