...
| CONFIG-LOGGER-001 | 2025-10-24 | logger, execution-config, lightning, mlflow | PyTorch training uses MLFlowLogger by default (`PyTorchExecutionConfig.logger_backend='mlflow'`) to capture train/validation metrics from Lightning `self.log()` calls. Allowed backends: `mlflow` (requires mlflow package + server), `csv` (zero deps, CI-friendly), `tensorboard` (requires tensorboard from TF install), `none` (disable). Use `--logger csv` for CI or environments without MLflow. Legacy `--disable_mlflow` flag deprecated with DeprecationWarning mapping to `--logger none`. MLflow migration to Lightning MLFlowLogger tracked as Phase EB3.C4 backlog. Decision rationale and implementation evidence in governance decision approval record. | [Link](plans/active/ADR-003-BACKEND-API/reports/2025-10-20T153300Z/phase_e_execution_knobs/2025-10-23T110500Z/decision/approved.md) | Active |
