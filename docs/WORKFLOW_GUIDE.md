...
└── ...                    # Backend-specific logs and configuration snapshots
```
...
├── reconstructions.npz     # Reconstructions for further analysis
└── reconstructions_aligned.npz  # Aligned reconstructions when registration is enabled
```
...
for fold in {1..5}; do
    ptycho_train --train_data_file "fold_${fold}/full_dataset_train.npz" \
        --test_data_file "fold_${fold}/full_dataset_test.npz" \
        --output_dir "model_fold_${fold}"
    
    ptycho_inference --model_path "model_fold_${fold}" \
        --test_data "fold_${fold}/full_dataset_test.npz" \
...
ptycho_inference --model_path model/ --test_data test.npz \
    --n_subsample 1000 --n_groups 500 --output_dir infer/
```
...
- **Compare Models Spec**: [specs/compare_models_spec.md](../specs/compare_models_spec.md) - Comparison workflow contract
