...
- [ ] 3.4: Run only the highest-value CDI ablations: spectral/FNO mode or capacity, local/ResNet capacity, skip routing if already supported, narrow SRU-Net mechanism ablations such as encoder branch necessity or bottleneck block family, and focused input-conditioning ablations such as fixed-probe channels or coordinate-grid channels. Do not let ablations replace the complete `lines128` table unless a checked-in design amendment changes the required rows.
...
- [ ] The complete `lines128` CDI benchmark includes `hybrid_resnet`, `spectral_resnet_bottleneck_net`, the selected FNO comparator, and FFNO. If the only completed FFNO lineage is the historical `fno_cnn_blocks=2` local-refiner proxy, pure-FFNO claims remain blocked until the checked-in no-refiner rerun and table-refresh chain promotes the corrected row.
...
The paper-evidence package design is a binding selector input for late Phase 2 and Phase 3 packaging. CNS paper work must pass through the contract-decision, row-lock, and table/figure-bundle sequence before claims are drafted. CDI paper work must distinguish the minimum draftable `hybrid_resnet`/paired-CDI-cnn-U-Net-class/FNO subset from the complete `lines128` benchmark table required by the detailed `lines128` design, including `spectral_resnet_bottleneck_net` and FFNO. Candidate inverse-wave preflights, including Born/Rytov diffraction tomography and WaveBench inverse source, may execute concurrently under `candidate-*` roadmap phases; priority values in backlog frontmatter should control when they run relative to CDI/CNS work instead of adding brittle one-off gate prefixes. Steering on 2026-04-30 moved WaveBench inverse source ahead of remaining optional U-NO table-extension work, without promoting it into a CDI/CNS replacement pillar or authorizing paper claims before a later evidence-package amendment. After a candidate preflight completes, append-only diagnostic follow-ups may remain in the same `candidate-*` phase when they preserve the completed bundle, state an explicit diagnostic question, and do not promote the lane into paper evidence. If steering requests candidate-lane paper evidence, the promotion must be authored as a candidate-phase evidence-amendment item rather than as Phase 5 artifact assembly: it must name exact rows, artifacts, visual sample policy, and claim boundary; rerun or recover required rows under paper-evidence provenance; update the checked-in evidence package surfaces; and keep `/home/ollie/Documents/neurips` outputs paused until Phase 5. The historical BRDT paper-evidence bundle `2026-05-05-brdt-supervised-born-40ep-paper-evidence` remains secondary context only because its FFNO row is a legacy local-refiner proxy. The corrected Born-image-input reruns `2026-05-06-brdt-corrected-ffno-row-rerun` and `2026-05-06-brdt-corrected-ffno-40ep-rerun` remain the authorized pure-BRDT-FFNO comparison lineage for the historical `born_init_image` contract, with the 40-epoch rerun still downstream of the corrected 20-epoch row. They are no longer the current manuscript-evidence route for sinogram-input BRDT claims. That route now lives under `candidate-brdt-sinogram-input`: `2026-05-07-brdt-sinogram-input-adapter-contract` is the required readiness prerequisite, and `2026-05-07-brdt-sinogram-input-40ep-paper-evidence` is the additive candidate-lane evidence-amendment item that may refresh BRDT paper evidence once it lands. Until that successor run completes, the 2026-05-06 roots remain discoverable Born-image-input lineage only, and sinogram-input BRDT claims stay blocked on the new contract's 40-epoch evidence gate. Phase 5 paper-facing index work remains paused until the roadmap gate changes; do not make `phase-5-*` selectable while `docs/backlog/roadmap_gate.json` disallows it.

The active CDI FFNO no-refiner repair chain is Phase 3 contract-repair work, not an optional comparator extension. The current authoritative six-row `lines128` CDI bundle remains valid headline evidence for the non-FFNO rows, but its `pinn_ffno` member is historical `FFNO-local proxy + PINN` evidence with `fno_cnn_blocks=2`; selectors and paper-facing packaging must preserve that proxy label and must not treat the existing bundle as canonical pure-FFNO evidence until the corrected no-refiner rows are promoted by lineage. This chain is split into two priority waves: first the cheap four-block no-refiner PINN and supervised reruns plus an interim table/figure/image refresh, then the later depth-24 no-refiner PINN and supervised reruns plus a final paper refresh. The depth-24 wave must not block the four-block refresh needed for near-term manuscript metrics and visuals.

After the authoritative six-row `lines128` CDI bundle is complete, Phase 3 may also admit optional append-only comparator, objective-control, and mechanism-ablation preflights that preserve the locked base bundle and run no existing rows again. The selected `2026-04-30-cdi-lines128-uno-design-preflight` item falls in this lane: it may verify the `neuraloperator`/`neuralop.models.UNO` environment and freeze the later `neuralop_uno` row contract, but it must not reopen the required six-row roster or silently promote U-NO into Phase 3 completion criteria without a checked-in design amendment. The supervised SRU-Net objective-control row is also allowed in this lane because it fills a Table 3 pairing gap without changing the base Lines128 benchmark requirement. The SRU-Net ConvNeXt bottleneck ablation is allowed as a separate mechanism item because it changes only the bottleneck block family while preserving the completed Lines128 contract and reusing completed rows by lineage. CDI FFNO depth-24 no-refiner work is a later same-contract depth ablation and paper-refresh wave: it should reuse the corrected four-block no-refiner rows by lineage, run only the missing 24-block FFNO rows, and update final paper assets only after the cheaper four-block refresh has landed. Probe-channel and coordinate-grid conditioning ablations are allowed as separate same-contract Lines128 mechanism items for SRU-Net and corrected pure FFNO, but they must not be combined in one row or used to overwrite the completed unconditioned table authority. A bounded SRU-Net FFNO-to-PtychoBlock encoder ablation may span Phase 3 CDI and Phase 2 CNS only if it preserves the SRU-Net shell outside the encoder, launches only the new variant rows, keeps CNS small-cap results labeled as capped decision-support, and reports CDI and CNS outcomes separately. These optional Phase 3 extensions are lower priority than currently eligible Phase 2 PDEBench and candidate-lane work unless steering explicitly reprioritizes them.

After the 2026-04-21 CNS readiness and capped-comparison updates, and now that the matched-condition refresh has selected `h5_512_64_64_40ep` as the headline capped lane, the selector may choose either `phase-2-pdebench-darcy-static-operator-benchmark` as the next full-training benchmark scope or a capped CNS follow-up scope that stays explicitly decision-support-only. If the selected CNS scope is an equal-footing whole-table history-contract compare, it must rerun the full four-row shell (`spectral_resnet_bottleneck_base`, `hybrid_resnet_cns`, `fno_base`, and `unet_strong`) rather than probing only a single row. Row-local temporal-context or mechanism ablations are also allowed when they keep the fixed reduced-setting contract explicit, compare only against frozen completed authorities, report runtime and valid-window deltas where relevant, and do not masquerade as a headline model-ranking table update. These CNS follow-up compares remain benchmark-incomplete until full-training Hybrid ResNet, FNO, and `unet_strong` rows run on the full available training split. The Darcy tranche is defined by `docs/plans/NEURIPS-HYBRID-RESNET-2026/tranches/phase-2-pdebench-darcy-static-operator-benchmark/execution_plan.md` and should implement Darcy static-operator support, strong local U-Net/FNO baselines, and literature-calibrated reporting before any full-suite summary.
...
fallback provenance only. The `2026-05-04-cns-history5-comparator-gap-fill`
item is now complete, and the follow-on
`2026-05-04-cns-matched-condition-table-refresh` item also closed on
2026-05-04 with `h5_512_64_64_40ep` selected as the headline matched-
condition CNS lane. The fixed headline contract is `history_len=5`,
`512 / 64 / 64`, `40` epochs, `batch_size=4`,
`max_windows_per_trajectory=8`, train-only-normalized MSE, and the standard
CNS metric family; its claim boundary remains
`bounded_capped_decision_support_only`. The earlier locked `history_len=2`,
`2048 / 256 / 256` capped bundle remains preserved as larger-cap adjacent
context and fallback provenance, but it is no longer the active matched-
condition headline authority. New capped CNS follow-up items may reuse the
completed `history_len=5` authorities and the refreshed matched-condition
table outputs by lineage, but they must not reopen the headline contract or
rerun completed baseline rows unless a later checked-in plan explicitly scopes
another whole-table compare. The spectral-history
