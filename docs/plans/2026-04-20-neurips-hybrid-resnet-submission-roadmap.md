...
- [ ] 3.3b: Produce the minimum draftable CDI subset under one paper-grade contract: `hybrid_resnet`, U-Net/SRU-Net + PINN local convolutional baseline, and the selected FNO comparator. Use CNN/PINN only as an explicit fallback if U-Net/SRU-Net is not protocol-compatible. This subset can unblock manuscript table shells, but it is not the complete `lines128` benchmark.
- [ ] 3.3c: Complete the `lines128` benchmark table required by `docs/plans/NEURIPS-HYBRID-RESNET-2026/lines128_paper_benchmark_design.md`: `hybrid_resnet`, `spectral_resnet_bottleneck_net`, selected FNO comparator, FFNO as a CDI/grid-lines generator, and the package-level U-Net/SRU-Net + PINN local baseline. Optional classical CDI rows may be added under the same contract.
...
- [ ] 3.3e: Add targeted supervised CDI control rows only after the physics-informed Lines128 table is locked: require the local U-Net/SRU-Net + supervised control if protocol-compatible, and add at most one predeclared FNO or FFNO supervised control if it answers a concrete training-procedure question.
...
- [ ] Any supervised CDI rows are targeted training-procedure controls, not a requirement to run supervised counterparts for every architecture family.
...
current selected Phase 2 CNS follow-up is the paper table/figure bundle, which
must consume the locked `history_len=2` bounded contract, preserve the current
`capped_decision_support` claim boundary, and avoid mixing row caps in one
headline table. That bundle may first try to upgrade to a same-contract
`1024 / 128 / 128` bounded roster by reusing the existing spectral-family row
and recovering or rerunning same-cap `author_ffno_cns_base`, `fno_base`, and
`unet_strong` rows. If any required same-cap row cannot be produced honestly,
the bundle must fall back to the earlier complete `512 / 64 / 64` locked row
set rather than merging `1024 / 128 / 128` and `512 / 64 / 64` rows into one
headline bundle. The Hybrid-spectral finalist scaling follow-up at
`2048 / 256 / 256` is now a completed capped CNS lane that depends on the
