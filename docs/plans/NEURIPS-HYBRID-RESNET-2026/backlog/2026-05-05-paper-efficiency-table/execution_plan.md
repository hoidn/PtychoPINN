...
- Do not include the historical BRDT `ffno` row as pure FFNO evidence. It was
  generated by the old FFNO-local-refiner proxy. Include BRDT FFNO efficiency
  only after the corrected no-refiner rerun lands, or label the old row
  explicitly as a proxy context row.
- Do not include the historical CDI `pinn_ffno` or `supervised_ffno` rows as
  pure FFNO evidence. Those rows used `fno_cnn_blocks=2`; include them only as
  `FFNO-local proxy` context until
  `2026-05-06-cdi-lines128-no-refiner-ffno-table-refresh` promotes corrected
  no-refiner rows.
...
def test_group_rows_by_benchmark_preserves_claim_boundaries():
    rows = [
        ...
        {"benchmark": "BRDT", "row_id": "ffno", "claim_boundary": "historical_brdt_40ep_proxy_context"},
    ]
...
Include exactly the `hybrid_resnet` and historical `ffno` rows. Extract `runtime.parameter_count`, `runtime.wall_time_train_s`, `runtime.wall_time_eval_s`, `runtime.device_name`, and compute inference samples/s as `split_counts.test / runtime.wall_time_eval_s`. Use claim boundary `historical_brdt_40ep_proxy_context` unless the corrected no-refiner BRDT FFNO rerun has regenerated the BRDT efficiency rows.

The older BRDT preflight table may be listed as superseded context, but it must not replace the 40-epoch historical context rows. The historical `ffno` row is a local-refiner proxy and must not be labeled as pure FFNO or as paper-approved secondary BRDT evidence.
