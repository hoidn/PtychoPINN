...
global mixing with convolutional local recovery. On coherent diffractive imaging
...
The remaining question is architectural. Wave inverse problems are simultaneously
global and local. Propagation and diffraction couple distant object regions
through Fourier-domain physics, while reconstruction quality is determined by
sharp local features, phase structure, and fine texture. Convolutional
encoder-decoders are strong local image priors, but their long-range interaction
capacity grows indirectly through depth and pooling. Fourier neural operators
provide global mixing in a single layer, but spectral truncation can suppress the
high-frequency detail needed for accurate reconstruction.

We therefore study SRU-Net, a spectral-residual U-Net for physics-constrained
wave reconstruction. The model uses Fourier-style spectral mixing for global
context, convolutional paths for local structure, and a U-shaped residual
encoder-decoder for image-domain refinement. The central hypothesis is not that
Fourier layers should replace convolutions, but that wave inverse networks
should combine global operator mixing with local feature preservation.
...
% figure_metadata: task=CDI_lines128; paper_copy=figures/cdi_synthetic_line_pattern_srunet_fno_ffno.png; visible_columns=gt,pinn_fno_vanilla,pinn_ffno,pinn_hybrid_resnet; source=.artifacts/work/NEURIPS-HYBRID-RESNET-2026/backlog/2026-04-27-cdi-ffno-generator-lines-best-config/lines128_ffno_vs_hybrid_resnet plus complete Lines128 FNO row
\begin{figure}[t]
\centering
...
model, or whether it should combine local recovery with explicit spectral
mixing.
...
recovery.} Top: $x$ is the task input, $\phi_0$ projects $C_{\rm in}$ input
...
\subsection{CDI benchmark on synthetically-generated diffraction data}
...
diffraction data protocol. Figure~\ref{fig:cdi_main_qualitative} shows the matched qualitative
comparison. Table~\ref{tab:cdi_lines128_complete} reports amplitude and phase
metrics, including direct squared-error and root-squared-error measures.
...
\caption{\textbf{Matched Lines128 CDI benchmark with direct error metrics.}
All rows use the same $N=128$ line-pattern protocol. RMSE is derived as
$\sqrt{\mathrm{MSE}}$ from the recorded MSE. Lower MAE, MSE, and RMSE are
better; higher SSIM is better.}
\label{tab:cdi_lines128_complete}
\resizebox{\textwidth}{!}{\input{tables/cdi_lines128_metrics_extended.tex}}
\end{table}
...
phase-side reconstruction among the PINN-trained models, while the spectral
SRU-Net variant gives the best amplitude-side MAE and SSIM. FFNO improves
strongly over the CNN and FNO PINN models but does not match the hybrid models
on this CDI task.
...
low-sample CDI setting. U-NO is competitive with FNO and FFNO on phase error,
but its amplitude reconstruction remains behind the hybrid SRU-Net rows.

\subsection{PDEBench CNS results}

Table~\ref{tab:cns_bundle} reports the best observed result for each model
family by aggregate error. The entries are not a matched head-to-head benchmark: they
differ in temporal context, training-set size, or training time. They summarize
the completed CNS evidence. FFNO gives the
lowest aggregate and Fourier-band errors. Among the SRU-Net, FNO, and U-Net
comparisons, SRU-Net$^*$ gives lower aggregate error than FNO and U-Net. The U-Net result is
much less accurate in these studies, consistent with the need for long-range
mixing in compressible-flow prediction.

% table_metadata: task=PDEBench_CNS; table_type=best_observed_by_family; selection_metric=relative_l2; data_split=train/val/test_trajectories=512/64/64; training_epochs=40; sources=paper_table_bundle,history_len3plus_summary,spectral_history_len4plus_summary; excludes_in_progress_2048cap_extension_until_row_lock
...
\caption{\textbf{PDEBench CNS results.} Rows show the best observed
aggregate-error result for each model family across completed CNS studies. All
rows use the 512/64/64 train/validation/test split and 40 training epochs;
the number of input time steps differs by family. SRU-Net$^*$ denotes the
spectral-bottleneck SRU-Net variant used in the CNS studies. Low-, mid-, and
high-band RMSE are Fourier-band errors. Lower values are better.}
\label{tab:cns_bundle}
\begin{tabular}{p{0.95in}crrrr}
\toprule
Model & Input time steps & nRMSE $\downarrow$ & Low RMSE $\downarrow$ &
Mid RMSE $\downarrow$ & High RMSE $\downarrow$ \\
\midrule
FFNO & 2 & \textbf{0.028} & \textbf{1.612} &
\textbf{0.076} & \textbf{0.121} \\
SRU-Net$^*$ & 5 & 0.033 & 1.854 & 0.171 & 0.262 \\
FNO & 3 & 0.057 & 3.160 & 0.173 & 0.610 \\
U-Net & 2 & 0.676 & 38.980 & 0.507 & 1.333 \\
\bottomrule
\end{tabular}
\end{table}
...
test sample comes from the matched 2-input-time-step 512/64/64 source
comparison used to assemble the initial CNS visual comparison. Error panels expose the local
high-gradient structure that is suppressed most strongly by the FNO and U-Net
rows in this example.}
\label{fig:cns_sample_predictions}
\end{figure}

The CNS table uses 512 training trajectories and serves as secondary
...
bounded BRDT preflight. The classical Born row is shown as blocked because the
ODTbrain reference inversion is unavailable in the local environment.}
...
networks that encode both propagation-scale coupling and local image structure.
