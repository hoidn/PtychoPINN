...
Recent extensions of the FNO architecture attempt to mitigate the
high-frequency and local-detail limitations of low-mode spectral convolution
by coupling global Fourier layers with spatially local operators
\cite{ufno,localno}. These convolutional, U-Net, or localized-kernel branches
provide a complementary inductive bias for fine-scale structure while
preserving the long-range mixing of Fourier layers. Such hybrids improve some
PDE benchmarks, but they are not uniformly better than strong spectral-only neural
operators architectures such as the factorized Fourier neural operator (cite). (cites)

Coherent diffractive imaging and phase retrieval are a challenge
...
FNO + PINN, and CNN + PINN denote architectures trained through the CDI
diffraction forward model rather than through direct supervised object targets.
The current FFNO rows are labeled FFNO-local proxy because they used
post-stack local CNN refinement; no-refiner FFNO rows are being rerun before
canonical FFNO claims are made. Rows without ``+ PINN'' denote non-PINN
baselines.
...
The synthetic CDI benchmark compares local, spectral, and hybrid
...
\caption{\textbf{Synthetic CDI model comparison under physics-informed training.}
...
The synthetic CDI benchmark shows a clear separation between purely local, purely
...
phase SSIM. The historical FFNO-local proxy improves strongly over the CNN and
FNO PINN models but does not match SRU-Net's amplitude-side reconstruction on
this CDI task.
The supervised FFNO control improves phase MAE but collapses amplitude quality
relative to the FFNO-local proxy + PINN row. This historical objective-control
comparison will be replaced by the no-refiner FFNO reruns before being used as
canonical pure-FFNO evidence. The same pattern appears for U-NO: supervised
training improves phase MAE, while PINN training gives stronger amplitude
reconstruction and phase SSIM.
...
from simulated wave measurements. The current historical 40-epoch bundle
compares SRU-Net with an FFNO-local-refiner proxy under the same supervised
object loss plus Born-consistency objective. SRU-Net gives the strongest
image-space reconstruction in that bundle. A corrected no-refiner BRDT FFNO
rerun is pending, so the proxy row is not used as a pure FFNO-paper-stack
claim.
...
supervised object loss plus Born-consistency loss. The model-based Born inverse
is a non-neural reference. This secondary study is reported as transfer context
rather than as a primary benchmark. Throughput is measured over the 256-sample
test split on the same GPU. Lower error is better; higher PSNR, SSIM, and
throughput are better. Parameter counts are reported for learned neural rows;
exact adapter settings are reported in
Appendix Table~\ref{tab:model_config_by_benchmark}.}
...
the historical FFNO-local-refiner proxy. The model-based Born inverse provides a
non-neural reference. Both neural rows evaluate in under 0.7 seconds over the
256-sample test split, roughly an order of magnitude faster than the iterative
model-based inverse. These results support the transfer behavior of SRU-Net,
while the paper's main quantitative claims remain the CDI and CNS benchmarks.
...
CNS benchmark. The rows separate the tested architectural or input-context axis
from the run context, showing how temporal context, spectral capacity, and
bottleneck depth affect aggregate and Fourier-band errors. All rows in this
summary use a 40-epoch training budget; longer convergence follow-ups are kept
out of the ablation table.
