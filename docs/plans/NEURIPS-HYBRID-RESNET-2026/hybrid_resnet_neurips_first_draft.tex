...
Problems such as phase retrieval, inverse scattering, and forward PDE prediction combine long-range coupling with localized structure. In lensless computed imaging, for example, each pixel in the measured image depends on the entirety of the target object, while at the same time it is essential to process local textural information (i.e. speckle). (cites)

Similarly, in fluid dynamics large-scale coherent dynamics coexist with localized features such as shocks and vortices. Consequently, classical numerical PDE solvers require fine discretization (for forward-solvers) or iterative convex optimization (for inverse-solvers). These techniques are highly computationally expensive. 

Standard computer vision architectures adapted to physics-prediction tasks provide fast approximate solutions (TODO say how much faster, motivate why this is important) but are typically too inaccurate to replace conventional numerical solvers in domains with prominent scale-mixing (cites). The accuracy gap traces to specific architectural mismatches: architectures that are efficient on natural images, such as CNNs, lack global receptive fields and are consequently poorly suited to modeling long-range interactions. 

This is a direct contrast to Fourier neural operator (FNO) networks, which, while highly suited to modeling continuous structure in physical systems, rely on spectral truncation and thus cannot represent sharp features (cites). 

(TODO maybe explain why we're not considering diffusion or transformer / attention models (basically they're too slow, too data hungry)). 
(TODO transition to instruding FNO, explaing why it's a strong class of models).

Recent extensions of the FNO architecture mitigate these limitations 
...
PDE benchmarks, but they are not uniformly better than strong spectral-only 
architectures such as the factorized Fourier neural operator (cite). (cites) 

Coherent diffractive imaging and phase retrieval is a scientfically interesting (TODO why?) 
task that reveals the failure modes of existing approaches because it requires ingestion \emph{and} synthesis of both high- and low-spatial frequency information. 
Our experiments demonstrate that FNO-based architectures that are strong baselines (or SOTA-class) on difficult forward-prediction tasks such as compressible Navier-Stokes fail entirely to predict real-space images from diffraction measurements (cites or reference to relevant fig in this manuscript), though they still outperform convolutional networks. 

(TODO maybe explain why we're not considering diffusion or transformer / attention models (basically they're too slow, too data hungry)). 

Our contributions in this paper are twofold. First we introduce SRU-Net, a computationally-efficient spectral-convolutional architecture that yields better performance on coherent diffractive imaging than existing neural operator and hybrid architectures. Second, we propose a physics-consistency training objective that significantly improves prediction accuracy, regardless of model class. 

(TODO summarize the architecture, introduce the other benchmarks, enumerate the baselines, etc.)

%Each encoder block combines a truncated Fourier mixing branch
...
the same objective.  We include ablations that vary the network's topology, connectivity, and the placement of spectral and convolutional components. These experiments confirm the importance of routing residual connections outside layer activations, as discovered by (cite FFNO authors), and illustrate the distinct roles of the inverse-mapping and image synthesis components of the overall network. 

We further test SRU-Net on a Born--Rytov diffraction tomography (BRDT)
inverse-scattering task of recovering a spatial scattering
potential from wave measurements. This evaluation provides a second wave-propagation inverse problem setting 
...
boundaries. Spectral mixing therefore helps interpret globally coupled wave measurements, but a real-space synthesis path remains important for recovering localized geometry. (TODO does this last sentence belong somehwere else?)

Third, we evaluate SRU-Net on PDEBench compressible Navier--Stokes. It outperforms standard PDEBench baselines but falls short of FFNO. FFNO's much larger depth ($24$ spectral layers vs. SRU-Net's $4$) represents a scaling dimension that we leave unexplored because it comes at the cost of reduced inference throughput. \cite{pdebench}

(TODO should there be an equal-network depth comparison? i.e. SRUNet vs. FFNO with 4 layers instead of 24)
