...
| --- | --- | --- | ---: | --- |
| SWE | `swe` | `2D_rdb_NA_NA.h5` | 6.2 GB | already staged locally under `/home/ollie/Documents/pdebench-data/swe/` |
| Darcy Flow | `darcy` | `2D_DarcyFlow_beta1.0_Train.hdf5` | 6.2 GB | to stage or locate |
| 2D diffusion-reaction | `2d_reacdiff` | `2D_diff-react_NA_NA.h5` | 13 GB | to stage or locate |
...
- Expected file: `2D_DarcyFlow_beta1.0_Train.hdf5`, with exact field names to confirm during preflight.
- Likely input/target: coefficient or forcing-like field to solution field. The exact contract must be derived from HDF5 metadata and PDEBench conventions before training.
...
- Split policy: deterministic sample-level train/validation/test manifest.

### 2D Diffusion-Reaction

- Task type: dynamic one-step next-state prediction.
- Expected file: `2D_diff-react_NA_NA.h5`, with exact field names, channel count, and axis order to confirm during preflight.
- Primary metric: `err_nRMSE`.
- Secondary metrics: `err_RMSE`, runtime, peak GPU memory, and optional rollout metrics only after one-step evidence is stable.
...
3. Darcy support with static operator-map data tests and smoke readiness.
...
The first next tranche should not launch expensive training. It should unblock exact data access, HDF5 schemas, and the minimal adapter design.
