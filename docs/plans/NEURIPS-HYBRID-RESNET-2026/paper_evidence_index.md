...
| `2026-04-29-brdt-dataset-preflight` | Candidate / Born-Rytov diffraction tomography | `feasibility` | smoke dataset only (16 train / 4 val / 4 test, `N=D=128`, `A=64`, `wavelength_px=8.0`, `n_m=1.333`); no adapters or training | [BRDT dataset preflight summary](brdt_dataset_preflight.md) | `.artifacts/NEURIPS-HYBRID-RESNET-2026/backlog/2026-04-29-brdt-dataset-preflight/` | Locks the BRDT smoke dataset around physical `q(x,z)=k_m^2((n/n_m)^2-1)`, train-only normalization, deterministic disjoint object seeds, and the unnormalize-before-physics rule. Consumes the operator authority from `operator_validation.json` rather than redefining it. Does **not** authorize manuscript evidence; adapters and four-row preflight remain follow-up backlog items. |
