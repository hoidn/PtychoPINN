# Lines128 Classical CDI Execution Authority

- Date: `2026-04-30`
- Backlog item: `2026-04-29-cdi-lines128-classical-baseline-feasibility`
- Scope: protocol-compatibility audit for one optional classical CDI extension
  against the frozen `lines128` CDI paper bundle.
- Primary claim boundary: `lines128_classical_optional_extension`

This authority freezes the classical-feasibility contract without reopening the
authoritative six-row `lines128` CDI paper bundle. The existing six-row
`paper_complete` bundle remains the main CDI claim authority; this item may
either publish one adjacent same-contract classical row or close truthfully as
`not_protocol_compatible`.

Preserved prerequisite root:
...
The classical audit may reference preserved same-contract evidence, but it must
not rewrite or relaunch the accepted six-row root in place.

<!-- lines128_classical_cdi_authority_json:start -->
{
  "claim_boundary": "lines128_classical_optional_extension",
  "allowed_closeout_states": [
    "classical_row_executed",
    "not_protocol_compatible"
  ],
  "fixed_contract": {
    "N": 128,
  ...
  "required_paper_grade_provenance_fields": [
    "invocation",
    "config",
    "git",
    "environment",
  ...
  "required_extension_bundle_artifacts": [
    "metrics.json",
    "metrics_table.csv",
    "metrics_table.tex",
  ...
  "downgrade_rule": "Any missing required field or extension-bundle artifact downgrades the row to benchmark_incomplete and prevents same-contract publication.",
  "preserved_roots": {
...
<!-- lines128_classical_cdi_authority_json:end -->
