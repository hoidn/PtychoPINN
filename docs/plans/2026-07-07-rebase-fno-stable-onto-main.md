...
### 2026-07-08 — POST-CUTOVER AMENDMENT (main tip replaced: `f66e8d43` → `868a461d` → `3124d20c`)
...
- **Final main tip = `3124d20c`** (tree `2b01b128`; supersedes intermediate `868a461d`, which held five dirs and had GH `pytest-cpu` green before `configs/` was added to the holds). Local gate on each extracted tree object: 540 passed / 0 failed. Force-pushed to internal and origin main (ruleset temporarily disabled by user for the window; re-enable after final green).
