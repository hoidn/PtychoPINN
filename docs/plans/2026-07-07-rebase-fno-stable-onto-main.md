...
- **CI flake identified (decision pending):** `tests/torch/test_cross_branch_rectangular_parity.py` (5 bit-exact assertions) failed on the `3124d20c` run but passed on the `868a461d` and `bab84837` runs — all three trees identical in code/tests, all three runs on torch 2.13.0. Local run on CI-exact software (torch 2.13.0+cpu, numpy 2.4.6, tensorflow-cpu 2.21) passes. Diagnosis: bit-exact float assertions against fixtures frozen on one CPU are not portable across GitHub's mixed runner microarchitectures (SIMD kernel dispatch differs). Expect ~random gate failures on future pushes until resolved: either deselect the 5 node IDs in `ci/known_failures.txt` (ratchet exception, user sign-off) or the TF-parity initiative re-freezes with tolerance-based assertions.
