...
### 2026-07-08 — POST-CUTOVER AMENDMENT (main tip replaced: `f66e8d43` → `868a461d` → `3124d20c` → `bab84837`)

User-directed changes after the initial public push of `f66e8d43`:

1. **Directory holds.** The overlay's diff vs `32082e91` carried changes under `archive/`, `configs/`, `datasets/`, `docs/`, `plans/`, `prompts/` that must not appear on main. The main tip was amended (same parent `32082e91`) with those six directories held **byte-identical to base** (`prompts/` absent at base → removed). `configs/` was adjudicated separately: the six overlay-added yamls are stale 2025-10-16 study configs with zero references in main's code/tests/CI (the one `examples/` reference to `gridsize2_minimal.yaml` was already dangling on old public main); the base `comparison_config.yaml` used by `scripts/run_comparison.sh` is kept by the hold. Invariant verified: `git diff 32082e91..3124d20c -- archive configs datasets docs plans prompts` is empty; nothing else differs from the overlay except the CI repair below.
...
- **Final main tip = `bab84837`** (tree `2b01b128`; user-authored release-style commit message; supersedes intermediates `868a461d` — five-dir hold — and `3124d20c` — six-dir hold, terse message). Local gate on each extracted tree object: 540 passed / 0 failed. GH `pytest-cpu` green on the final tip. Force-pushed to internal and origin main (ruleset temporarily disabled by user for the window; re-enabled after final green).
- **CI flake identified and resolved (2026-07-09):** `tests/torch/test_cross_branch_rectangular_parity.py` (5 frozen-float assertions) failed on the `3124d20c` run but passed on the `868a461d` and `bab84837` runs — all three trees identical in code/tests, all three runs on torch 2.13.0. Local run on CI-exact software (torch 2.13.0+cpu, numpy 2.4.6, tensorflow-cpu 2.21) passes. Diagnosis: exact float comparisons against fixtures frozen on one CPU are not portable across GitHub's mixed runner microarchitectures (SIMD kernel dispatch differs). The cross-build fixture checks now use a shared `rtol=1e-5, atol=1e-6` contract, with a fixture-backed boundary regression that accepts roundoff and rejects material drift; same-build or structural comparisons retain any exactness claims.
...
- **Go-forward addendum:** changes under the six held directories stay off main — cherry-picks to main must not touch them, and any future transform run must encode the holds in the `scripts/main_overlay/` inventory (exclude + graft-from-base) before use.

### 2026-07-08 — SECOND AMENDMENT WAVE (main tip: `bab84837` → `8bd24e58`)

Further user-directed exclusions, same amend-in-place mechanics (parent `32082e91`, tree `4593800a`):

1. **Directory removals:** `history/` (1 file) and `loaders/` (3 files) — present at base but deprecated — REMOVED outright (main now intentionally diverges from base there); `workflows/` (65 files, overlay-added orchestration library) removed. Verified: no imports or runtime path references anywhere in the tree.
2. **`hybrid`/`stable_hybrid` architectures removed** (SDD: implementer + independent review, both approved): `HybridUNOGenerator`/`StableHybridUNOGenerator`/`HybridGenerator`/`StableHybridGenerator` deleted from `ptycho_torch/generators/fno.py`; registry entries, config literals (`ptycho/config/config.py`, `ptycho_torch/config_params.py`), model dispatch, 12 hybrid-only `PyTorchExecutionConfig` knobs, and all hybrid test cases removed — 22 files. Kept intact: `cnn`/`ffno`/`fno`/`fno_vanilla`/`neuralop_uno`, shared spectral blocks (`PtychoBlock`, `StablePtychoBlock`, `SpatialLifter`), CascadedFNO. Reviewer independently reproduced the gate on both sides of the diff: 540→530 passed with exact accounting of the 10 removed non-slow cases; zero architecture-literal leftovers in `ptycho*/tests`. Inert `hybrid` label/CLI plumbing remains in `scripts/studies/*` paper tooling (documented, out of scope).
3. **Commit message** (user-authored) updated accordingly: architecture set `{cnn, ffno, fno, fno_vanilla, neuralop_uno}`; dropped "hybrid encoder fusion/gating" and the `workflows/` clause.
4. **CI harness backported to fno-stable** (`a73303af`, pushed internal): `ci/*`, `requirements-ci.txt` (incl. neuraloperator pin), `.github/workflows/tests.yml`, `docs/ci.md` — previously main-only. Workflow triggers only on pushes to `main`, so it stays advisory on fno-stable.
- Gate on the extracted tree object `4593800a`: **530 passed / 0 failed**. Force-pushed to internal + origin main.
