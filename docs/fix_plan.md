...
**Last Updated:** 2025-11-11 (Galph s=266)
...
- Latest Attempt (2025-11-11T215600Z): done — Interpreter discipline enforced (PYTHON-ENV-001). Orchestrator subprocesses now use the active interpreter; loop/supervisor wrappers resolve `PYTHON_BIN`. Next: implement generator dose filter and pass `--dose` to Phase C.

Attempts History (archived):
- See `docs/archive/2025-11-06_fix_plan_archive.md` for full attempt logs and evidence links.

Do Now (updated):
- Implement `--dose/--doses` in `studies/fly64_dose_overlap/generation.py` and filter the generation loop.
- In `plans/active/.../bin/run_phase_g_dense.py` Phase C command, append `--dose {dose}` once supported.
- Re-run counted pipeline for dose=1000 using interpreter-safe invocation from `input.md`; verify `{analysis}` contains SSIM grid summary/log, verification report/log, highlights check log, metrics summary/digest, and artifact inventory.
