...
- [ ] DEBUG-SIM-LINES-DOSE-001.F1: Await Maintainer <2> acknowledgement of the delivered bundle. Latest scan (artifacts under `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-23T133500Z/`) adds per-actor follow-up tables for the current drop, but the persistent history artifacts still omit those outbound metrics. Upcoming drop must persist the follow-up stats into the JSONL/Markdown history logs and the history dashboard (new "Ack Actor Follow-Up Trends" table) so Maintainers <2>/<3> can see the long-term cadence without opening the latest snippet. Tests: add a targeted regression (e.g., `tests/tools/test_check_inbox_for_ack_cli.py::test_history_followups_persist`) plus the existing suite `pytest tests/tools/test_check_inbox_for_ack_cli.py -q` and `pytest tests/test_generic_loader.py::test_generic_loader -q`.
