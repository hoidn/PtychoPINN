...
- Attempts History:
  ...
  - *2026-01-20T112000Z:* **Phase D1 (initial) COMPLETE** — Extended `bin/compare_sim_lines_params.py` with loss weight extraction (mae_weight, nll_weight, realspace_weight, realspace_mae_weight) by instantiating TrainingConfig per scenario. Generated Markdown + JSON diff artifacts that appeared to show a MAE/NLL inversion (`mae_weight=1.0, nll_weight=0.0` in dose_experiments vs `mae_weight=0.0, nll_weight=1.0` in sim_lines_4x).
