...
- [ ] DEBUG-SIM-LINES-DOSE-001.F1: Await Maintainer <2> acknowledgement of the delivered bundle. Latest scan (artifacts under `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-23T123500Z/`) shows SLA breach plus Maintainer <3> escalation brief, but we still lack per-actor outbound cadence data proving how often Maintainer <1> is pinging each actor. Upcoming drop must add follow-up tracking to the CLI (per-actor last outbound timestamp/hours/count), surface it in Markdown outputs, ship a regression test, and capture a fresh evidence bundle once the new instrumentation lands. Test selectors: `pytest tests/tools/test_check_inbox_for_ack_cli.py -q` (full suite including the new follow-up tracker test) and `pytest tests/test_generic_loader.py::test_generic_loader -q`.
