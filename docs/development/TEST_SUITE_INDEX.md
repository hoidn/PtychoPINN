...
| `test_dose_overlap_training.py` | Tests Phase E training job matrix enumeration. Validates that build_training_jobs() produces correct job count (9 jobs = 3 doses × 3 variants) with proper dataset paths and metadata. | `test_build_training_jobs_matrix` | `pytest tests/study/test_dose_overlap_training.py::test_build_training_jobs_matrix -vv` | Tests job builder logic with dependency injection (tmp_path fixtures for Phase C/D datasets). Validates baseline (gs1) and overlap (gs2 dense/sparse) job generation. |
