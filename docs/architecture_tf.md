...
- Reassembly: `ptycho.image.reassemble_patches` ↔ Torch has two paths, not a single "MVP parity, native planned" story — the `ptycho_torch.helper.reassemble_patches_position_real` MVP path (TF-parity) and the `ptycho_torch/reassembly.py` barycentric/VarPro path (inference-only); see <doc-ref type="doc">docs/architecture_torch.md</doc-ref> §4 and §5, and <doc-ref type="spec">docs/specs/spec-ptycho-tensor-correspondence.md</doc-ref> §2.3 for shapes and the offset-sign convention
