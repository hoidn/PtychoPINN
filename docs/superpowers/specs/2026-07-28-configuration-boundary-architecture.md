...
- The accepted simulation design proves one narrow, representation-preserving
  use of Pydantic. It does not authorize migration of another family.
...
|---|---|---|
| Simulation recipe | Existing complete-document adapter | Adopted |
...
- The public architecture domain is the five values shared by the public
  dataclass, Torch registry, ModelSpec, configuration guide, and corrected
  external API contract. Future expansion requires an explicit registry and
