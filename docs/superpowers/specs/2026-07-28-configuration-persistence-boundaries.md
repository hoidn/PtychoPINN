...
| `ModelSpec` internal schema | Torch construction and reload | Sealed model graph/state identity |
| Torch artifact identity | Versioned artifact codec | Versioned Torch runtime configuration identity |
...
- explicit versioned field tuples independent of current dataclass reflection;
- deterministic `torch-model-spec-v1` to `torch-model-spec-v2` upgrade;
...
- backend and `torch-artifact-v1` / `torch-artifact-v2` schema markers;
...
Therefore they are observability and compatibility data, not sealed Torch model
identity.
...
### Current dataclass fields as the ModelSpec schema
...
differs from the versioned Torch artifact contract.
...
1. byte- or value-exact `torch-model-spec-v1` / `torch-model-spec-v2` and
   `torch-artifact-v1` / `torch-artifact-v2` fixtures;
