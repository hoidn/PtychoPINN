...
Verify current behavior with `RawData.generate_grouped_data()` and `tests/test_coordinate_grouping.py`.

Expected behavior:
- First-run performance should be fast without cache generation.
- Reproducible runs should use an explicit seed.
- No grouping cache files should be created.
