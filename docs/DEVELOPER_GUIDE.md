...
**IDL-style component contracts:** For signature/input/output/dependency/error-mode contracts of core data and workflow APIs (`RawData`, `PtychoDataContainer`, `run_grid_lines_torch`, `run_cdi_example_torch`, `resolve_generator`), see <doc-ref type="guide">docs/architecture_torch.md</doc-ref> §Component Contracts.
...
**The Canonical Data Format:** All tools that produce or consume standalone NPZ datasets for training or evaluation **MUST** adhere to the format defined in **<doc-ref type="contract">docs/specs/spec-ptycho-core.md</doc-ref>**. This document is the single source of truth for standalone-NPZ array shapes, key names, and data types; Ptychodus HDF5 product files are governed by `specs/data_contracts.md`.
