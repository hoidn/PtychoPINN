...
The `torch-model-spec-v2` identifier denotes a frozen field set. This branch
currently computes `MODEL_SPEC_V2_MODEL_FIELDS` from the live dataclass, so
before adding a field, first materialize the existing v2 names as an explicit
constant; otherwise the new field would silently change the meaning of v2.
Then:

1. leave the v1 and newly frozen v2 tuples unchanged;
2. introduce a v3 tuple containing the new fields;
3. make `torch-model-spec-v3` the current schema;
...
8. update the hard-coded v2 imports and field-shape classification in
