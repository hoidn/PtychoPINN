...
Run from the repository root, for this checkout `/home/ollie/Documents/PtychoPINN`.
...
2. Clear stale local workflow state and artifacts for the run you are restarting (`state/*.txt` and stale run-scoped files under `artifacts/{work,checks,review}/runs/<run_id>/` if present).
...
cd /home/ollie/Documents/PtychoPINN
export PYTHONPATH=/home/ollie/Documents/agent-orchestration
python -m orchestrator resume <run_id> --debug \
