...
| `--logger` | str | `'mlflow'` | Experiment tracking backend. Options: `'mlflow'` (MLFlowLogger, requires mlflow package and server URI configuration), `'csv'` (CSVLogger, zero dependencies, CI-friendly, stores metrics in `{output_dir}/lightning_logs/version_N/metrics.csv`), `'tensorboard'` (TensorBoardLogger, enables rich visualization via `tensorboard --logdir {output_dir}/lightning_logs/`), `'none'` (disable logging, discards all metrics from `self.log()` calls). Use `'none'` with `--quiet` to suppress all output. |
...
The default MLflow logger integrates with MLflow tracking servers for experiment management and requires the `mlflow` package plus a configured tracking URI. The CSV backend captures all metrics logged via `self.log()` calls in the Lightning module (train/validation losses, learning rates) without external dependencies; metrics are saved as CSV files under `{output_dir}/lightning_logs/version_N/metrics.csv` for easy parsing and analysis and are recommended for CI/automated workflows. The TensorBoard backend enables interactive visualization (line plots, histograms) but requires the `tensorboard` package (auto-installed via TensorFlow dependency). Use `--logger none` to completely disable metric capture (e.g., for quick smoke tests or when metrics are not needed).
