...
- Recommended project baselines live in [docs/model_baselines.md](/home/ollie/Documents/tmp/PtychoPINN/docs/model_baselines.md). This guide defines available knobs and execution behavior; it is not the authority for which concrete parameter combination should be treated as the current best-practice starting point.
- `config.model.architecture`: Generator architecture for PINN models (`'cnn'`, `'ffno'`, `'fno'`, `'hybrid'`, `'stable_hybrid'`, `'fno_vanilla'`, `'neuralop_uno'`, `'hybrid_resnet'`). Default: `'cnn'`. All architectures train via Lightning with the full physics pipeline. The `'stable_hybrid'` variant uses InstanceNorm-stabilized residual blocks (Norm-Last, zero-init gamma/beta) for improved training stability in deep FNO stacks; see `docs/strategy/mainstrategy.md §1.A`. The `'ffno'` baseline runs a constant-resolution factorized Fourier-flow stack with the same CDI real/imag output contract, `'fno_vanilla'` runs a constant‑resolution FNO stack, `'neuralop_uno'` wraps external `neuraloperator==2.0.0` U-NO for the locked Lines128 CDI path only (`N=128`, `gridsize=1`, `C=1`, `real_imag`), and `'hybrid_resnet'` uses an FNO encoder with a CycleGAN ResNet‑6 decoder. See `ptycho_torch/generators/README.md` for adding new architectures.
...
- The Torch runner CLI (`grid_lines_torch_runner.py`) accepts `--architecture ffno`, `stable_hybrid`, `fno_vanilla`, `neuralop_uno`, and `hybrid_resnet`; the compare wrapper (`grid_lines_compare_wrapper.py`) routes the established non-U-NO architectures through the Torch runner with metrics keys `pinn_ffno`, `pinn_stable_hybrid`, `pinn_fno_vanilla`, and `pinn_hybrid_resnet`
...
| `--scheduler` | str | `'Default'` | Learning rate scheduler type: `'Default'` (no scheduler), `'Exponential'` (exponential decay), `'MultiStage'` (step-wise decay), `'Adaptive'` (plateau-based reduction), `'WarmupCosine'` (linear warmup + cosine annealing), `'ReduceLROnPlateau'` (uses `plateau_*` config fields; defaults factor=0.5, patience=2, min_lr=1e-4, threshold=0.0) |
