...
**Standard Keys** (see [PtychoPINN Core Physics & Data Contracts](specs/spec-ptycho-core.md)):
- `diff3d`: `(n_images, H, W)` - Amplitude data
- `objectGuess`: `(M, M)` - Complex object
- `probeGuess`: `(H, W)` - Complex probe
- `xcoords`, `ycoords`: `(n_images,)` - Scan positions

`'diffraction'` is not the standalone-NPZ key: it names the H5 `/raw_data` bundle dataset (`../specs/data_contracts.md`), a separate container from the standalone-NPZ contract above.

Torch loaders (`ptycho_torch/dataloader.py`, `ptycho_torch/memmap_bridge.py`) auto-transpose legacy `(H,W,N)` diffraction arrays to canonical `(N,H,W)` at load time; TF's `RawData.from_file` does not auto-transpose and only asserts the array is square. See [Findings](../docs/findings.md) FORMAT-001.
...
### Legacy Grouping Cache Files

Older grouping code could create `*.groups_cache.npz` files. The current sample-then-group path does not require them.

**Management:** Safe to delete; current code does not depend on these files.
...
def validate_dataset(filepath):
    ...
    # Check required keys
    required = ['diff3d', 'objectGuess', 'probeGuess', 
                'xcoords', 'ycoords']
...
See [PtychoPINN Core Physics & Data Contracts](specs/spec-ptycho-core.md) for:
...
- Use sampling parameters such as `n_subsample` and `n_groups` to bound loaded data.
- Use batch/data-generator paths for large datasets instead of loading every array eagerly.
- Prefer current data-flow references: [GridSize & n_groups Guide](GRIDSIZE_N_GROUPS_GUIDE.md) and [Troubleshooting](debugging/TROUBLESHOOTING.md).

### Legacy Grouping Cache Files Taking Too Much Space
```bash
# Find all legacy grouping cache files
find . -name "*.groups_cache.npz" -exec du -h {} \;
# Remove all legacy grouping cache files
find . -name "*.groups_cache.npz" -delete
```
...
- [ ] Legacy cache files not included
...
- [PtychoPINN Core Physics & Data Contracts](specs/spec-ptycho-core.md) - standalone-NPZ data format specifications
