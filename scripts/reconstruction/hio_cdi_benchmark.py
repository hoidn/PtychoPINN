...
def _initial_object(
    ...
    initial_exit_wave = _initial_exit_wave_from_magnitude(target, seed)
    max_amp = float(np.abs(probe_array).max(initial=0.0))
    if max_amp <= 0:
        raise ValueError("zero-amplitude probe cannot be used")
    epsilon = float(epsilon_ratio) * max_amp
...
def write_solver_manifest(
    ...
    if selected_solver not in SOLVER_CHOICES:
        raise ValueError(f"unsupported solver {selected_solver!r}; expected one of {SOLVER_CHOICES!r}")
    output_root = Path(output_root)
...
def _selected_solver(args: argparse.Namespace) -> str:
    ...
    if solver not in SOLVER_CHOICES:
        raise ValueError(f"unsupported solver {solver!r}; expected one of {SOLVER_CHOICES!r}")
    return solver
