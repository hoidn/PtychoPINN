...
class TestEntry:
    ...
    def display_name(self) -> str:
        return f"`{self.rel_path.as_posix()}`"


def _normalize_doc_text(text: str) -> str:
    """Normalize stale generated-doc routes without editing test docstrings."""
    return text.replace("plans/active/", "docs/plans/")
...
def _uses_unittest_testcase(filepath: Path) -> bool:
    """Detect whether a module defines a genuine unittest.TestCase subclass."""
    with open(filepath, "r", encoding="utf-8") as fh:
        tree = ast.parse(fh.read())
...
def _module_command(rel_path: Path, uses_unittest: bool) -> str:
    module = rel_path.with_suffix('')
    if uses_unittest:
        dotted = ".".join((Path('tests') / module).parts)
        return f"python -m unittest {dotted}"
    posix_path = (Path('tests') / rel_path).as_posix()
    return f"python -m pytest {posix_path}"
...
def _collect_entries() -> Dict[str, List[TestEntry]]:
    ...
    for root, _, files in os.walk(TESTS_ROOT):
        ...
        for name in sorted(files):
            ...
            uses_unittest = _uses_unittest_testcase(abs_path)
            command = _module_command((rel_parent / name), uses_unittest)
    ...
    for entry in entries:
        rows.append(
            "| {name} | {purpose} | {key_tests} | `{command}` | {notes} |".format(
                ...
                purpose=_normalize_doc_text(entry.purpose.replace("\n", " ").strip()),
...
def build_index() -> str:
    ...
    lines: List[str] = [
        ...
        "",
        "Legacy unittest modules (genuine `unittest.TestCase` subclasses) may still be run via "
        "`python -m unittest <dotted.module>`; see the Usage/Command column below for the exact form per module.",
        "",
        "---",
        "",
        "## Test Modules",
