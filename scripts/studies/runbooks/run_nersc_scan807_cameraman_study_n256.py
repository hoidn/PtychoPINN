#!/usr/bin/env python3
"""Runbook entrypoint for NERSC scan807 + cameraman orchestration at N=256."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run NERSC orchestration at fixed N=256 with no cross-N conversion: "
            "PtychoViT checkpoint inference on scan807/cameraman, hybrid_resnet training "
            "on cameraman half split, cross-dataset hybrid inference, and per-dataset "
            "metrics/visual aggregation."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help="Hybrid training epochs (smoke usually 5, full study 40).",
    )
    parser.add_argument(
        "--probe-mode-policy",
        type=str,
...
def main(argv: list[str] | None = None) -> None:
    ...
    write_invocation_artifacts(
        ...
        script_path="scripts/studies/runbooks/run_nersc_scan807_cameraman_study_n256.py",
