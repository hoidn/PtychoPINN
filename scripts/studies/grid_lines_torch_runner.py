...
class TorchRunnerConfig:
    ...
    # VarPro/probe-weighting ModelConfig knobs (Phase 2, B7 plumbing). Defaults mirror
    # ptycho_torch.config_params.ModelConfig so omitting these flags reproduces current
    # behavior exactly. use_shared_decoder intentionally has no runner CLI flag (Task 2.7 brief).
    cnn_output_mode: str = "amp_phase"
...
def _normalize_position_inputs(
    ...
    *,
    ...
    if offsets_b112.shape[0] != patches.shape[0]:
        # gridsize>1 inference: coords_offsets carry B groups of C channels while
        # patches are flattened to (B*C, H, W, 1) in g-major/c-minor order
        # (patch g*C+c). Build one global offset per flattened patch, index-aligned
        # to that flattening, and swap the stored (y, x) coordinate axis back to the
        # (x, y) order the shift backends expect. The old silent block-repeat gave
        # every flattened patch its group's whole C-channel offset block, which the
        # downstream modular gather collapses 4:1 into a blob lattice.
        # See docs/findings.md#TORCH-GS2-POSITION-OFFSETS-001.
        coord_channels = int(coords_np.shape[3])
        groups = int(offsets_b12c.shape[0])
        if (
            groups == 0
            or patches.shape[0] % groups != 0
            or patches.shape[0] // groups != coord_channels
        ):
            raise ValueError(
                "Cannot align coords_offsets "
                f"(groups={groups}, channels={coord_channels}) with "
                f"{patches.shape[0]} patches: expected patches == groups * channels."
            )
        # (B, 1, 2, C) -> (B, C, 1, 2) -> (B*C, 1, 1, 2): stored (y, x) per patch,
        # in the same g-major/c-minor order as the patch flattening.
        offsets_b112 = np.transpose(offsets_b12c, (0, 3, 1, 2)).reshape(-1, 1, 1, 2)
        offsets_b112 = offsets_b112[..., ::-1].copy()  # swap (y, x) -> (x, y)
        # Keep the batched backend consistent: it re-derives offsets via
        # transpose(offsets_b12c, (0, 1, 3, 2)); returning the swapped values as
        # (B*C, 1, 2, 1) makes that transpose reproduce offsets_b112 exactly.
        offsets_b12c = np.transpose(offsets_b112, (0, 1, 3, 2)).copy()
...
def _reassemble_predictions_for_metrics(
    ...
    """Reassemble raw per-patch inference predictions into a single canvas for metrics.

    Dispatches between the ``position`` reassembly path
    (``_reassemble_with_coords_offsets``) and the ``grid_lines`` grid-tiling
    stitch (``_stitch_for_metrics`` / ``stitch_predictions``).

    gridsize>1 runs left at the default ``reassembly_mode="grid_lines"`` are
    auto-routed to ``position``: the grid_lines/``stitch_predictions``
    grid-tiling stitch silently skips its gs>1 regrid guard on channels-first
    ``forward_predict`` output (``processed.shape[-1] == gridsize**2`` never
    matches a channels-first array), flat-tiling the heavily-overlapping
    per-patch predictions into a checkerboard artifact
    (docs/findings.md#TORCH-GS2-STITCH-001). gridsize=1 runs and any
    explicitly-configured ``reassembly_mode`` are unaffected.

    Returns:
        Tuple of (reassembled predictions, effective reassembly mode used,
        position-reassembly runtime contract; empty unless the position path
        was used).
    """
    effective_reassembly_mode = cfg.reassembly_mode
    if cfg.gridsize > 1 and cfg.reassembly_mode == "grid_lines":
        effective_reassembly_mode = "position"
        logger.warning(
            "gridsize=%d with reassembly_mode='grid_lines' auto-routed to "
    ...
    if effective_reassembly_mode == "position":
    ...
    return pred_for_metrics, effective_reassembly_mode, position_reassembly_runtime_contract
    ...
    if cfg.gridsize > 1:
        ...
        if cfg.architecture not in gridsize_gt1_ported_architectures:
            raise ValueError(
                ...
                f"only {sorted(gridsize_gt1_ported_architectures)} support "
...
def _resolve_split_nphotons(
    metadata: Optional[Dict[str, Any]], split_name: str, default: float = 1e9
) -> Tuple[float, str]:
    """Resolve nphotons for count-scale derivation from a split's NPZ metadata.

    Reads `_metadata.physics_parameters.nphotons`; falls back to `default` with
    a logged warning if absent. Does NOT read `config.nphotons` (frozen configs
    carry None).
    """
    from ptycho.metadata import MetadataManager

    physics_params = (metadata or {}).get("physics_parameters", {})
    if "nphotons" in physics_params:
        return float(MetadataManager.get_nphotons(metadata, default)), "metadata"
    logger.warning(
        "No nphotons in %s split metadata; defaulting count-scale nphotons to %s",
        split_name, default,
    )
...
def run_torch_training(
    ...
    results["count_scale_provenance"] = {
        ...
        "physics_scale_test": (
            float(physics_scale_test.item()) if physics_scale_test is not None else None
        ),
        "nphotons": {
            "train": {"value": train_nphotons, "source": train_nphotons_source},
...
def save_run_artifacts(
    ...
    *,
    ...
    effective_reassembly_mode: Optional[str] = None,
) -> Path:
...
def run_grid_lines_torch(
    ...
    *,
    ...
    try:
        ...
        pred_for_metrics, effective_reassembly_mode, position_reassembly_runtime_contract = (
            _reassemble_predictions_for_metrics(
                pred_for_metrics,
                ground_truth,
                test_data,
                test_metadata,
        ...
        run_dir = save_run_artifacts(
            ...
            effective_reassembly_mode=effective_reassembly_mode,
        )
...
def main(argv=None) -> None:
    ...
    parser.add_argument("--cnn-output-mode", type=str, default=None,
                        choices=["amp_phase", "real_imag"],
                        help="CNN generator output mode (ModelConfig.cnn_output_mode). "
                             "The built-in 'cnn' architecture ignores --output-mode "
                             "(that knob only drives non-cnn generators); if this flag "
                             "is omitted and --architecture cnn is selected, it defaults "
                             "to --output-mode's value so the two knobs stay in lockstep "
                             "for cross-architecture comparability. Defaults to "
                             "'amp_phase' otherwise.")
    parser.add_argument("--training-patch-weighting", type=str, default="central_mask",
                        choices=["central_mask", "probe", "uniform"],
                        help="Training-time patch weighting/reassembly strategy for object_big.")
    parser.add_argument("--physics-forward-mode", type=str, default="amplitude",
    ...
    # The built-in 'cnn' architecture reads cnn_output_mode (not
    # generator_output_mode -- see ptycho_torch/model.py::_resolve_generator_from_config)
    # to pick its output contract. Default it from --output-mode when the user
    # did not pass --cnn-output-mode explicitly, so '--architecture cnn
    # --output-mode real_imag' actually trains real_imag instead of silently
    # falling back to amp_phase.
    cnn_output_mode = args.cnn_output_mode
    if cnn_output_mode is None:
        if args.architecture == "cnn":
            cnn_output_mode = args.output_mode if args.output_mode in ("real_imag", "amp_phase") else "amp_phase"
        else:
