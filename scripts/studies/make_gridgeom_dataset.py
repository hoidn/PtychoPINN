...
Output: flat RawData-format npz mirroring lines_N128_{train,test}.npz's exact
key/dtype/shape contract (xcoords/ycoords/xcoords_start/ycoords_start/
diff3d(uint16)/probeGuess/objectGuess/scan_index/ground_truth_patches), plus
...
def main() -> int:
    ...
    provenance = {
        ...
        "nphotons_note": "M.NPHOTONS is inert legacy metadata, not the binding dose parameter; target_mean_count (below) sets the counts scale S via to_counts(), and photons/image is MEASURED per split, not assumed.",
