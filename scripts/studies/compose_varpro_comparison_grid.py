...
figures plus one merged metrics table:
  ...
  - Axis B (dynamic scaling across flux): the two flux-sweep SCALE tables
    (`cnn_ri`, `hybres`) parsed from their eval logs.
  - Axis C (hybrid_resnet neither vs both): `hybres_gs1_neither`/
    `hybres_gs1_both` merged with the Phase-1 CNN `gs1_frozen` dyad.
...
def compose_axis_c_figure(
    ...
    """Axis C: `hybres_gs1_neither`/`hybres_gs1_both` merged with the Phase-1
    CNN gs1 dyad(s). Columns: amplitude | phase (no scatter column).
...
_FALLBACK_SCALE = {
    ...
    "hybres": [
        (1, 1.0, 0.0029, 0.0025, 0.0039, 40.47, 0.0038, 1.426),
        (100, 10.0, 0.0282, 0.0262, 0.0385, 42.84, 0.0391, 1.441),
        (10000, 100.0, 0.282, 0.262, 0.385, 42.83, 0.390, 1.441),
    ],
}
...
def parse_scale_table(eval_txt_path: Path, generator: str) -> List[Dict[str, float]]:
    ...
    `generator` in {"cnn_ri", "hybres"}) if fewer than 3 rows are recovered.
    """
...
def compose_axis_b_figure(
    cnn_ri_eval_txt: Path, hybres_eval_txt: Path, out_path: Path,
) -> Tuple[Dict[str, List[Dict[str, float]]], Dict[str, List[Dict[str, float]]]]:
    """Axis B: dynamic scaling across flux for both generators.
    ...
    scale_tables = {
        ...
        "hybres": parse_scale_table(hybres_eval_txt, "hybres"),
    }
    fidelity_tables = {
        "cnn_ri": parse_fidelity_table(cnn_ri_eval_txt),
        "hybres": parse_fidelity_table(hybres_eval_txt),
    }

    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    colors = {"cnn_ri": "tab:blue", "hybres": "tab:orange"}
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    axis_c_rows_spec = [
        ("hybres_gs1_neither", args.ext_matrix_root / "hybres_gs1_neither" / "uniform_novarpro"),
        ("hybres_gs1_both", args.ext_matrix_root / "hybres_gs1_both" / "probe_varpro"),
    ...
    axis_c_rows = compose_axis_c_figure(
        axis_c_rows_spec, args.truth, args.out_dir / "axis_c_hybres_dyads.png",
    )

    scale_tables, fidelity_tables = compose_axis_b_figure(
        args.fluxsweep_root / "cnn_ri_eval.txt",
        args.fluxsweep_root / "hybres_eval.txt",
