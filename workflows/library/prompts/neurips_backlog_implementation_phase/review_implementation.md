...
- treat failing required `check_commands` as blocking unless the approved plan explicitly justified a narrower or stronger replacement and the implementation updated the authoritative check accordingly; treat supplemental verification as blocking only when explicitly marked blocking or when no equivalent evidence supports the delivered claim
...
- treat unfinished current-scope work as a `REVISE` finding for the fix loop, not as an item-level block
