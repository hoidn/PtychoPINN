...
# These sets deliberately spell out public-family ownership. Reflection below
# is only a completeness guard against adding a dataclass field without making
...
assert _MODEL_INPUT_NAMES == frozenset(
...
_FNO_INPUT_TRANSFORMS = frozenset(
    {"none", "sqrt", "log1p", "instancenorm"}
)
_GENERATOR_OUTPUT_MODES = frozenset(
    {"real_imag", "amp_phase_logits", "amp_phase"}
)
...
_TRAINING_PATCH_WEIGHTINGS = frozenset(
    {"central_mask", "uniform", "probe"}
)
...
_TRAINING_PATH_NAMES = frozenset(
    {"train_data_file", "test_data_file", "output_dir"}
)
_INFERENCE_PATH_NAMES = frozenset(
    {"model_path", "test_data_file", "output_dir"}
)
...
def _require_exact_int(
    ...
    *,
    ...
    if type(value) is not int:
        raise ValueError(
            f"{name} must be an exact built-in integer, got {value!r}"
        )
...
def _require_exact_bool(name: str, value: Any) -> None:
    if type(value) is not bool:
        raise ValueError(
            f"{name} must be an exact built-in boolean, got {value!r}"
        )
...
def _require_number(
    ...
    *,
    ...
    if type(value) not in {int, float} or not math.isfinite(value):
        raise ValueError(
            f"{name} must be a finite built-in number, got {value!r}"
        )
    ...
    if maximum is not None:
        ...
        if above:
            ...
            raise ValueError(
                f"{name} must be {operator} {maximum}, got {value!r}"
            )
...
def _require_path(name: str, value: Any, *, optional: bool = False) -> None:
    ...
    if not isinstance(value, Path):
        ...
        raise ValueError(
            f"{name} must be a pathlib.Path{suffix}, got {value!r}"
        )
...
def _validate_sampling_structure(config: TrainingConfig | InferenceConfig) -> None:
...
def validate_training_config_structure(config: TrainingConfig) -> None:
    ...
    _require_exact_int("batch_size", config.batch_size, minimum=0)
