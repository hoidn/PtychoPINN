...
def load_xpp_npz(file_path, train_size=512):
    ...
    """
    ptycho_data = RawData.from_file(file_path)

    # Create RawData object for the training subset
    ptycho_data_train = RawData(
        ptycho_data.xcoords[:train_size],
        ptycho_data.ycoords[:train_size],
        ptycho_data.xcoords_start[:train_size],
        ptycho_data.ycoords_start[:train_size],
        ptycho_data.diff3d[:train_size],
        ptycho_data.probeGuess,
        ptycho_data.scan_index[:train_size],
        objectGuess=ptycho_data.objectGuess,
        Y=(ptycho_data.Y[:train_size] if ptycho_data.Y is not None else None),
        object_index=ptycho_data.object_index[:train_size],
    )

    # Retain the historical third return value for lazy XPP attribute access.
    return ptycho_data, ptycho_data_train, np.load(file_path)
