"""Framework-neutral helpers for persisting invocation artifacts."""
...
def build_command_line(script_path: str, argv: Iterable[str]) -> str:
    """Build a copy-paste command string for a script and argument payload."""
...
def _capture_torch_provenance() -> Dict[str, Any]:
    ...
    if cuda_available:
        try:
            payload["device_name"] = torch.cuda.get_device_name(
...
def capture_runtime_provenance() -> Dict[str, Any]:
    """Capture effective Python/runtime import provenance."""
    payload: Dict[str, Any] = {
...
def get_git_dirty(repo_root: Path | None = None) -> Optional[bool]:
    """Return the worktree dirty state, or ``None`` when unavailable."""
    repo_root = Path(repo_root or Path.cwd())
    try:
        result = subprocess.run(
...
def write_invocation_artifacts(
    ...
    """Write invocation JSON and shell artifacts in ``output_dir``."""
    ...
    shell_path = output_dir / "invocation.sh"
    json_path.write_text(json.dumps(payload, indent=2))
    shell_path.write_text(command + "\n")
