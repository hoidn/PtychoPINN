...
- [ ] **Map canonical supervised configs to a supported PyTorch loss:** Update the bridge/factory (`ptycho/workflows/backend_selector.py` overrides or `ptycho_torch/config_factory.py`) so that when `TrainingConfig.model.model_type='supervised'`, the PyTorch `ModelConfig.loss_function` resolves to `'MAE'` (or another Lightning-supported option) before instantiating `PtychoPINN_Lightning`. Guard unsupervised runs so they continue to use `'Poisson'`.
- [ ] **Add regression tests:** Extend `tests/scripts/test_training_backend_selector.py` (or a new factory-specific test) to assert that supervised CLI invocations set the PyTorch loss function to `'MAE'` and that `_train_with_lightning` receives a model with `loss_name` defined. Consider a focused unit test around `create_training_payload`/`backend_selector.run_cdi_example_with_backend`.
- [ ] **Rerun validation:** After the loss-function mapping fix, rerun `pytest tests/scripts/test_training_backend_selector.py::TestTrainingCliBackendDispatch::test_pytorch_execution_config_flags tests/scripts/test_inference_backend_selector.py::TestInferenceCliBackendDispatch::test_pytorch_execution_config_flags -vv | tee "$HUB"/green/pytest_backend_selector_cli.log` to prove nothing regressed.
- [ ] **Repeat PyTorch CLI smoke:** Execute the training command with the new flags plus the inference command (retaining `--torch-inference-batch-size 2`). Capture stdout/stderr under `$HUB/cli/pytorch_cli_smoke_training/` and ensure a bundle/PNG set exists. If failure persists, update `$HUB/red/` with the new stack trace.
- [ ] **Refresh hub docs:** Update `analysis/artifact_inventory.txt`, `summary.md`, and `summary/summary.md` with the supervised-loss fix, new pytest log, and CLI evidence; remove or supersede the current blocker note once the run succeeds.
