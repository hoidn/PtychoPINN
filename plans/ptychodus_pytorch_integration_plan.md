...
- [ ] **Backfill spec defaults in PyTorch configs:** Extend `ptycho_torch/config_params.py::{DataConfig,ModelConfig,TrainingConfig}` to include spec-critical fields (`pad_object`, `probe_scale`, `gaussian_smoothing_sigma`, `n_groups`, `train_data_file`, `test_data_file`, `n_subsample`, `subsample_seed`, `output_dir`) with TensorFlow-parity defaults so we no longer rely on ad-hoc overrides when bridging.
- [ ] **Update factories + bridge:** Teach `ptycho_torch/config_factory.py::{create_training_payload,create_inference_payload}` and `ptycho_torch/config_bridge.py::{to_model_config,to_training_config,to_inference_config}` to consume the new dataclass fields (instead of injecting defaults), and ensure `populate_legacy_params()` passes the richer configs through `update_legacy_dict` with the correct KEY_MAPPINGS.
- [ ] **Parity regression coverage:** Extend `tests/torch/test_config_bridge.py::TestConfigBridgeParity` with assertions that `pad_object`, `probe_scale`, `gaussian_smoothing_sigma`, `train_data_file`, `test_data_file`, and `n_groups` values set in the PyTorch configs survive translation into TensorFlow dataclasses and populate `params.cfg` identically to the TensorFlow baseline.
- [ ] **Validation selector:** Run `pytest tests/torch/test_config_bridge.py::TestConfigBridgeParity -vv | tee "$HUB"/green/pytest_config_bridge.log` to prove the bridge parity suite stays green after the dataclass/factory changes.
- [ ] **PyTorch inference CLI refresh:** Invoke `python scripts/inference/inference.py --model_path "$HUB"/cli/pytorch_cli_smoke_training/train_outputs/wts.h5.zip --test_data tests/fixtures/pytorch_integration/minimal_dataset_v1.npz --backend pytorch --torch-accelerator cpu --torch-num-workers 0 --torch-inference-batch-size 2 --output_dir "$HUB"/cli/pytorch_cli_smoke_training/inference_outputs` (mirroring the documented command), capture stdout to `$HUB/cli/pytorch_cli_smoke_training/inference.log`, save amplitude/phase PNGs, and refresh `analysis/artifact_inventory.txt`, `summary.md`, and `summary/summary.md`. Record supervised blockers under `$HUB/red/` if they persist.
