...
- [ ] **Surface PyTorch execution knobs in scripts/training/train.py:** Extend the CLI surface to accept the training-specific flags documented in `docs/workflows/pytorch.md §12` (at minimum accelerator, deterministic toggle, num-workers, accumulate-grad-batches, learning-rate, scheduler, logger backend, checkpoint knobs). Ensure help text makes it clear these options only apply when `--backend pytorch` and that POLICY-001 / CONFIG-LOGGER-001 still hold.
- [ ] **Delegate to build_execution_config_from_args(mode='training'):** After parsing, build a validated `PyTorchExecutionConfig` (reuse `ptycho_torch.cli.shared.build_execution_config_from_args`) and pass it through the backend selector. This likely requires threading an optional `execution_config` parameter down to the PyTorch path (e.g., via `run_cdi_example_with_backend(..., torch_execution_config=...)` or by storing it on the TrainingConfig bridge). Keep the TensorFlow path untouched.
- [ ] **Strengthen backend-selector tests:** Add a `test_pytorch_execution_config_flags` case under `tests/scripts/test_training_backend_selector.py` (and keep the inference execution-config test green) that patches the helper(s) to assert the CLI flags propagate exactly once into `build_execution_config_from_args` and into the PyTorch workflow. Re-run `pytest tests/scripts/test_training_backend_selector.py::TestTrainingCliBackendDispatch::test_pytorch_execution_config_flags tests/scripts/test_inference_backend_selector.py::TestInferenceCliBackendDispatch::test_pytorch_execution_config_flags -vv | tee "$HUB"/green/pytest_backend_selector_cli.log`.
- [ ] **PyTorch CLI smoke rerun with training flags:** Repeat the minimal dataset commands, but include the new training flags (e.g., `--torch-accelerator cpu --torch-num-workers 0 --torch-accumulate-grad-batches 2 --torch-learning-rate 5e-4 --torch-logger csv`). Keep the inference command from the previous Do Now (with `--torch-inference-batch-size 2`) so both logs demonstrate the knobs. Store stdout/stderr plus regenerated PNGs under `$HUB/cli/pytorch_cli_smoke/`.
- [ ] **Hub + documentation refresh:** Update `$HUB/analysis/artifact_inventory.txt`, `$HUB/summary.md`, and `$HUB/summary/summary.md` with the new training CLI instructions, pytest selector link, execution-config log excerpts, and any new artifacts. Drop blockers under `$HUB/red/blocked_<timestamp>.md` if PyTorch still refuses the flags.
