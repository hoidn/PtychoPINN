...
- [ ] **Manual-optimization guard for gradient accumulation:** Update `ptycho_torch/workflows/components.py::_train_with_lightning` (and `docs/workflows/pytorch.md §12`) to detect `execution_config.accum_steps > 1` when `model.automatic_optimization=False` and raise a descriptive `RuntimeError` referencing EXEC-ACCUM-001 before Lightning raises `MisconfigurationException`. Extend `tests/scripts/test_training_backend_selector.py` with `test_manual_accumulation_guard` that patches the Trainer, sets `--torch-accumulate-grad-batches 2`, and asserts the guard triggers.
- [ ] **Supervised data-contract detection:** Fail fast when `TrainingConfig.model.model_type='supervised'` but the grouped dataset / dataloader lacks `label_amp` / `label_phase` keys (DATA-SUP-001). Add a check in the PyTorch data-container bridge or dataloader initialization so `_train_with_lightning` raises `RuntimeError` instructing users to supply labeled NPZ or switch to PINN mode. Document the requirement in `docs/workflows/pytorch.md` and reference the new finding.
- [ ] **CLI smoke rerun (PINN mode) + hub refresh:** Drop `--torch-accumulate-grad-batches` from the documented smoke commands, run the proven PINN scenario (`--model_type pinn`, `--torch-accelerator cpu`, `--torch-num-workers 0`, `--torch-learning-rate 5e-4`, `--torch-logger csv`) against `tike_outputs/fly001_final_downsampled/...`, and rerun inference with `--torch-inference-batch-size 2`. Rerun the backend-selector selector including the new guard test, capture logs under `$HUB/green/` and `$HUB/cli/`, update `analysis/artifact_inventory.txt`, `summary.md`, `summary/summary.md`, and log any remaining supervised blockers under `$HUB/red/blocked_<timestamp>.md`.
