...
| B2 | Implement training/inference factory functions | [P] | 🚧 2025-10-19 RED scaffold landed — `ptycho_torch/config_factory.py` skeleton (367 lines, stubs raise NotImplementedError) and `tests/torch/test_config_factory.py` (406 lines, 19 cases) committed with artefacts at `reports/2025-10-19T234600Z/phase_b2_skeleton/{summary.md, pytest_factory_red.log}`. **Follow-up:** current tests use `pytest.raises(NotImplementedError)`, so the selector reports **19 passed** instead of the intended RED failure; next loop must remove the guards, restore assertions, and capture a failing log before proceeding to GREEN (per `plan.md` §B2). |
