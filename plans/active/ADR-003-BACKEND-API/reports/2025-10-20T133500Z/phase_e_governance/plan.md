...
| E.A3 | Refresh workflow guide and knowledge base | [ ] | Align `docs/workflows/pytorch.md` §§11–13 with the spec redline: correct training table defaults (`--accelerator` `'auto'`, expanded choices), add pointer to §4.9 PyTorchExecutionConfig contract, and ensure helper/deprecation narratives match Phase D artifacts. Capture edits + before/after notes in `reports/<TS>/phase_e_governance_workflow_docs/doc_update_summary.md`. Add a new `docs/findings.md` entry documenting the execution-config contract (accelerator auto-default, params.cfg isolation, CLI helper enforcement) with evidence link to `spec_redline.md`. |
