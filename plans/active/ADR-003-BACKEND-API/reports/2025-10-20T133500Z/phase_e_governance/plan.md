...
| E.A3 | Refresh workflow guide and knowledge base | [x] | Align `docs/workflows/pytorch.md` §§11–13 with the spec redline: correct training table defaults (`--accelerator` `'auto'`, expanded choices), add pointer to §4.9 PyTorchExecutionConfig contract, and ensure helper/deprecation narratives match Phase D artifacts. Capture edits + before/after notes in `reports/<TS>/phase_e_governance_workflow_docs/doc_update_summary.md`. Add a new `docs/findings.md` entry documenting the execution-config contract (accelerator auto-default, params.cfg isolation, CLI helper enforcement) with evidence link to `spec_redline.md`. **COMPLETE:** Updated `docs/workflows/pytorch.md:317` training table (accelerator default `'cpu'`→`'auto'`, expanded description with all hardware options + dataclass/CLI default clarification), added spec §4.9 pointer paragraph (lines 350-352) documenting 17-field execution config catalog, and inserted CONFIG-002 finding to `docs/findings.md:11` with comprehensive synopsis (isolation guarantee, auto-accelerator, priority level 2, CLI helper enforcement). Artifacts: `reports/2025-10-20T151734Z/phase_e_governance_workflow_docs/{doc_update_summary.md,findings_update.md}`. |
...
| E.B4 | Runtime smoke extensions | [x] | ✅ 2025-10-24 (Attempt #71) — Smoke executed with gridsize adjusted to 2 due to missing `--neighbor-count` CLI flag. Validated `--accelerator auto` (auto→cpu fallback), CSV logger metrics (206 rows), checkpoint top-k=2 (best + last), and early-stop patience=5. Artifacts: `reports/2025-10-20T153300Z/phase_e_execution_knobs/runtime_smoke/2025-10-24T061500Z/{train_cli_runtime_smoke.log,metrics.csv,lightning_tree.txt,checkpoints_ls.txt,summary.md}`. Follow-up backlog: expose `--neighbor-count` CLI flag for higher gridsize smokes. |
