...
| --- | --- | --- | --- |
| E.A1 | Draft ADR-003 acceptance addendum | [ ] | Author `plans/active/ADR-003-BACKEND-API/reports/<TS>/phase_e_governance/adr_addendum.md` summarising Phases A–D evidence, open issues, and acceptance rationale. Pull data from `factory_design.md`, `override_matrix.md`, and `phase_d_cli_wrappers_smoke/smoke_summary.md`. Reference `docs/architecture/adr/ADR-003.md` §Decision to note the acceptance timestamp. |
| E.A2 | Update specs with execution config contract | [ ] | Redline `specs/ptychodus_api_spec.md` §§4.7–4.9 to enumerate PyTorch execution config fields (accelerator, deterministic, num_workers, learning_rate, batch sizes). Capture before/after diff in `reports/<TS>/phase_e_governance/spec_redline.md`. Align wording with CONFIG-001 + POLICY-001 warns. |
