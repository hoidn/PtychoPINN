...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| EB4.A | CLI smoke scripts | [x] | ✅ 2025-10-24 (Attempt #71) — Smoke executed with gridsize=2 adjustment (original gridsize=3 blocked by missing --neighbor-count CLI flag). Runtime: 14.75s, accelerator auto→cpu fallback confirmed. Artifacts: `runtime_smoke/2025-10-24T061500Z/{train_cli_runtime_smoke.log,metrics.csv,lightning_tree.txt,checkpoints_ls.txt}`. All execution knobs validated: accelerator auto-detection, CSV logger (206 rows), checkpoint top-k=2 (1 best + 1 last), early-stop patience=5. |
| EB4.B | Ledger + plan updates | [x] | ✅ 2025-10-24 (Attempt #71) — Authored `runtime_smoke/2025-10-24T061500Z/summary.md` (6.8 KB) with runtime analysis, accelerator resolution, checkpoint validation, early-stop configuration, contract compliance matrix. Updated `docs/fix_plan.md` with Attempt #71 completion record. Marked plan rows EB4.A/EB4.B `[x]`. Recommended follow-up: expose --neighbor-count CLI flag for gridsize=3 testing. |
