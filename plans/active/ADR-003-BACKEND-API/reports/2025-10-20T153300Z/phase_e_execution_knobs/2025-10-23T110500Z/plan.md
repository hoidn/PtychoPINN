...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| D1 | Run training CLI with logger enabled on minimal dataset; archive log output (e.g., generated TensorBoard files or warning message). Store under `smoke/<TS>/`. | [x] | **COMPLETE (Attempt #70):** CSV logger smoke test executed successfully. Runtime: 9.92s real (29.35s user, 1.56s sys). Command: `CUDA_VISIBLE_DEVICES="" /usr/bin/time -p python -m ptycho_torch.train --train_data_file tests/fixtures/pytorch_integration/minimal_dataset_v1.npz --test_data_file tests/fixtures/pytorch_integration/minimal_dataset_v1.npz --output_dir tmp/logger_smoke --n_images 64 --max_epochs 1 --gridsize 2 --batch_size 4 --accelerator cpu --deterministic --num-workers 0 --logger csv`. **Artifacts archived** at `smoke/2025-10-24T050500Z/`: (1) `train_cli_logger_csv.log` (full command output), (2) `metrics.csv` (36 rows: poisson_train_loss_step, poisson_val_loss, learning_rate, physics_weight, training_stage), (3) `logger_tree.txt` (directory structure: `lightning_logs/version_0/{hparams.yaml, metrics.csv}`), (4) `summary.md` (9.92s runtime, metrics validation, CSV format verification, zero warnings observed). **Key validation**: CSV logger captured all training/validation metrics per Lightning `self.log()` calls, default backend functioning per CONFIG-LOGGER-001. **Cleanup**: `tmp/logger_smoke/` removed post-artifact-capture. |
| D2 | Document CI guidance in `docs/ci_logger_notes.md` (integration strategy, env vars, how to disable in CI). | [x] | **COMPLETE (Attempt #70):** Created comprehensive CI integration guide at `docs/ci_logger_notes.md` (8.3 KB, 230 lines). **Content**: (1) Background section linking to CONFIG-LOGGER-001, spec §4.9, and smoke evidence; (2) CSV artifact handling guidance (default behavior, CI attachment patterns, parsing details); (3) Suppression strategies (`--logger none --quiet` for smoke tests); (4) Deprecation notice for `--disable_mlflow` with migration path; (5) Cleanup guidance (preventing `lightning_logs/` accumulation, checkpoint directory management); (6) TensorBoard/MLflow optional backend notes (requirements, CI integration patterns); (7) Summary table comparing use cases (full training, smoke test, visualization, experiment tracking); (8) Cross-references to spec, workflow guide, findings ledger, and plan artifacts. **Key decisions documented**: CSV default, none for smoke tests, deprecation warning for legacy flag, cleanup after artifact upload. |
