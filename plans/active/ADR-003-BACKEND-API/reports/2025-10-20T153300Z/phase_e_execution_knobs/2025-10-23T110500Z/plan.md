...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| C1 | Update `specs/ptychodus_api_spec.md` §4.9 and §7.1 to reflect logger behaviour (field default, allowed values, CLI exposure or deprecation warning). Generate `docs/` diff and store as `docs/spec_redline.md`. | [ ] | - Replace the `logger_backend` bullet in §4.9 with the approved policy: default `'csv'`, allowed values `{'csv','tensorboard','mlflow','none'}`, fallback to `'csv'` when dataclass field is `None`, and note dependency warnings (MLflow optional).<br>- Document DeprecationWarning for `--disable_mlflow` and pointer to `--logger none` + `--quiet` in §4.9 narrative.<br>- Add new training CLI table row in §7.1 for `--logger`/`--logger-backend` (default `'csv'`, field `PyTorchExecutionConfig.logger_backend`, description covering CSV/TensorBoard/MLflow/None semantics).<br>- Update the “programmatic-only parameters” callout (line ~424) to remove logger backend from the backlog list.<br>- Capture diff via `git diff specs/ptychodus_api_spec.md > docs/spec_redline.md`. |
| C2 | Refresh `docs/workflows/pytorch.md` §12 with new flag description or warning banner. Include guidance for enabling/disabling logging and highlight dependency requirements. | [ ] | - Extend the training execution flags table with `--logger` row mirroring spec defaults/options.<br>- Add prose explaining CSV default behaviour, how to opt into TensorBoard/MLflow, and how to disable logging (`--logger none`).<br>- Document the `--disable_mlflow` DeprecationWarning and direct users to `--logger none` + `--quiet`.<br>- Store rendered diff in `docs/summary.md` (same directory) summarising updates and linking to spec redline. |
| C3 | Update `docs/findings.md` if policy-level decision (e.g., POLICY-LOGGING-001) and mark `plans/active/ADR-003-BACKEND-API/implementation.md` Phase E rows accordingly. Append fix_plan Attempt entry with artifact links and update plan checklists to `[x]`. | [ ] | - Add new finding (e.g., `CONFIG-LOGGER-001`) capturing the CSV default + optional backends decision with evidence link to decision summary.<br>- Update `plans/active/ADR-003-BACKEND-API/implementation.md` Phase E row E2/E3 commentary to note documentation sync + backlog.<br>- Log Attempt #69 in `docs/fix_plan.md` with artifact paths (`spec_redline.md`, `summary.md`, updated findings). |
| C4 | Record MLflow logger refactor backlog item and reference future fix_plan entry once created. | [ ] | During documentation sync, append note in `summary.md` and `docs/fix_plan.md` pointing to a forthcoming fix_plan entry (e.g., `ADR-003-MLFLOW-LOGGER`) that will migrate to Lightning `MLFlowLogger`. Ensure backlog note cites `decision/approved.md`. |
