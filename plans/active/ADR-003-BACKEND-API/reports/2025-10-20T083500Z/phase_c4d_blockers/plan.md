...
| B2 | Diagnose & patch gridsize mismatch | [P] | Updated diagnosis: dataloader hands `coords_relative` tensors shaped `(batch, 1, 2, C)` into `reassemble_patches_position_real`, which expects `(batch, C, 1, 2)`. Broadcast with `norm_factor` expands the last dim (size 1 → 2), so `Translation(...).view` sees 16 elements but still tries to reshape to `[4, 2, 1]`. See `plans/active/ADR-003-BACKEND-API/reports/2025-10-20T103200Z/phase_c4d_coords_debug/summary.md`. Fix plan: permute/reshape coords before batching (e.g., `coords_rel = coords_rel.permute(0, 3, 1, 2).contiguous()`), add regression test asserting `(batch, gridsize**2, 1, 2)` layout, then rerun targeted selectors/CLI smoke. |
