...
| B4 | Inspect bundle module types | [ ] | After rerunning the integration selector, unpack `training_outputs/wts.h5.zip` and `torch.load` each `model.pth`, verifying both map to `PtychoPINN_Lightning` (not sentinel dicts). Record findings under `reports/2025-10-20T090900Z/debug/` and flag mismatches in this plan before attempting fixes. |
