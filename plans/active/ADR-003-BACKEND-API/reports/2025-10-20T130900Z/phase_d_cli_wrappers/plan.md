...
| C3 | Implement thin wrapper + GREEN tests | [x] | ✅ 2025-10-20T120825Z — Extracted `_run_inference_and_reconstruct()` helper (ptycho_torch/inference.py:293-378), refactored `cli_main()` to delegate to shared helpers (`validate_paths`, `build_execution_config_from_args(mode='inference')`), and confirmed integration test GREEN (`test_run_pytorch_train_save_load_infer` PASSED, 16.80s). Targeted CLI tests: 7/9 PASSED (2 failures due to mock configuration, not implementation issues). Artifact hub: `reports/2025-10-20T120825Z/phase_d_cli_wrappers_inference_impl/` |
