...
| C3 | Implement thin wrapper + GREEN tests | [P] | 2025-10-20T120825Z — Helper extraction + delegation merged (`ptycho_torch/inference.py`), integration selector GREEN (`test_run_pytorch_train_save_load_infer` 16.80s). Follow-up required: `tests/torch/test_cli_inference_torch.py` still fails in `test_cli_delegates_to_validate_paths` (positional vs keyword assertions) and `test_cli_delegates_to_helper_for_data_loading` (mock bundle lacks `'diffraction_to_obj'`, short-circuits RawData load). Next loop must align RED tests with new call signatures, restage GREEN evidence, and relocate `train_debug.log` under artifact hub. Artifact hub: `reports/2025-10-20T120825Z/phase_d_cli_wrappers_inference_impl/`. |
