...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| B1 | Author training CLI refactor blueprint | [ ] | Draft `training_refactor.md` describing new entry-point layout (e.g., `parse_args_new_interface()`, `run_training_from_args()`), legacy shim strategy, and MLflow disable handling. Reference spec §4.8 for CONFIG-001 ordering. Capture decisions on: (i) moving RawData loading + Lightning trainer orchestration out of CLI scope (baseline.md §Training CLI call graph), (ii) where shared helpers (e.g., `cli/shared.py:resolve_accelerator`) live, (iii) deprecation messaging for `--device` vs `--accelerator` (design_notes.md §D1), and (iv) how `disable_mlflow` maps onto execution config toggles without reviving MLflow dependencies. |
