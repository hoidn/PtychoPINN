...
| C4.D3 | Full regression suite | [P] | `CUDA_VISIBLE_DEVICES="" pytest tests/ -v` executed (log: `plans/active/ADR-003-BACKEND-API/reports/2025-10-20T050500Z/phase_c4_cli_integration/pytest_full_suite_c4.log`) and dataloader TensorDict parity is now GREEN, but integration selector still fails on `test_run_pytorch_train_save_load_infer`. Latest reproduction (`plans/active/ADR-003-BACKEND-API/reports/2025-10-20T070610Z/phase_c4_cli_integration_debug/manual_train_cli.log`) shows Lightning aborting with `ValueError: Expected value argument … within the support … of the distribution Poisson` because `batch[0]['images']` contains normalized amplitude floats. TensorFlow path squares amplitudes and applies `nphotons`/physics scaling before Poisson log-likelihood; PyTorch workflow now reaches Poisson without discretising inputs. Next: design TDD coverage around Poisson support (e.g. regression in `TestWorkflowsComponentsTraining::test_lightning_poisson_count_contract`), decide where amplitude→count conversion lives (TensorDict assembly vs `compute_loss`), implement scaling using existing `rms_scaling_constant`/`physics_scaling_constant`, then rerun targeted PyTorch integration selector. |
