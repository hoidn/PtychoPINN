...
| C4.D3 | Full regression suite | [P] | `CUDA_VISIBLE_DEVICES="" pytest tests/ -v` executed (log: `plans/active/ADR-003-BACKEND-API/reports/2025-10-20T050500Z/phase_c4_cli_integration/pytest_full_suite_c4.log`) but integration selector still failing (`test_run_pytorch_train_save_load_infer`). Supervisor debug 2025-10-20 pinned the blocker to `ModelConfig.C_forward` not being synchronized with the factory’s inferred channel count (`coords_relative` only supplies 1 channel while helpers assume 4). See `coords_relative_investigation.md` under the same report hub for replication steps. Remains open until the factory is patched and the selector is GREEN. |
