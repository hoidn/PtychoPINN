...
| C4.D3 | Full regression suite | [P] | `CUDA_VISIBLE_DEVICES="" pytest tests/ -v` executed (log: `plans/active/ADR-003-BACKEND-API/reports/2025-10-20T050500Z/phase_c4_cli_integration/pytest_full_suite_c4.log`) but integration selector still failing (`test_run_pytorch_train_save_load_infer`). Channel-count mismatch fixed (Attempt #23), but inference now aborts because `ptycho_torch.train` legacy pathway never writes `wts.h5.zip`, so updated inference CLI (factory + bundle loader) cannot locate the archive. Supervisor triage captured at `reports/2025-10-20T060955Z/phase_c4_cli_integration_debug/triage.md`; requires switching training CLI to workflow persistence (or injecting `save_torch_bundle`) before marking GREEN. |
