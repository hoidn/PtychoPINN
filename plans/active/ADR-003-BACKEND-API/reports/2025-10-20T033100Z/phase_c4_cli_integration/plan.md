...
| C4.D3 | Full regression suite | [P] | `CUDA_VISIBLE_DEVICES="" pytest tests/ -v` executed (log: `plans/active/ADR-003-BACKEND-API/reports/2025-10-20T050500Z/phase_c4_cli_integration/pytest_full_suite_c4.log`) but integration selector still fails on `test_run_pytorch_train_save_load_infer`. Latest run (`reports/2025-10-20T060955Z/phase_c4_cli_integration_debug/pytest_integration_green.log`) shows Lightning validation crashing with `IndexError: too many indices for tensor of dimension 4` because `_build_lightning_dataloaders()` yields `(Tensor, Tensor)` tuples instead of the TensorDict/tuple (`{'images', 'coords_relative', 'rms_scaling_constant', 'physics_scaling_constant'}`, probe, scale) expected by `PtychoPINN_Lightning.compute_loss()`. Probe output captured in `reports/2025-10-20T073500Z/phase_c4_cli_integration_debug/dataloader_probe.txt` + summary.md. Next: add RED regression in `tests/torch/test_workflows_components.py::TestWorkflowsComponentsTraining` asserting TensorDict batches, refactor `_build_lightning_dataloaders()` to reuse `TensorDictDataLoader`/`Collate_Lightning`, then rerun the integration selector. |
