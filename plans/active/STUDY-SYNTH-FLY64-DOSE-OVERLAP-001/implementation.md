...
- **Do Now — scalar-safe mask + dense rerun (ready_for_implementation):**
  ...
  8. Immediately inspect `$HUB/cli/phase_d_dense.log`, `$HUB/cli/phase_e_dense_gs2_dose1000.log`, and the regenerated `data/phase_d/dose_1000/dense/{train.npz,test.npz}` + `data/phase_e/dose_1000/dense/gs2/wts.h5.zip`; if any remain missing or the CLI log still prints “No jobs match the specified filters,” stop and capture a `$HUB/red/blocked_<timestamp>.md` with the failing excerpts before running metrics scripts.
  9. Evidence expectations: `{analysis}` must now contain `ssim_grid_summary.md`, `ssim_grid.log`, `verification_report.json`, `verify_dense_stdout.log`, `check_dense_highlights.log`, `metrics_summary.json`, `metrics_delta_highlights_preview.txt`, `metrics_digest.md`, and `artifact_inventory.txt` with MS-SSIM ±0.000 / MAE ±0.000000 deltas + phase-only preview verdict referenced in `$HUB/summary/summary.md`, docs/fix_plan.md, and galph_memory. Failures go under `$HUB/red/blocked_<timestamp>.md` with the exact command + exit code before pausing.
...
2. **Fix grouped inference gridsize:** In `scripts/compare_models.py`, immediately before `pinn_model.predict(...)`, force and log `params.cfg['gridsize'] = int(math.isqrt(diffraction_channels))` (with an assertion that `channels == gridsize**2`) so grouped diffraction tensors cannot trigger the `Translation.call` mismatch captured in `$HUB/analysis/dose_1000/dense/train/comparison.log:435-554`.
3. **Targeted smoke:** Run a limited comparison to prove the fix works before touching the full pipeline, e.g.  
   ...
   If the run still raises the Translation ValueError (or any other exception), capture `$HUB/red/blocked_$(date -Iseconds)_compare_models.md` with the command + stack trace before proceeding.
4. **Re-run targeted selectors (GREEN evidence under `$HUB/green/`):**
   - `pytest tests/study/test_dose_overlap_overlap.py::test_filter_dataset_by_mask_handles_scalar_metadata -vv | tee "$HUB"/green/pytest_filter_dataset_by_mask.log`
   - `pytest tests/study/test_dose_overlap_overlap.py::test_generate_overlap_views_dense_acceptance_floor -vv | tee "$HUB"/green/pytest_dense_acceptance_floor.log`
5. **Counted dense rerun:** `python plans/active/STUDY-SYNTH-FLY64-DOSE-OVERLAP-001/bin/run_phase_g_dense.py --hub "$HUB" --dose 1000 --view dense --splits train test --clobber |& tee "$HUB"/cli/run_phase_g_dense_stdout.log`.
6. **Fully parameterized post-verify:** `python plans/active/STUDY-SYNTH-FLY64-DOSE-OVERLAP-001/bin/run_phase_g_dense.py --hub "$HUB" --dose 1000 --view dense --splits train test --post-verify-only |& tee "$HUB"/cli/run_phase_g_dense_post_verify_only.log`.
7. **Refresh metrics helpers (if `analysis/metrics_summary.json` predates the rerun):**
   - `python plans/active/STUDY-SYNTH-FLY64-DOSE-OVERLAP-001/bin/report_phase_g_dense_metrics.py --hub "$HUB" --metrics "$HUB"/analysis/metrics_summary.json`
   - `python plans/active/STUDY-SYNTH-FLY64-DOSE-OVERLAP-001/bin/analyze_dense_metrics.py --hub "$HUB"`
8. **Verify regenerated artifacts:** Confirm `$HUB/cli/phase_d_dense.log`, `$HUB/cli/phase_e_dense_gs2_dose1000.log`, `data/phase_d/dose_1000/dense/{train.npz,test.npz}`, and `data/phase_e/dose_1000/dense/gs2/wts.h5.zip` have fresh timestamps. If any log still prints “No jobs match…” or a file is missing, capture `$HUB/red/blocked_<timestamp>.md` with the relevant excerpts before continuing.
9. **Comparison log check:** After the rerun, inspect `$HUB/analysis/dose_1000/dense/train/comparison.log` and `$HUB/cli/phase_g_dense_train.log`. If the Translation error (or any comparison failure) returns, stop and file `$HUB/red/blocked_<timestamp>_comparison.md` with the command + first stack trace; only proceed once the logs end with the success summary.
10. **Publish evidence:** `{analysis}` must now include `ssim_grid_summary.md`, `ssim_grid.log`, `verification_report.json`, `verify_dense_stdout.log`, `check_dense_highlights.log`, `preview.txt`, `metrics_summary.json`, `metrics_delta_highlights_preview.txt`, `metrics_digest.md`, and `analysis/artifact_inventory.txt`. Update `analysis/verification_report.json` so 10/10 checks pass, then refresh `plans/active/STUDY-SYNTH-FLY64-DOSE-OVERLAP-001/summary.md`, `$HUB/summary/summary.md`, docs/fix_plan.md, and galph_memory with MS-SSIM ±0.000 / MAE ±0.000000 deltas plus artifact links. Any failure must be logged under `$HUB/red/blocked_<timestamp>.md` with the exact command + exit code before pausing.
