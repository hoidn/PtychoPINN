...
- [ ] Normalize success-banner path prints to hub-relative strings (`run_phase_g_dense.py::main`, both full run and `--post-verify-only`) and extend `test_phase_g_dense_orchestrator.py::test_run_phase_g_dense_post_verify_only_executes_chain` to assert the relative `cli/` + `analysis/` lines (TYPE-PATH-001).
