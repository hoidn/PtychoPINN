...
- [ ] Extend `tests/study/test_phase_g_dense_orchestrator.py::test_run_phase_g_dense_exec_runs_analyze_digest` so it asserts **exactly one** `Metrics digest:` line appears in stdout (guarding against future banner duplication) and continue to check for the `Metrics digest log:` reference.
