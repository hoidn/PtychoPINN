...
- [ ] Implement offset-centering + debug plumbing inside `scripts/compare_models.py`: add `--baseline-debug-limit`/`--baseline-debug-dir`, log DIAGNOSTIC baseline_input/output/off­set stats per split, center flattened `baseline_offsets` before inference (architecture contract requires zero-centered offsets), and emit NPZ/JSON dumps of the first debug batch under `$HUB/analysis/dose_1000/dense/{split}/debug/`. Record new logs as `$HUB/cli/compare_models_dense_{split}_debug.log`.
...
- **Do Now — Counted Phase G rerun + verification bundle (ready_for_implementation):**
  ...
  2. Keep the translation regression guard GREEN before touching the hub:
     ...
     pytest tests/study/test_dose_overlap_comparison.py::{test_pinn_reconstruction_reassembles_batched_predictions,test_pinn_reconstruction_reassembles_full_train_split} -vv \
       | tee "$HUB"/green/pytest_compare_models_translation_fix_v11.log
     ```
