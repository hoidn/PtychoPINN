...
- [ ] Add a follow-on assertion in `tests/study/test_phase_g_dense_orchestrator.py::test_run_phase_g_dense_exec_runs_analyze_digest` to ensure `stdout.count("Metrics digest log: ") == 1` so CLI log references cannot duplicate when future banner edits land (TYPE-PATH-001, TEST-CLI-001).
