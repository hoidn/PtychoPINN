...
| D2 | Implement `overlap.py::generate_overlap_views` that loads Phase C train/test NPZs, forms gridsize=2 groups (KNN-based) using neighbor_count=7, filters to dense/sparse selections via D1 mask, runs validator with `view` hints, and writes `{dense,sparse}_{train,test}.npz` + spacing stats JSON. | [P] | Core pipeline implemented (commit `d9521b95`) with validator hook + metadata. Outstanding: emit a consolidated metrics bundle (`metrics_bundle_path`) aggregating train/test stats, and ensure the CLI copies it to `metrics/<dose>/<view>.json` under the reports hub (per Phase D charter). Update tests to guard the new return contract. |
