...
| D2 | Implement `overlap.py::generate_overlap_views` that loads Phase C train/test NPZs, forms gridsize=2 groups (KNN-based) using neighbor_count=7, filters to dense/sparse selections via D1 mask, runs validator with `view` hints, and writes `{dense,sparse}_{train,test}.npz` + spacing stats JSON. | [x] | Completed in Attempt #10 (`reports/2025-11-04T045500Z/phase_d_cli_validation/`). Function now emits `metrics_bundle.json` and per-split metrics paths; pytest guard `test_generate_overlap_views_metrics_manifest` asserts bundle contents (train/test). CLI copies bundle into reports hub; see summary.md + pytest logs for evidence. |
...
| D4 | CLI + docs: add `python -m studies.fly64_dose_overlap.overlap --phase-c-root ... --output-root ...` to iterate doses, produce dense/sparse NPZs, write manifest + spacing metrics. Update `implementation.md` Phase D, `test_strategy.md` sections, `summary.md`, and `docs/fix_plan.md` Attempt log with artifact paths & findings alignment. | [x] | COMPLETE in Attempt #11 (`reports/2025-11-04T051200Z/phase_d_doc_sync/`): Updated `implementation.md` Phase D section (lines 105-131) to COMPLETE status with metrics bundle workflow description, delivered components, and artifact hub references; updated `test_strategy.md` Phase D section (lines 64-82) with executed selectors, coverage proof, and execution evidence paths; marked plan.md D4 `[x]`; appended Attempt #11 to `docs/fix_plan.md` with doc-sync artifact links. CLI execution + metrics bundle workflow evidence at `reports/2025-11-04T045500Z/phase_d_cli_validation/`. Findings: CONFIG-001, DATA-001, OVERSAMPLING-001 preserved throughout documentation. |
