# Dense Phase G Evidence Run + Post-Verify Sweep (2025-11-11T180800Z)

## Reality Check
- Retrospective (per cadence): `git log -10 --oneline` still tops out at supervisor sync commits (`cb66fa1f`, `ddbb79fd`) plus Ralph’s last functional change `5cd130d3` (allow_pickle fix), so no dense rerun evidence has landed since we handed off the ready_for_implementation Do Now. `git pull --rebase` now succeeds only after `git stash push --include-untracked` to preserve the deleted Phase C manifest + local `.bak` notes; remember to `git stash pop` once synced so the hub stays dirty for inspection.
- Hub state is unchanged: `analysis/` contains only `blocker.log`, while `cli/` still has `phase_c_generation.log`, `phase_d_dense.log`, and the trio of `run_phase_g_dense_stdout*.log`. There are **no** SSIM grid summaries, verification logs, metrics delta files, preview/highlights artifacts, or artifact inventory outputs.
- `cli/phase_d_dense.log` shows the last counted run executed **inside this repo** (`/home/ollie/Documents/PtychoPINN`) and still failed with `ValueError: Object arrays cannot be loaded when allow_pickle=False`, meaning the subsequent code fix never received a rerun. Guard the working directory with `test "$(pwd -P)" = "/home/ollie/Documents/PtychoPINN"` before launching commands so we do not collect mixed evidence.
- `data/phase_c/run_manifest.json` remains deleted (tracked by git); regenerate it via the counted run rather than restoring it manually so evidence lines up with the new outputs.
...
- 2025-11-11T180800Z audit: The hub still mirrors the stale failure—`analysis/` only contains `blocker.log`, `{cli}` has `{phase_c_generation,phase_d_dense,run_phase_g_dense_stdout(_retry,_v2).log}`, and `cli/phase_d_dense.log` ends with the pre-fix `ValueError: Object arrays cannot be loaded when allow_pickle=False`. Since commit `5cd130d3` already landed, rerunning `run_phase_g_dense.py --clobber ...` followed by `--post-verify-only` from `/home/ollie/Documents/PtychoPINN` should regenerate Phase C data plus SSIM grid/verification/highlights evidence in this hub.
