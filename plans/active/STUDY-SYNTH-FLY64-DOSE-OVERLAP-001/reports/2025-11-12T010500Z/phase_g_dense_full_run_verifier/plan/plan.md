# Dense Phase G Evidence Run + Post-Verify Sweep (2025-11-11T130158Z)

## Reality Check
- As of 2025-11-11T13:01Z no new commits or artifacts have landed since the prior loop; `{analysis,cli}` still mirror the workspace-mismatch attempt from `/home/ollie/Documents/PtychoPINN2` and contain only `blocker.log`, `phase_c_generation.log`, `phase_d_dense.log`, and `run_phase_g_dense_stdout.log`.
- `timeout 30 git pull --rebase` again reported “You have unstaged changes” because the hub continues to track the deleted `data/phase_c/run_manifest.json` plus updated CLI/pytest logs. I temporarily stashed those evidence files, pulled (no upstream changes), then restored the stash so the workspace stays dirty but synced—Ralph still needs to regenerate the manifest via the counted run rather than deleting/reverting the file.
