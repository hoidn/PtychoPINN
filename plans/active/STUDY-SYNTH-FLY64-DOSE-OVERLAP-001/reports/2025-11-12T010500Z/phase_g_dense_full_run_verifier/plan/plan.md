# Dense Phase G Evidence Run + Post-Verify Sweep (2025-11-11T131617Z)

## Reality Check
- `timeout 30 git pull --rebase` still refuses to run while the deleted `data/phase_c/run_manifest.json` and CLI/pytest logs remain unstaged, so I once again `git stash push --include-untracked`, pulled (no upstream commits), then `git stash pop` to keep the evidence dirty but synced—Ralph must regenerate the manifest through the counted run instead of restoring the file manually.
- No new artifacts appeared after the sync: `analysis/` still only contains `blocker.log` and `cli/` still holds just `phase_c_generation.log`, `phase_d_dense.log`, and `run_phase_g_dense_stdout.log`, confirming there are **zero** SSIM grid summaries, verification logs, preview artifacts, metrics deltas, highlights outputs, or artifact inventory files from a counted rerun.
- `analysis/blocker.log` continues to document the prior workspace mismatch (`/home/ollie/Documents/PtychoPINN2`) and the resulting `ValueError: Object arrays cannot be loaded when allow_pickle=False`, so none of the dense overlap or verification stages have executed from the canonical repo.
- `prepare_hub` already archived the stale Phase C data under `archive/phase_c_20251111T122258Z/`, leaving only the patched NPZs plus the deleted manifest in `data/phase_c/`; the next run must rebuild Phase C outputs (manifest + NPZ validation) in-place.
- Until we execute `run_phase_g_dense.py --clobber ...` followed immediately by `--post-verify-only` from **/home/ollie/Documents/PtychoPINN**, `{analysis,cli}` cannot prove SSIM grid/verifier/highlights success, `analysis/artifact_inventory.txt` regeneration, or MS-SSIM/MAE + preview evidence. Start every command with `test "$(pwd -P)" = "/home/ollie/Documents/PtychoPINN"` to prevent another workspace mismatch.
