# Dense Phase G Evidence Run + Post-Verify Sweep (2025-11-12T201200Z)

## Reality Check
- Commit `6a51d47a` extended `test_run_phase_g_dense_exec_runs_analyze_digest` with SSIM grid + verification banner guards, so the full Phase C→G execution path now enforces all success-banner lines.
- `test_run_phase_g_dense_post_verify_only_executes_chain` still focuses on command ordering and artifact inventory only; it does **not** guard the SSIM grid summary/log or verification report/log/highlights lines the `--post-verify-only` path prints. A regression could silently drop those references even though we rely on the short rerun for day-2 verification.
...
1. **Post-verify-only banner guard** — Extend `tests/study/test_phase_g_dense_orchestrator.py::test_run_phase_g_dense_post_verify_only_executes_chain` so it asserts the success banner references the SSIM grid summary/log plus `analysis/verification_report.json`, `analysis/verify_dense_stdout.log`, and `analysis/check_dense_highlights.log`. Update the test stubs to create those CLI logs so `.exists()` checks pass (TYPE-PATH-001, TEST-CLI-001).
...
2. Extend `tests/study/test_phase_g_dense_orchestrator.py::test_run_phase_g_dense_post_verify_only_executes_chain` with assertions that `stdout` contains the SSIM grid summary/log lines and the verification evidence lines (`Verification report/log`, `Highlights check log`). Update its `stub_run_command` helper to create each `log_path` so existence checks succeed even when only the SSIM/verify/check commands run.
3. `pytest --collect-only tests/study/test_phase_g_dense_orchestrator.py -k post_verify_only_executes_chain -vv | tee "$HUB"/collect/pytest_collect_post_verify_only.log` (move failures to `$HUB`/red/ before rerun).
4. `pytest tests/study/test_phase_g_dense_orchestrator.py::test_run_phase_g_dense_post_verify_only_executes_chain -vv | tee "$HUB"/green/pytest_post_verify_only.log`.
