...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| F3.1 | Surface `selection_strategy` + acceptance metrics in CLI manifest/summary for sparse jobs (update `reconstruction.py::main`, extend Phase F pytest to assert metadata) | [x] | Completed during Attempt #88 validation — implementation already present (commit 90103497). Evidence archived at `reports/2025-11-04T133218Z/phase_f_ptychi_baseline_f3_metadata_recovery/green/pytest_phase_f_sparse_green.log`. |
| F3.2 | Run sparse/train LSQML job (num_epochs=100, deterministic modes) capturing CLI transcript, reconstruction log, manifest + skip summary | [x] | Sparse/train run executed in Attempt #88; artifacts under `reports/2025-11-04T133218Z/phase_f_ptychi_baseline_f3_metadata_recovery/{cli/sparse_train.log,real_run/reconstruction_manifest_sparse_train.json,real_run/skip_summary_sparse_train.json}`. |
| F3.3 | Run sparse/test LSQML job mirroring train settings; archive outputs and update summary with acceptance + runtime notes | [x] | Sparse/test run captured in Attempt #88; evidence stored alongside train split (`.../cli/sparse_test.log`, manifest/skip summaries). Summary records acceptance_rate=0.125, selection_strategy="greedy". |
| F3.4 | Update initiative summary + registries after sparse evidence (docs/TESTING_GUIDE.md §Phase F, TEST_SUITE_INDEX) | [x] | Documentation and ledger updates performed in Attempt #88 — summary.md updated with sparse telemetry; selectors already registered and GREEN. |
