...
| F2.4 | Extend LSQML evidence to dense/test split and sync CLI selector documentation | [ ] | Run CLI for dense/test with deterministic seeds, capture logs under `reports/2025-11-04T230000Z/phase_f_ptychi_baseline_f2_dense_test_run/{cli,real_run}/`, rerun targeted selectors, and update `docs/TESTING_GUIDE.md` + `docs/development/TEST_SUITE_INDEX.md` once GREEN. Note FIXME: `tests/scripts/test_ptychi_reconstruct_tike.py` still uses absolute repo path—convert to relative import during this loop. |
