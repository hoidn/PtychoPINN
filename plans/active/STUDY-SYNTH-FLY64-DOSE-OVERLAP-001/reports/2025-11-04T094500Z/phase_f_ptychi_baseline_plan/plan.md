...
### F3 — Sparse LSQML Execution
Goal: Execute LSQML reconstructions for sparse overlap views (train/test) and capture greedy selection telemetry.
Prereqs: Phase D greedy downsampling fix (Attempt #86) emitted sparse NPZs; F2 dense runs + doc sync complete.
Exit Criteria: Sparse train/test LSQML runs complete with manifest/summary including selection_strategy metadata; pytest selectors GREEN; docs/test registries updated with new evidence.

| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| F3.1 | Surface `selection_strategy` + acceptance metrics in CLI manifest/summary for sparse jobs (update `reconstruction.py::main`, extend Phase F pytest to assert metadata) | [ ] | RED→GREEN cycle captured under `reports/2025-11-05T050500Z/phase_f_ptychi_baseline_f3_sparse_runs/{red,green}/`. Ensure `test_cli_executes_selected_jobs` (or new selector) fails before implementation, then expect manifest JSON to report selection strategy + acceptance stats for sparse jobs so CLI evidence confirms greedy rescue usage. |
| F3.2 | Run sparse/train LSQML job (num_epochs=100, deterministic modes) capturing CLI transcript, reconstruction log, manifest + skip summary | [ ] | Command template in How-To map of `input.md`; store outputs under `reports/2025-11-05T050500Z/phase_f_ptychi_baseline_f3_sparse_runs/real_run/dose_{dose}/sparse/train/`. Include reconstruction stdout/stderr, manifest snapshot, and `selection_strategy` confirmation in summary.md. |
| F3.3 | Run sparse/test LSQML job mirroring train settings; archive outputs and update summary with acceptance + runtime notes | [ ] | Use same deterministic flags, deposit artifacts under `/real_run/dose_{dose}/sparse/test/`. Capture CLI transcript (`cli/sparse_test.log`), reconstruction log, and updated manifest JSON verifying no skipped datasets. |
| F3.4 | Update initiative summary + registries after sparse evidence (docs/TESTING_GUIDE.md §Phase F, TEST_SUITE_INDEX) | [ ] | After GREEN tests + real runs, add sparse run commands/selectors to docs, append Attempt #87+ ledger entry, and summarize sparse vs dense observations in `reports/2025-11-05T050500Z/phase_f_ptychi_baseline_f3_sparse_runs/docs/summary.md`. |
