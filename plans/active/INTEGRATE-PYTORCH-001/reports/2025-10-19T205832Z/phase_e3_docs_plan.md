...
| B2 | Refresh agent guidance & onboarding docs | [ ] | Follow `reports/2025-10-19T213900Z/phase_e3_docs_b2/guidance.md`: (1) Add PyTorch backend selection + CONFIG-001 reminder to `CLAUDE.md` citing spec §4.8 and workflow §12; (2) Insert `### Dual-Backend Architecture` subsection into `README.md` highlighting TensorFlow default, PyTorch availability, workflow doc pointer, and runtime evidence (`plans/active/TEST-PYTORCH-001/reports/2025-10-19T193425Z/phase_d_hardening/runtime_profile.md`). Record file:line anchors in `diff_notes.md` + summarize in `summary.md`. |
| B3 | Verify documentation cross-links | [ ] | After edits, run `rg "NotImplementedError" docs/workflows/pytorch.md docs/architecture.md \| tee reports/2025-10-19T213900Z/phase_e3_docs_b2/rg_notimplemented.log` and note the (expected empty) output in `summary.md`. Confirm new sections reference spec §4.8 / workflow §12 correctly. |
