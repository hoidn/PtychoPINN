...
| D3.C | Implement loader & validation | [ ] | Next up: author torch-optional failing tests for `load_torch_bundle` covering params restoration + model reconstruction, then implement loader + round-trip validation. Required artifacts: `reports/<timestamp>/phase_d3_loader.{md,pytest_red.log,pytest_green.log}` capturing (1) params.cfg before/after snapshots, (2) manifest + archive tree, (3) reconstructed model check. Tests must exercise missing params failure and successful load via `tests/torch/test_model_manager.py` selectors. |
