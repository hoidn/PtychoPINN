...
| B2 | Author minimal failing test (TDD) | [ ] | **Critical:** Write test demonstrating config bridge failure BEFORE implementation. Use MVP fields called out in `scope_notes.md` (N, gridsize, model_type, nphotons, train_data_file, test_data_file, model_path, n_groups, neighbor_count). Test flow: (1) Instantiate the existing PyTorch singleton configs (`ptycho_torch.config_params`) with representative MVP values; (2) expect a new adapter module (`ptycho_torch.config_bridge`) to expose `to_model_config()`, `to_training_config()`, and `to_inference_config()` that accept the PyTorch configs plus explicit overrides for missing fields; (3) feed the returned spec dataclasses into `update_legacy_dict(ptycho.params.cfg, …)`; (4) assert `params.cfg` contains MVP keys (`'gridsize'`, `'model_type'`, `'train_data_file_path'`, `'n_groups'`, `'neighbor_count'`, etc.). Expected failure today: adapter missing and/or fields not translated. Follow TDD methodology per `docs/DEVELOPER_GUIDE.md §4`; store test + log outputs under `reports/2025-10-17T033500Z/{failing_test.md,pytest.log}`. Selector: `pytest tests/torch/test_config_bridge.py -k mvp -v`. |
