...
| D1b | Inspect Lightning checkpoint hyperparameters | [ ] | Extract `last.ckpt` from the integration run (rerun training if necessary), load it with `torch.load`, and record the keys + types under `reports/<TS>/phase_d2_completion/checkpoint_inspection.md`. Confirm whether `hyper_parameters` contains the four config objects (`model_config`, `data_config`, `training_config`, `inference_config`) and note serialization format issues. This evidence unblocks D2 remediation choices. |
