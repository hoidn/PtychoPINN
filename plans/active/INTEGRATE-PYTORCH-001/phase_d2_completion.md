...
| C3 | Implement `_reassemble_cdi_image_torch` | [ ] | Follow `reports/2025-10-19T084016Z/phase_d2_completion/phase_c3_playbook.md`: (1) unblock `_ensure_container` by teaching `RawDataTorch.generate_grouped_data` to accept/forward `dataset_path`, (2) add `_build_inference_dataloader` for deterministic Lightning prediction, (3) implement `_reassemble_cdi_image_torch` to reuse the trained Lightning module, apply flip/transpose options, stitch via `ptycho_torch.reassembly`, and return `(recon_amp, recon_phase, results)` with `obj_tensor_full` + offsets. Document behaviour updates in summary + docstrings. |
| C4 | Validate stitching tests | [ ] | Run `pytest tests/torch/test_workflows_components.py::TestReassembleCdiImageTorchRed -vv` (expect GREEN) and any new integration selector requiring `do_stitching=True` (e.g., `TestRunCdiExampleTorch::test_stitching_path`). Capture log at `reports/2025-10-19T084016Z/phase_d2_completion/pytest_stitch_green.log` and update plan summary. |
