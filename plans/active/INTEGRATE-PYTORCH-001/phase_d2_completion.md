...
| A2 | Reproduce latest integration failure | [P] | Run `pytest tests/torch/test_integration_workflow_torch.py::TestPytorchWorkflow::test_pytorch_train_save_load_infer_cycle -vv` (requires `pip install -e .[torch]`). Capture log at `reports/2025-10-17T233109Z/phase_d2_completion/pytest_integration_baseline.log` via `tee`. Baseline.md §2 records the stack trace, but the log file is missing from the repository—rerun and store it before advancing to Phase B. |
...
| ID | Task Description | State | How/Why & Guidance |
| --- | --- | --- | --- |
| B1 | Author failing Lightning regression tests | [ ] | Extend `tests/torch/test_workflows_components.py` with pytest cases that assert `_train_with_lightning` instantiates `PtychoPINN_Lightning` with correct params, wires DataModule hooks, and respects `config.output_dir`. Store red log under `reports/<TS>/phase_d2_completion/pytest_train_red.log`. |
