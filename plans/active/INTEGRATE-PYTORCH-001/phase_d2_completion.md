...
| D1d | Eliminate float64 tensors in PyTorch inference path | [ ] | **NEW (Attempt #35, 2025-10-19):** Integration test now fails inside `PtychoPINN_Lightning.forward` with `RuntimeError: Input type (double) and bias type (float)` (see `reports/2025-10-19T134500Z/phase_d2_completion/pytest_integration_checkpoint_green.log`). Triage note captured at `reports/2025-10-19T110500Z/phase_d2_completion/dtype_triage.md` outlining hypotheses (loader collation vs. module preprocessing) and proposed evidence. Tasks: (1) Author RED tests asserting `_build_inference_dataloader` and stitching pipeline preserve float32 tensors; (2) Implement dtype enforcement (likely casting loader batches to `torch.float32` before Lightning invocation or fixing upstream conversion); (3) Re-run integration selector and attach new green log under the 2025-10-19T110500Z timestamp; (4) Relocate ancillary logs (`train_debug.log`) into that report directory (done). Keep `[ ]` until full red→green cycle and integration passes. |
