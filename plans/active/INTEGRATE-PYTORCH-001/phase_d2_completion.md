...
| C4 | Modernize stitching tests + capture green log | [ ] | Close the loop opened by Attempt #26: incorporate the channel-order fix and finish test modernization. Follow `reports/2025-10-19T092448Z/phase_d2_completion/debug_shape_triage.md` → (1) update `_reassemble_cdi_image_torch` so predictions are channel-last before delegating to TensorFlow (handle both `(n,C,H,W)` and `(n,H,W,C)` inputs), (2) refresh `TestReassembleCdiImageTorch*` to use a complex mock Lightning module with explicit `gridsize**2` channel dimension while keeping the `train_results=None` guard, and (3) assert finite stitched outputs plus channel-last `results['obj_tensor_full']`. Re-run `pytest tests/torch/test_workflows_components.py -k ReassembleCdiImageTorch -vv | tee reports/2025-10-19T092448Z/phase_d2_completion/pytest_stitch_green.log` and update summary.md + docs/fix_plan.md with the green evidence. |
