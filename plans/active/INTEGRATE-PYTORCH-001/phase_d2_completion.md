...
| D1c | Restore Lightning hyperparameter serialization | [ ] | TDD loop capturing failing + passing evidence under `reports/2025-10-19T134500Z/phase_d2_completion/`. Steps: (1) Author `tests/torch/test_lightning_checkpoint.py::TestLightningCheckpointSerialization` with two red tests — one asserting fresh checkpoints expose a non-`None` `hyper_parameters` payload, the other verifying `PtychoPINN_Lightning.load_from_checkpoint(path)` succeeds without explicit config kwargs. Capture red log via `pytest tests/torch/test_lightning_checkpoint.py -k checkpoint -vv \| tee .../pytest_checkpoint_red.log`. (2) Implement fix in `ptycho_torch/model.py` by invoking `self.save_hyperparameters()` immediately after `super().__init__()` with sanitized dataclass copies (convert `Path`/`Tensor` fields to serialisable forms as needed) and ensuring inference uses restored configs. (3) Re-run targeted selector + integration workflow test, storing green logs (`pytest_checkpoint_green.log`, `pytest_integration_checkpoint_green.log`) and update `summary.md` with remediation notes + follow-on checks (e.g., regression against datasets lacking probe mask). |
