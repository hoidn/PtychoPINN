...
| B4 | Turn Lightning regression suite green | [ ] | Fix the `TestTrainWithLightningRed.test_train_with_lightning_instantiates_module` fixture so the monkeypatched stub inherits from `lightning.pytorch.core.LightningModule` (minimal `training_step` + `configure_optimizers`), then rerun `pytest tests/torch/test_workflows_components.py::TestTrainWithLightningRed -vv \| tee plans/active/INTEGRATE-PYTORCH-001/reports/2025-10-18T171500Z/phase_d2_completion/pytest_train_green.log`. If green, update this plan and docs/fix_plan Attempts with the new artifact; if still failing, capture traceback in the same log and log blocker rationale. |
