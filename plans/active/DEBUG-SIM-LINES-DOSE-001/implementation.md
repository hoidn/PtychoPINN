...
- [x] D2: **Normalization pipeline parity.** *(PARTIALLY COMPLETE 2026-01-20T123330Z — normalization-invariant instrumentation landed)*
      ...
      - [x] D2b — *dose_experiments parity capture*: add a dedicated CLI (plan-local) that replays the `dose_experiments_param_scan.md` configuration through the same simulation→group→normalize stack (via compatibility runner or locally replicated parameters) and writes `dose_normalization_stats.{json,md}` mirroring the sim_lines format so ratios can be compared without rerunning the legacy training loop. *(COMPLETE 2026-01-20T124212Z — created `bin/capture_dose_normalization.py` CLI that loads dose_experiments defaults (gridsize=2, probe_scale=4, neighbor_count=5), simulates nongrid data, records stage telemetry using `record_intensity_stage`, computes both dataset-derived and closed-form intensity scales per spec, and emits JSON/Markdown outputs. Key finding: dataset intensity_scale=262.78 vs fallback=988.21; grouped→normalized ratio=0.26 confirms ~74% amplitude drop at normalize_data. Evidence: `reports/2026-01-20T124212Z/dose_normalization/{dose_normalization_stats.json,dose_normalization_stats.md,capture_config.json,capture_summary.md}`)*
...
- [ ] D3: **Hyperparameter delta audit.**
      ...
      - [x] **D3b — 60-epoch gs2_ideal retrain (H-NEPOCHS validation).** *(COMPLETE 2026-01-20T141215Z — H-NEPOCHS REJECTED)*  
        Reran `gs2_ideal` with `--nepochs 60` (stable profile defaults unchanged) under `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-20T140531Z/gs2_ideal_nepochs60/`. EarlyStopping halted training at epoch 30 (loss plateau by epoch ~20); metrics barely moved (MAE 2.3646→2.3774, pearson_r 0.1353→0.1392, amplitude bias mean -2.30 both runs) and analyzer still reports ~6.7× prediction↔truth gap (`full_chain_product=18.571`). Training length alone does **not** resolve the amplitude collapse.
        - Scenario command template (executed with `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md` in env):  
          `python plans/active/DEBUG-SIM-LINES-DOSE-001/bin/run_phase_c2_scenario.py --scenario gs2_ideal --output-dir <hub>/gs2_ideal_nepochs60 --nepochs 60 --group-limit 64 --prediction-scale-source least_squares`
        ...
        - Guard: `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v` (logged under the same hub).
      - [x] **D3c — Document retrain outcome + decide escalation.** *(COMPLETE 2026-01-20T141215Z)*  
        Logged the verdict in this plan, `docs/fix_plan.md`, `plans/active/DEBUG-SIM-LINES-DOSE-001/summary.md`, and `docs/findings.md` (H-NEPOCHS ruled out). Next focus moves to Phase D4 architecture/loss wiring to explain the persistent ~6.7× amplitude gap.
      Test: `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
- [ ] D4: **Architecture / loss-wiring verification.**
      ...
      - [x] **D4a — Dataset intensity-scale telemetry:** *(COMPLETE 2026-01-20T232800Z)* Extended the plan-local runner/analyzer so each scenario records the dataset-derived intensity scale (`s = sqrt(nphotons / E_batch[Σ_xy |Ψ|²])` per `specs/spec-ptycho-core.md §Normalization Invariants`), the closed-form fallback, and their deltas inside `intensity_stats.json`, `run_metadata.json`, and the Markdown summary.
      ...
      - [x] **D4c — Dataset-derived intensity scale fix (COMPLETE 2026-01-21T004000Z).** Implemented the spec equation inside `ptycho/train_pinn.py::calculate_intensity_scale()` so the preferred dataset-derived path computes `s = sqrt(nphotons / E_batch[Σ_xy |X|²])` and only falls back to `sqrt(nphotons)/(N/2)` when `batch_mean ≤ 1e-12`. Added `tests/test_train_pinn.py::TestIntensityScale` (dataset-derived, fallback, rank-3, multi-channel cases) plus CLI smoke coverage. Verification run `plans/active/DEBUG-SIM-LINES-DOSE-001/bin/run_phase_c2_scenario.py --scenario gs2_ideal` shows dataset_scale=577.74 vs fallback=988.21 (ratio=0.585) — the ≈0.58 ratio is **expected** because gs2_mean intensity (≈2996) exceeds the theoretical `(N/2)²=1024`. The amplitude bias (~6.6×) persists, so remaining work must inspect downstream loss/architecture paths.
            - Evidence: `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-21T002114Z/{bias_summary.*,gs2_ideal/**,logs/}`
            - Tests: `pytest tests/test_train_pinn.py::TestIntensityScale -v`, `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
      - [x] **D4d — Lazy-container safe dataset scale reducer (COMPLETE 2026-01-21T00:55Z).** D4c now calls `ptycho_data_container.X`, which materializes the full tensor on GPU and defeats the lazy-loading design (see docs/findings.md PINN-CHUNKED-001). Extended `calculate_intensity_scale()` so it prefers the container’s NumPy backing (`_X_np` or an iterator) and performs the dataset-derived reduction entirely on CPU without touching `_tensor_cache`. The TensorFlow path remains as a fallback for legacy containers that lack NumPy buffers.
            - **Implementation:** Added a NumPy reducer (`_X_np` → float64) to compute `sum_xy |X|²`, reused the existing fallback guard for near-zero means, and left `.X` untouched when NumPy storage exists. Only falls back to the TensorFlow reduction when `_X_np` is unavailable.
            - **Regression test:** Extended `tests/test_train_pinn.py::TestIntensityScale` with `test_lazy_container_does_not_materialize` using a stubbed container. The test asserts that the dataset-derived result matches the spec equation **and** that `_tensor_cache` remains empty after calling `calculate_intensity_scale()`.
            - **Verification:** Logged evidence under `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-21T004455Z/` (pytest logs + summary). A gs2_ideal rerun is optional because the telemetry is unchanged; analyzer evidence from D4c already proves dataset/fallback parity.
            - Tests: `pytest tests/test_train_pinn.py::TestIntensityScale -v`, `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
      - [ ] **D4e — Normalize loader inputs with dataset-derived scale (REJECTED 2026-01-21T010700Z).** Investigation proved `normalize_data()` intentionally enforces a fixed `(N/2)²` L2 target that is independent of photon count. Switching it to the dataset-derived formula double-applied physics scaling and collapsed training (loss hit NaN within five steps). Reverted the code, kept the clarified docstrings/tests, and documented that the amplitude bias must be solved elsewhere.
            - **Status:** CLOSED — wrong scope; see `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-21T005723Z/summary.md` for the postmortem.
      - [ ] **D4f — Compute intensity_scale from raw diffraction BEFORE normalization (NEW 2026-01-21T012500Z).** `calculate_intensity_scale()` still inspects the normalized tensors inside `PtychoDataContainer`, so the dataset-derived formula always degenerates to the closed-form `(N/2)` fallback (normalize_data enforces that invariant). Plan: carry the pre-normalization diffraction stats forward from `raw_data.generate_grouped_data()`/`loader.load()`, store them on the container, and make `calculate_intensity_scale()` consume those raw stats so the saved bundles finally record the true dataset scale (≈577 instead of 988). Add regression tests proving (a) loader attaches `dataset_intensity_stats` computed from the raw `diffraction` field (including after train/test splits) and (b) `calculate_intensity_scale()` prioritizes those stats over `_X_np`. After landing the code, rerun the gs1_ideal + gs2_ideal Phase C2 scenarios under `reports/2026-01-21T012500Z/`, regenerate `bias_summary.*`, and rerun the CLI smoke pytest guard so we can show `bundle_intensity_scale` now matches the dataset-derived value.
            - Tests: `pytest tests/test_loader_normalization.py::TestNormalizeData::test_dataset_stats_attachment -v`, `pytest tests/test_train_pinn.py::TestIntensityScale::test_uses_dataset_stats -v`, `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
            - [ ] **D4f.2 — Manual constructors must propagate dataset_intensity_stats (NEW 2026-01-21T013900Z).** Loader-backed flows now carry raw diffraction stats, but helper scripts that instantiate `PtychoDataContainer` directly (e.g., `scripts/studies/dose_response_study.py`, `ptycho/data_preprocessing.py`, `scripts/inspect_ptycho_data.py`, baseline/CLI inspectors) never pass `dataset_intensity_stats`, so they silently revert to the `(N/2)` fallback. Scope: add a shared NumPy reducer (likely in `ptycho/loader.py`) that can compute `batch_mean_sum_intensity` from raw diffraction (or from normalized data rescaled by the recorded `intensity_scale`), call it from every manual constructor + NPZ inspector, and extend regression coverage (unit tests + a targeted `calculate_intensity_scale` test exercising a manual container) proving the dataset-derived branch fires without touching `_tensor_cache`. After landing the code, rerun the mapped tests plus the CLI smoke selector to ensure Phase D telemetry scripts remain healthy.
            - [x] **D4f.3 — Grid-mode + preprocessing constructors attach dataset stats (COMPLETE 2026-01-21T020608Z).** Updated `scripts/studies/dose_response_study.py::simulate_datasets_grid_mode` and `ptycho/data_preprocessing.py::create_ptycho_dataset` to call `compute_dataset_intensity_stats` on the raw diffraction arrays (or normalized tensors + recorded `intensity_scale`) and pass the resulting dict into every `PtychoDataContainer`. Refreshed `docs/DATA_GENERATION_GUIDE.md` with the mandatory stats-attachment guidance and added regression tests covering the grid-mode helpers plus preprocessing factory. Evidence: `plans/active/DEBUG-SIM-LINES-DOSE-001/reports/2026-01-21T020608Z/` (`pytest_dose_response_stats.log`, `pytest_data_preproc_stats.log`, `pytest_train_pinn_dataset_stats.log`, `pytest_cli_smoke.log`). All mapped selectors pass.
            - [ ] **D5 — Train/test intensity-scale parity instrumentation (NEW 2026-01-21T024500Z).** Analyzer now proves bundle intensity_scale reflects training stats, but we still lack visibility into how train/test raw diffraction distributions diverge. Scope: (a) extend `bin/run_phase_c2_scenario.py` so it computes `compute_dataset_intensity_stats()` for both the train and test `RawData` splits (before normalization), records the derived dataset-scale (`sqrt(nphotons / batch_mean_sum_intensity)`) for each split, and persists them inside `run_metadata.json`; (b) teach `bin/analyze_intensity_bias.py` to surface the new train/test stats and ratios vs the model’s bundle intensity_scale directly in `bias_summary.{json,md}` with a `specs/spec-ptycho-core.md §Normalization Invariants` citation; (c) add regression tests for the analyzer parser to lock in the new metadata contract. Verification: rerun the gs1_ideal + gs2_ideal stable profiles with the enriched telemetry, regenerate `bias_summary` under the new artifacts hub, and keep the CLI smoke selector green so plan-local tooling stays runnable.  
              Tests: `pytest tests/scripts/test_analyze_intensity_bias.py::TestDatasetStats::test_reports_train_test -v`, `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
