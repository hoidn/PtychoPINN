...
- [ ] D4: **Architecture / loss-wiring verification.**
      ...
      - [ ] **D4f — Compute intensity_scale from raw diffraction BEFORE normalization (NEW 2026-01-21T012500Z).** `calculate_intensity_scale()` still inspects the normalized tensors inside `PtychoDataContainer`, so the dataset-derived formula always degenerates to the closed-form `(N/2)` fallback (normalize_data enforces that invariant). Plan: carry the pre-normalization diffraction stats forward from `raw_data.generate_grouped_data()`/`loader.load()`, store them on the container, and make `calculate_intensity_scale()` consume those raw stats so the saved bundles finally record the true dataset scale (≈577 instead of 988). Add regression tests proving (a) loader attaches `dataset_intensity_stats` computed from the raw `diffraction` field (including after train/test splits) and (b) `calculate_intensity_scale()` prioritizes those stats over `_X_np`. After landing the code, rerun the gs1_ideal + gs2_ideal Phase C2 scenarios under `reports/2026-01-21T012500Z/`, regenerate `bias_summary.*`, and rerun the CLI smoke pytest guard so we can show `bundle_intensity_scale` now matches the dataset-derived value.
            ...
            - [ ] **D4f.3 — Grid-mode + preprocessing constructors attach dataset stats (NEW 2026-01-21T020608Z).** `simulate_datasets_grid_mode()` and `ptycho/data_preprocessing.py::create_ptycho_dataset()` still instantiate `PtychoDataContainer` without `dataset_intensity_stats`, so dose-response experiments and legacy preprocessing paths immediately fall back to the closed-form 988.21 constant. Scope: import `compute_dataset_intensity_stats` in both modules, compute stats from the raw diffraction arrays (or normalized data + recorded `intensity_scale` when only normalized tensors exist), and pass the resulting dict into every container (train/test). Update docs/examples to emphasize the requirement for manual constructors. Regression coverage: (a) extend `tests/scripts/test_dose_response_study.py` with a monkeypatched grid-mode simulation that asserts the returned containers expose `dataset_intensity_stats` matching the helper output; (b) add `tests/test_data_preprocessing.py::TestCreatePtychoDataset::test_attaches_dataset_stats` to prove preprocessing-generated datasets carry stats for both splits; (c) keep `tests/test_loader_normalization.py::TestNormalizeData::test_manual_dataset_stats_helper` + `tests/test_train_pinn.py::TestIntensityScale::test_uses_dataset_stats` green to guard regressions. Guard with the CLI smoke selector after edits so plan-local tooling stays runnable.
