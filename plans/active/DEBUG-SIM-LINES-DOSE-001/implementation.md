...
- [ ] D4: **Architecture / loss-wiring verification.**
      ...
      - [x] **D4d — Lazy-container safe dataset scale reducer (COMPLETE 2026-01-21T00:55Z).** D4c now calls `ptycho_data_container.X`, which materializes the full tensor on GPU and defeats the lazy-loading design (see docs/findings.md PINN-CHUNKED-001). Extended `calculate_intensity_scale()` so it prefers the container’s NumPy backing (`_X_np` or an iterator) and performs the dataset-derived reduction entirely on CPU without touching `_tensor_cache`. The TensorFlow path remains as a fallback for legacy containers that lack NumPy buffers.
            ...
            - Tests: `pytest tests/test_train_pinn.py::TestIntensityScale -v`, `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
      - [ ] **D4e — Normalize loader inputs with dataset-derived scale (NEW 2026-01-21T005723Z).** Even after D4d, the loader’s `normalize_data()` still applies the closed-form `(N/2)` fallback, so grouped→normalized ratios remain ≈0.58 and the normalization-invariant check fails with a 18.5× cumulative gain. Update the normalization step to use the same dataset-derived scale (`s = sqrt(nphotons / E_batch[Σ_xy |Ψ|²])`) that the training loop now uses, keeping the closed-form fallback only when dataset statistics are unavailable. Add regression tests covering both NumPy- and TensorFlow-backed flows, update plan-local tooling to surface the new ratio, rerun gs2_ideal with analyzer + CLI smoke guard, and document the before/after ratios.
            - Tests (planned): `pytest tests/test_train_pinn.py::TestIntensityScale -v` (collect guard), `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`, plus a new targeted normalization test (see D4e plan).
