...
- [ ] D4: **Architecture / loss-wiring verification.**
      ...
      - [ ] **D4d — Lazy-container safe dataset scale reducer (NEW 2026-01-21T004455Z).** D4c now calls `ptycho_data_container.X`, which materializes the full tensor on GPU and defeats the lazy-loading design (see docs/findings.md PINN-CHUNKED-001). Extend `calculate_intensity_scale()` so it prefers the container’s NumPy backing (`_X_np` or an iterator) and performs the dataset-derived reduction entirely on CPU without touching `_tensor_cache`. Keep the TensorFlow path as a fallback for legacy containers that lack NumPy buffers.
            - **Implementation:** Introduce a helper that accepts a NumPy view (or streaming reducer) to compute `sum_xy |X|²` using float64 arithmetic, then reuse the existing fallback guard for near-zero means. Ensure the helper never calls `.X` and that the container’s `_tensor_cache` remains empty. Only fall back to the TensorFlow reduction when `_X_np` is unavailable.
            - **Regression test:** Extend `tests/test_train_pinn.py::TestIntensityScale` with `test_lazy_container_does_not_materialize` (or equivalent) using a stub that mimics `PtychoDataContainer` (`_X_np` + lazy `.X`). Assert `calculate_intensity_scale()` returns the expected scalar **and** leaves `_tensor_cache` empty to prove no GPU tensors were allocated.
            - **Verification:** Rerun `AUTHORITATIVE_CMDS_DOC=./docs/TESTING_GUIDE.md python plans/active/DEBUG-SIM-LINES-DOSE-001/bin/run_phase_c2_scenario.py --scenario gs2_ideal --group-limit 64 --prediction-scale-source least_squares --output-dir <new hub>/gs2_ideal` followed by `bin/analyze_intensity_bias.py` + the CLI smoke guard so runtime telemetry (intensity_stats.json, bias_summary.*) captures the unchanged dataset/fallback ratio while exercising the new CPU-only path.
            - Tests: `pytest tests/test_train_pinn.py::TestIntensityScale -v`, `pytest tests/scripts/test_synthetic_helpers_cli_smoke.py::test_sim_lines_pipeline_import_smoke -v`
