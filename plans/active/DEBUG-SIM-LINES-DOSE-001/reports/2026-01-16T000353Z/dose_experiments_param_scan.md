...
| --- | --- | --- |
| `nphotons` | CLI argument (varies per experiment) | Passed to both `init()` and `execute()` before training. |
| `gridsize` | 2 | Hard-coded in both `init()` and `execute()`. |
| `offset` | 4 | Fixed neighbor spacing in both initialization paths. |
| `outer_offset_train` | 8 | Configured during `init()` before dataset generation. |
| `outer_offset_test` | 20 | Configured during `init()` before dataset generation. |
| `nimgs_train` | 2 | Legacy grid generator uses two training positions per axis. |
| `nimgs_test` | 2 | Legacy grid generator uses two test positions per axis. |
| Loss selection | Default `nll`; set `mae` by calling `init(nphotons, loss_fn='mae')` | Table reflects the `loss_fn` parameter handling near lines 12–24. |
