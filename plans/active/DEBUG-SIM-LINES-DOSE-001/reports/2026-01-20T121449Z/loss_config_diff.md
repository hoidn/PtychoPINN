...
## Legacy params.cfg Framework Defaults

The following loss weight defaults are defined in the legacy `ptycho.params.cfg`
(source: `ptycho/params.py:64`)
...
**Conclusion:** The legacy framework defaults in `ptycho/params.py:64` define
`mae_weight=0.0, nll_weight=1.0` (pure NLL loss), which matches the modern
`TrainingConfig` dataclass defaults. **H-LOSS-WEIGHT is ruled out** — both
pipelines use identical loss weights under default operation.
