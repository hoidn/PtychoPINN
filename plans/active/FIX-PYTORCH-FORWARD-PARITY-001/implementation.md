...
- Latest git evidence (f2264803) added `outputs/torch_forward_parity_baseline/analysis/torch_patch_stats*.json` plus `train_debug.log`, but no hub paths changed; `$HUB/analysis/artifact_inventory.txt` still calls out the Nov 14 rerun, so copy over the new stats/grid artifacts once a fresh run completes and update `$HUB/analysis/` + `$HUB/summary.md` immediately.
