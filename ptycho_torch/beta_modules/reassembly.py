...
# Intro
# Contains all modules necessary for rectangular coordinate solving
...
class VarProScaler:
    ...
    terms = 3):
        ...
        # X4 = torch.ones_like(X1) # Background term
    ...
    def solve(self, verbose = True):
        ...
        return s1.float(), s2.float()#, bg.float()
    ...
    def solve_quadratic_direct(self, max_iter=50, verbose=True):
        ...
        Minimizes: ||s1²X1 + s2²X2 + s1*s2*X3 - I||²
        """
    ...
    def solve_autograd(self, max_iter=100, lr=0.1, verbose=True):
        ...
        # Simple initialization based on diagonal solution
        ...
        for iter_num in range(max_iter):
            ...
            # The objective is ||s1²X1 + s2²X2 + s1*s2*X3 - I||²
            # Expanded: (s1²X1 + s2²X2 + s1*s2*X3)'(s1²X1 + s2²X2 + s1*s2*X3) 
            #          - 2(s1²X1 + s2²X2 + s1*s2*X3)'I + I'I
    ...
    def solve_lbfgs(self, max_iter=50, verbose=True):
        ...
        def closure():
            ...
            # Ensure positivity
            # s1_pos = torch.abs(s1) + 1e-6
            # s2_pos = torch.abs(s2) + 1e-6
        ...
        # Ensure final values are positive
        # s1_final = torch.abs(s1) + 1e-6
        # s2_final = torch.abs(s2) + 1e-6
...
class VectorizedWeightedAccumulator:
    ...
    def accumulate_batch(self, 
        ...
        # We pre-multiply the texture by the probe intensity
        # and also create a weight-map representing the probe intensity
        # probe_mag_sq shape: [H, W] -> expands to [N, H, W]
        ...
        # Helper to apply bilinear weights and scatter
        def scatter_weighted(target, source_data, weight_coeff, indices):
            # source_data: [N, H*W], source_weight: [N, H*W], weight_coeff: [N, 1]
            # Payload = PatchValue * ProbeIntensity * BilinearWeight
...
def reconstruct_and_scale(model, loader, canvas_size, probe):
    device = next(model.parameters()).device
    model.eval()
    
    # 1. Initialize logic
    scaler = VarProScaler(device)
    accumulator = VectorizedWeightedAccumulator(canvas_size, device)
    
    canvas = torch.zeros(canvas_size, device=device, dtype=torch.complex64)
    canvas_weights = torch.zeros(canvas_size, device=device, dtype=torch.float32)
    
    probe_mag_sq = torch.abs(probe)**2

    with torch.no_grad():
        for batch in loader:
            # Inputs
            I_raw = batch['images'].to(device)
            coords = batch['coords_relative'].to(device)
            glob_coords = batch['coords_global'].to(device) # for stitching
            
            # PINN Inference (Predicts normalized texture a_tilde, b_tilde)
            # Assuming model.forward_predict returns [B, 2, H, W] or similar
            texture_raw = model.forward_predict(batch) 
            a_tilde = texture_raw[:, 0]
            b_tilde = texture_raw[:, 1]
            
            # --- VarPro Accumulation ---
            # Generate Basis Fields (Physics)
            # Psi_a = F[p * a_tilde]
            # Psi_b = F[j * p * b_tilde]
            Psi_a = torch.fft.fft2(probe * a_tilde)
            Psi_b = torch.fft.fft2(1j * probe * b_tilde)
            scaler.accumulate_batch(I_raw, Psi_a, Psi_b)
            
            # --- Weighted Stitching ---
            # Complex texture O_tilde = a_tilde + j*b_tilde
            O_tilde = torch.complex(a_tilde, b_tilde)
            accumulator.accumulate_batch(canvas, canvas_weights, O_tilde, 
                                        glob_coords, probe_mag_sq)
            

    # 2. Finalize scaling
    s1, s2, background = scaler.solve()
    print(f"Solved Scales: s1={s1:.2f}, s2={s2:.2f}, Background={background:.2f}")
    
    # 3. Finalize Canvas
    # texture_canvas is now unit-normalized [-1, 1] range
    texture_canvas = canvas / (canvas_weights + 1e-12)
    
    # Apply solved physical scales
    final_reconstruction = (s1 * texture_canvas.real) + 1j * (s2 * texture_canvas.imag)
    
    return final_reconstruction, background
...
def detect_swap_probe_reference(model: nn.Module,
    ...
    Uses model.forward_predict (same path as the inference loop) rather
    than accessing the autoencoder directly.
...
def reconstruct_image_barycentric_weighted(model: nn.Module,
                     ...
                     verbose: bool = True,
                     swap_detection: str = 'None') -> Tuple[torch.Tensor, Any]:
    """
    ...
    Args:
        ...
        swap_detection: Method for detecting real/imag channel swap.
            'mean' (default) - compare |mean(real)| vs |mean(imag)| on assembled canvas
    ...
    """
    from ptycho_torch.reassembly import setup_multi_gpu_model
    ...
    # print(f"max both dims: {torch.max(torch.abs(adjusted_coords),dim =)}")
    ...
    #Canvas should be bigger than what is possible here. Middle trim should be larger than 1/2 size of scan itself (e.g. 32 pixels at least if 64 x 64)
    ...
    if inference_config.patch_weighting == 'uniform':
    ...
    with torch.no_grad():
        for i, batch in enumerate(infer_loader):
            ...
            global_coords_2d = global_coords.squeeze(2).view(B * C, 2)
            ...
            # Memory cleanup
            del I_raw, positions, probe, in_scale, global_coords
    ...
    if swap_detection == 'probe':
        ...
        # Default: compare mean magnitudes on assembled canvas
    ...
    return scaled_canvas, ptycho_dset, [total_inference_time, total_assembly_time, Psi_a, Psi_b, s1, s2], modified_scaled_canvas
