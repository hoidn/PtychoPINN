...
class FeatureRefinementBlock(nn.Module):
    ...
    to allow the network to disentangle amplitude-vs-phase feature subsets before the
...
class Decoder_shared(Decoder_base):
    """
    Shared decoder for amplitude and phase components. Inherits upsampling and
    attention logic from Decoder_base (no skip connections, no merge blocks).
    Adds per-level refinement blocks and ECA attention before two separate output
    heads (Decoder_last_Amp and Decoder_last_Phase), preserving native activations.
    ...
    def __init__(self, model_config: ModelConfig, data_config: DataConfig):
        ...
        # Final ECA before heads -- operates on finest decoder features
        base_ch = self.filters[-1]  # n_filters_scale * 32
        self.eca = ECALayer(channel=base_ch)

        # Amplitude head
        num_amp_channels = copy.deepcopy(model_config.decoder_last_amp_channels)
        assert num_amp_channels in [1, model_config.C_model]
...
#Autoencoder
#Note that "amp" and "phase" will actually be real and imaginary modules
class Autoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if getattr(self, '_shared', False):
            x_amp, x_phase = self.decoder(x)  # shared trunk -> two heads
        else:
...
class PtychoPINN(nn.Module):
    ...
    def get_decoder_params(self):
        ...
        For shared decoder: upsampling blocks + attention + refinement + eca (excluding heads).
        """
        params = []

        if getattr(self.autoencoder, '_shared', False):
            # Shared decoder: base blocks + attention + refinement + eca (not the heads)
    ...
    def get_phase_head_params(self):
        ...
        For dual-branch: Decoder_last_Phase from decoder_phase.
        For shared decoder: Decoder_last_Phase from decoder.phase.
        """
        if getattr(self.autoencoder, '_shared', False):
            if hasattr(self.autoencoder.decoder, 'phase'):
                return list(self.autoencoder.decoder.phase.parameters())
            else:
                print("Warning: shared decoder phase head not found")
                return []
        else:
    ...
    def get_amp_head_params(self):
        ...
        For dual-branch: Decoder_last_Amp from decoder_amp.
        For shared decoder: Decoder_last_Amp from decoder.amp.
        """
        if getattr(self.autoencoder, '_shared', False):
            if hasattr(self.autoencoder.decoder, 'amp'):
                return list(self.autoencoder.decoder.amp.parameters())
            else:
                print("Warning: shared decoder amp head not found")
                return []
        else:
