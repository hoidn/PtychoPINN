...
def _uses_native_cli(argv) -> bool:
    """Return whether argv selects the bundle-based native inference CLI."""
    if len(argv) <= 1:
        return False
    first_option = argv[1].split("=", 1)[0]
    return first_option in {"--model_path", "--help", "-h"}


if __name__ == '__main__':
    # Determine which mode to run based on command-line arguments
    if _uses_native_cli(sys.argv):
