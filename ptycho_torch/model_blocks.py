"""nn.Module block zoo extracted from ptycho_torch/model.py (W4 split).

Owns the activation/conv/encoder/decoder blocks, probe modules, forward/loss
modules, and the three top-level models (PtychoPINN, Ptycho_Supervised,
PtychoPINN_Lightning). The config-resolution helpers these blocks call
(``_effective_cnn_output_mode``, ``_semantic_component_channels``,
``_decoder_component_channels``, ``_resolve_generator_from_config``,
``_predict_complex_patches``, ``_build_optimizer``) stay in
``ptycho_torch.model`` and are imported lazily inside the methods that need
them, so ``ptycho_torch.model`` stays the single config-resolution /
monkeypatch seam and no import cycle exists.
"""
...
class Encoder(nn.Module):
    def __init__(self, model_config: ModelConfig, data_config: DataConfig):
        ...
        from ptycho_torch.model import _semantic_component_channels
        self.model_config = model_config
...
class Decoder_phase(Decoder_base):
    def __init__(self, model_config: ModelConfig, data_config: DataConfig):
        ...
        from ptycho_torch.model import _effective_cnn_output_mode, _semantic_component_channels
...
class Decoder_amp(Decoder_base):
    def __init__(self, model_config: ModelConfig, data_config: DataConfig):
        ...
        from ptycho_torch.model import _decoder_component_channels, _effective_cnn_output_mode
        self.model_config = model_config # Store configs if needed directly
        self.data_config = data_config
...
class Decoder_shared(Decoder_base):
    ...
    def __init__(self, model_config: ModelConfig, data_config: DataConfig):
        ...
        from ptycho_torch.model import _decoder_component_channels
        self.model_config = model_config
        self.data_config = data_config
...
class Autoencoder(nn.Module):
    def __init__(self, model_config: ModelConfig, data_config: DataConfig):
        ...
        from ptycho_torch.model import _effective_cnn_output_mode
        self.model_config = model_config
        self.data_config = data_config
...
class PoissonIntensityLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        CRITICAL FIX (the config-factory contract (docs/specs/spec-ptycho-config-bridge.md)-BACKEND-API):
...
class PtychoPINN(nn.Module):
    ...
    def __init__(
        ...
        from ptycho_torch.model import _resolve_generator_from_config
        self.model_config = model_config
        self.data_config = data_config
    ...
    def _predict_complex(self, x):
        ...
        """
        from ptycho_torch.model import _predict_complex_patches
...
class PtychoPINN_Lightning(L.LightningModule):
    ...
    def __init__(
        ...
        artifact_identity: Optional[Dict[str, Any]] = None,
    ):
        super().__init__()
        from ptycho_torch.model import _resolve_generator_from_config

        if artifact_identity is not None:
            # New era: reconstruct every section from the sealed identity payload
            # (single source; Lightning hparams no longer dual-write the four
            # asdict config dicts).
            from ptycho_torch.artifact_schema import decode_artifact_identity

            decoded = decode_artifact_identity(artifact_identity)
            ...
            model_spec = decoded.model_spec
            parity_scale_mode = decoded.model_spec.parity_scale_mode
            parity_fixed_delta = float(decoded.model_spec.parity_fixed_delta)
            parity_init_scheme = decoded.model_spec.parity_init_scheme
        else:
            # Legacy/dual-write eras: Lightning passes saved hyperparameters as
            # dicts during load_from_checkpoint. Dual-write agreement checks run
            # in decode_checkpoint_hparams before the loaders reach strict state
            # loading; this constructor only performs construction and capture.
        ...
        # Save hyperparameters for checkpoint serialization. The new era
        # single-writes the sealed identity payload; only the schema-era tag and
        # the non-identity runtime knobs remain beside it. The generator module
        # is never saved (reconstructed at load time).
        serialized_hparams = {
        ...
        if decoded_model_spec is not None:
            ...
            serialized_hparams["artifact_identity"] = encode_artifact_identity(
                decoded_model_spec,
                data_config,
                training_config,
                inference_config,
            )
            serialized_hparams["schema_era"] = CURRENT_ARTIFACT_SCHEMA_VERSION
        else:
            # Pre-spec legacy protocol (no ModelSpec): dual-write the four
            # config dicts exactly as before.
