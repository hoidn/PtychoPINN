...
def test_main_finalizes_complete_table_recovered_torch_launcher_completion(tmp_path, monkeypatch):
    from scripts.studies import lines128_paper_benchmark as benchmark
    ...
    def fake_run_grid_lines_compare(**kwargs):
        ...
        if kwargs.get("reuse_existing_recons"):
            _write_text(
                output_dir / "launcher_stdout.log",
                "\n".join(
                    [
                        "DEBUG eval_reconstruction [pinn_hybrid_resnet]: amp_target stats: mean=1.0",
                        "DEBUG eval_reconstruction [pinn_fno_vanilla]: amp_target stats: mean=1.0",
                        "DEBUG eval_reconstruction [pinn_ffno]: amp_target stats: mean=1.0",
                    ]
                )
                + "\n",
            )
            _write_text(output_dir / "launcher_stderr.log", "")
            for model_id in kwargs["models"]:
                _write_text(output_dir / f"visuals/amp_phase_{model_id}.png")
                _write_text(output_dir / f"visuals/amp_phase_error_{model_id}.png")
            _write_text(output_dir / "visuals/amp_phase_gt.png")
    ...
    assert (
        manifest_rows["pinn_hybrid_resnet"]["artifacts"]["launcher_completion_json"]
        == "runs/pinn_hybrid_resnet/launcher_completion.json"
    )
    assert (
        manifest_rows["pinn_fno_vanilla"]["artifacts"]["launcher_completion_json"]
        == "runs/pinn_fno_vanilla/launcher_completion.json"
    )
    assert (
