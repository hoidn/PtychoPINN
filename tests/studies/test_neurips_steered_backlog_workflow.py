...
def test_neurips_drain_workflow_uses_local_backlog_implementation_phase():
    ...
    assert workflow["inputs"]["roadmap_gate_path"]["under"] == "docs/backlog"
    assert workflow["inputs"]["roadmap_gate_path"]["must_exist_target"] is True


def test_neurips_backlog_implementation_phase_uses_claude_cli_opus_alias():
    workflow = _load_yaml("workflows/library/neurips_backlog_implementation_phase.yaml")

    claude_opus = workflow["providers"]["claude_opus"]
    assert claude_opus["command"][:5] == [
        "claude",
        "-p",
        "${PROMPT}",
        "--model",
...
def test_neurips_drain_workflow_materializes_selected_item_inputs_and_rewrites_plan_path():
    ...
    assert {
        ...
        "plan_target_path",
        "plan_gate_state_root",
        "final_plan_gate_bundle_path",
    } <= field_names
    assert "item_id" not in field_names

    recover_plan = _step_by_name(workflow, "RecoverPlanGateOutputs")
    assert recover_plan["command"][-3:] == [
        "${steps.MaterializeSelectedItemInputs.artifacts.final_plan_gate_bundle_path}",
        "--status-output",
        "${inputs.state_root}/plan_gate_status.txt",
    ]
    assert recover_plan["expected_outputs"][0]["name"] == "plan_gate_status"

    validate_plan = _step_by_name(workflow, "ValidatePlanGateBundle")
    assert validate_plan["command"][3] == "${steps.MaterializeSelectedItemInputs.artifacts.final_plan_gate_bundle_path}"
    validate_fields = {field["name"]: field for field in validate_plan["expected_outputs"]}
    assert validate_fields["plan_path"]["under"] == "docs/plans"
    assert validate_fields["plan_review_report_path"]["under"] == "artifacts/review"
    assert validate_fields["selected_item_in_progress_path"]["under"] == "docs/backlog/in_progress"
...
def test_neurips_drain_workflow_dispatches_selected_items_through_local_selected_item_subworkflow():
    ...
    assert run_selected["with"]["manifest_path"] == {
        "ref": "parent.steps.ReconcileBacklogRoadmapGate.artifacts.eligible_manifest_path"
    }
...
def test_neurips_drain_workflow_asserts_plan_and_implementation_approval_before_completion():
    ...
    assert finalize["expected_outputs"][0]["allowed"] == ["CONTINUE", "BLOCKED"]
...
def test_neurips_backlog_implementation_phase_emits_explicit_execution_state():
    ...
    materialize_script = materialize["command"][2]
    assert "Current implementation pass produced neither execution report nor blocker report" in materialize_script
    assert "phase_started_at_ns" in "\n".join(str(part) for part in materialize["command"])
    assert "Blocker Class" in materialize_script
    bundle_fields = {field["name"]: field for field in materialize["output_bundle"]["fields"]}
    assert bundle_fields["implementation_state"]["allowed"] == ["COMPLETED", "BLOCKED"]
