...
def test_wrapper_preflight_only_routes_coordinate_conditioning_rows(monkeypatch, tmp_path):
    from scripts.studies.grid_lines_compare_wrapper import run_grid_lines_compare
    ...
    result = run_grid_lines_compare(
        ...
        probe_npz=probe_path,
        architectures=(),
        models=("pinn_hybrid_resnet_grid_channels", "pinn_ffno_grid_channels"),
        model_n={
            "pinn_hybrid_resnet_grid_channels": 128,
            "pinn_ffno_grid_channels": 128,
        },
        preflight_only=True,
        seed=3,
        set_phi=True,
    ...
    assert all(entry["input_conditioning_mode"] == "coordinate_grid" for entry in captured)
    assert captured[0]["architecture"] == "hybrid_resnet"
    assert captured[1]["architecture"] == "ffno"
    assert captured[1]["fno_cnn_blocks"] == 0
    assert result["row_plan"][0]["overrides"]["input_conditioning_mode"] == "coordinate_grid"
    assert result["row_plan"][1]["overrides"]["input_conditioning_mode"] == "coordinate_grid"
