...
def test_raw_data_torch_round_trip_preserves_selected_acquisition_boundary(monkeypatch):
    ...
    try:
        ...
        object_guess = np.ones((128, 128), dtype=np.complex64) * (2 - 1j)
        target = np.arange(
            n_points * patch_size * patch_size, dtype=np.float32
        ).reshape(n_points, patch_size, patch_size).astype(np.complex64)
        normalization = np.float32(3.5)
        metadata = {"source": "slice-4a"}
        sample_indices = np.arange(n_points, dtype=np.int64) * 2 + 1
        object_index = np.arange(n_points, dtype=np.int64) % 2
        probe_simulated = probe * (3 - 1j)
        label = target * (2 + 1j)
        ...
        raw = RawData(
            ...
            object_amplitude_scale=np.float64(2.5),
            label=label,
        ...
        for field in fields(AcquisitionRecord):
            expected_value = getattr(record, field.name)
