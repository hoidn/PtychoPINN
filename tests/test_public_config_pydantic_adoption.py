...
@pytest.mark.parametrize(
    ...
    [
        (
            validate_training_config_structure,
            TrainingConfig(
                ModelConfig(architecture="hybrid_resnet", fno_blocks=2),
                n_groups=7,
            ),
        ),
        (
            validate_inference_config_structure,
            InferenceConfig(
                ModelConfig(architecture="hybrid_resnet", resnet_width=30),
                Path("model"),
                Path("test.npz"),
            ),
        ),
...
def test_root_validators_retain_nested_model_semantics(validator, record):
    with pytest.raises(ValueError, match="fno_blocks|resnet_width"):
        validator(record)
