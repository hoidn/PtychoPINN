...
class TestTorchRunnerConfig:
    ...
    def test_varpro_probe_weighting_knob_defaults_are_stable(self, tmp_path):
        """Omitting the new B7 knobs must reproduce ModelConfig's existing defaults."""
        cfg = TorchRunnerConfig(
            train_npz=tmp_path / "train.npz",
            test_npz=tmp_path / "test.npz",
            output_dir=tmp_path / "output",
        ...
        assert cfg.cnn_output_mode == "amp_phase"
        assert cfg.training_patch_weighting == "central_mask"
        assert cfg.physics_forward_mode == "amplitude"
        assert cfg.rect_s1s2_trainable is True

    def test_varpro_probe_weighting_knobs_are_overridable(self, tmp_path):
        cfg = TorchRunnerConfig(
            train_npz=tmp_path / "train.npz",
            test_npz=tmp_path / "test.npz",
            output_dir=tmp_path / "output",
        ...
        assert cfg.cnn_output_mode == "real_imag"
        assert cfg.training_patch_weighting == "probe"
        assert cfg.physics_forward_mode == "rectangular_scaled"
        assert cfg.rect_s1s2_trainable is False
    ...
    def test_apply_probe_real_imag_conditioning_appends_probe_channels(self, tmp_path):
        ...
        metadata = {
            "additional_parameters": {
                ...
                "probe_transform_pipeline": "pad_extrapolate:4|smooth:0.5",
                "probe_npz": str(tmp_path / "probe.npz"),
            }
        }
        ...
        assert conditioned.shape == (2, 4, 4, 3)
        assert np.allclose(conditioned[..., 0], 7.0)
        assert np.allclose(conditioned[..., 1], 1.0)
        assert np.allclose(conditioned[..., 2], 2.0)
        assert contract["enabled"] is True
        assert contract["learned_input_channels"] == 3
        assert contract["probe_channel_count"] == 2
        assert contract["probe_lineage"]["probe_transform_pipeline"] == "pad_extrapolate:4|smooth:0.5"

    def test_apply_probe_real_imag_conditioning_rejects_multi_channel_diffraction(self, tmp_path):
        diffraction = np.ones((2, 4, 4, 2), dtype=np.float32)
        ...
        with pytest.raises(ValueError, match="single-channel"):
            _apply_input_conditioning(diffraction, probe, cfg)
...
class TestSetupTorchConfigs:
    ...
    def test_setup_configs_threads_probe_conditioning_input_channels(self, tmp_path):
        """Probe-conditioned rows must advertise a 3-channel learned input."""
        cfg = TorchRunnerConfig(
            train_npz=tmp_path / "train.npz",
            test_npz=tmp_path / "test.npz",
            output_dir=tmp_path / "output",
...
class TestChannelGridsizeAlignment:
    ...
    def test_runner_accepts_hybrid_resnet_ffno_ptychoblock_encoder_architecture(self, tmp_path):
        cfg = TorchRunnerConfig(
            train_npz=tmp_path / "train.npz",
            test_npz=tmp_path / "test.npz",
            output_dir=tmp_path / "out",
    ...
    def test_runner_channels_derived_from_gridsize(self, synthetic_npz, tmp_path):
        ...
        For architectures ported to gridsize>1 (e.g. hybrid_resnet), the
        number of channels should match gridsize squared to handle
        ...
        for gridsize in [1, 2]:
            ...
            payload = create_training_payload(
                ...
                output_dir=tmp_path / f"payload_out_{gridsize}",
                overrides={"n_groups": 4, "gridsize": gridsize, "architecture": "hybrid_resnet"},
            )
            assert payload.pt_data_config.C == gridsize**2
    ...
    def test_create_training_payload_preserves_learned_input_channels(self, synthetic_ptycho_npz, tmp_path):
        """Test that config_factory preserves learned_input_channels across both config layers."""
        from ptycho_torch.config_factory import create_training_payload
        ...
        payload = create_training_payload(
            train_data_file=train_npz,
            output_dir=tmp_path,
            overrides={"n_groups": 4, "gridsize": 1, "learned_input_channels": 3},
        )
    ...
    def test_gridsize2_cnn_sets_object_big_true(self, synthetic_ptycho_npz, tmp_path, monkeypatch):
        ...
        payload = create_training_payload(
            train_data_file=train_npz,
            output_dir=tmp_path / "payload_out",
            overrides={"n_groups": 4, "gridsize": cfg.gridsize},
        )
        assert payload.pt_data_config.C == 4

    def test_gridsize1_object_big_false_all_archs(self, tmp_path):
        """Regression pin: gridsize=1 must keep object_big False for every architecture."""
        architectures = [
            "cnn",
            "hybrid_resnet",
            "fno",
        ...
        for architecture in architectures:
            cfg = TorchRunnerConfig(
                train_npz=tmp_path / "train.npz",
                test_npz=tmp_path / "test.npz",
                output_dir=tmp_path / f"out_{architecture}",
                architecture=architecture,
                gridsize=1,
            )
            training_config, _ = setup_torch_configs(cfg)
            assert training_config.model.object_big is False, (
                f"object_big should be False at gridsize=1 for architecture={architecture!r}"
            )

        neuralop_cfg = TorchRunnerConfig(
            train_npz=tmp_path / "train.npz",
            test_npz=tmp_path / "test.npz",
            output_dir=tmp_path / "out_neuralop_uno",
            architecture="neuralop_uno",
            gridsize=1,
            N=128,
            generator_output_mode="real_imag",
        )
        training_config, _ = setup_torch_configs(neuralop_cfg)
        assert training_config.model.object_big is False

    def test_gridsize2_unported_generator_raises(self, tmp_path):
        """Architectures outside the gridsize>1 allowlist must raise a clear error."""
        cfg = TorchRunnerConfig(
            train_npz=tmp_path / "train.npz",
            test_npz=tmp_path / "test.npz",
            output_dir=tmp_path / "out",
        ...
        with pytest.raises(ValueError, match="not ported for gridsize>1"):
            setup_torch_configs(cfg)
...
class TestArchitecturePropagation:
    ...
    def test_workflow_forwards_learned_input_channels_to_factory(self, monkeypatch, tmp_path):
        ...
        assert captured["overrides"]["learned_input_channels"] == 3
...
class TestForwardSignatureEnforcement:
    ...
    def test_amplitude_mode_inference_uses_raw_input_when_ci_adapter_is_inactive(
        self,
        tmp_path,
    ):
        """Inference must use the same CI-activation predicate as training."""
        ...
        raw_amplitude = np.arange(2 * 4 * 4, dtype=np.float32).reshape(2, 4, 4, 1)
        raw_test_data = {
        ...
        model = SpyModel()
        run_torch_inference(model, raw_test_data, cfg)

        torch.testing.assert_close(
            model.seen_input,
            torch.from_numpy(raw_amplitude).permute(0, 3, 1, 2),
        )
...
class TestTorchTrainingPath:
    ...
    def test_probe_conditioning_keeps_observed_diffraction_separate(self, synthetic_npz, tmp_path, monkeypatch):
        """Probe conditioning should not replace the single-channel diffraction loss target."""
        from unittest.mock import MagicMock
    ...
    def test_runner_forwards_varpro_probe_weighting_flags_to_lightning_overrides(
        self, synthetic_npz, tmp_path, monkeypatch
    ):
        """Task 2.7 (B7) fix: --training-patch-weighting, --physics-forward-mode,
        --cnn-output-mode, and --freeze-s1s2 (=> rect_s1s2_trainable=False) must reach
        _train_with_lightning's `overrides` kwarg, mirroring --output-mode's functional
        wiring. Before the fix these 4 knobs were captured on TorchRunnerConfig and
        provenance but never forwarded to training, making them inert.
        """
        from unittest.mock import MagicMock
    ...
    def test_runner_default_varpro_probe_weighting_flags_forward_modelconfig_defaults(
        ...
        """DEFAULTS NEVER CHANGE: omitting the 4 flags must still forward
        ptycho_torch.config_params.ModelConfig's existing defaults (not skip
        forwarding), so training behavior is unchanged from before Task 2.7."""
        from unittest.mock import MagicMock
...
def test_main_defaults_count_scale_mode_to_off(tmp_path, monkeypatch):
    from scripts.studies import grid_lines_torch_runner as runner
...
def test_main_accepts_varpro_probe_weighting_flags(tmp_path, monkeypatch):
    """--training-patch-weighting/--physics-forward-mode/--cnn-output-mode/--freeze-s1s2
    must reach TorchRunnerConfig through argparse + the cfg construction kwargs (Task 2.7/B7),
    mirroring how --output-mode reaches generator_output_mode."""
    from scripts.studies import grid_lines_torch_runner as runner
...
def test_main_varpro_probe_weighting_flags_default_unchanged(tmp_path, monkeypatch):
    """Omitting the new B7 flags must reproduce current behavior exactly (defaults never change)."""
    from scripts.studies import grid_lines_torch_runner as runner
