...
def test_cross_dataset_inference_loads_single_model_pt_and_runs_two_test_npzs(
    ...
    def fake_run_single_dataset(
        model, config, dataset_name, test_npz, recon_npz, allow_oom_fallback, runtime_contract_out
    ):
        _ = (model, config, test_npz, allow_oom_fallback, runtime_contract_out)
...
def test_cross_dataset_inference_writes_recon_npz_for_each_dataset(monkeypatch, tmp_path):
    ...
    def fake_run_single_dataset(
        ...
        _ = (model, config, dataset_name, test_npz, allow_oom_fallback, runtime_contract_out)
        recon_npz.parent.mkdir(parents=True, exist_ok=True)
        np.savez(recon_npz, YY_pred=np.ones((16, 16), dtype=np.complex64))
        return recon_npz
...
def test_manifest_includes_runtime_decision_fields(monkeypatch, tmp_path):
    from scripts.studies.grid_lines_torch_runner import TorchRunnerConfig
    from scripts.studies.hybrid_checkpoint_inference import run_cross_dataset_hybrid_inference
    ...
    def fake_run_single_dataset(
        ...
        recon_npz,
        allow_oom_fallback,
        runtime_contract_out,
    ):
        _ = (model, config, dataset_name, test_npz, allow_oom_fallback)
        runtime_contract_out.update(
            {
                "requested_reassembly_backend": "auto",
                "resolved_reassembly_backend": "shift_sum",
    ...
    manifest = json.loads((tmp_path / "outputs" / "hybrid_cross_dataset_manifest.json").read_text())
    assert manifest["requested_reassembly_backend"] == "auto"
    assert manifest["resolved_reassembly_backend"] == "shift_sum"
    assert manifest["allow_oom_fallback"] is True
    assert manifest["fallback_used"] is False


def test_manifest_fallback_is_false_in_strict_mode(monkeypatch, tmp_path):
    from scripts.studies.grid_lines_torch_runner import TorchRunnerConfig
    from scripts.studies.hybrid_checkpoint_inference import run_cross_dataset_hybrid_inference
    ...
    def fake_run_single_dataset(
        ...
        runtime_contract_out.update(
            {
                "requested_reassembly_backend": "shift_sum",
                "resolved_reassembly_backend": "shift_sum",
...
def test_cross_dataset_inference_propagates_allow_oom_fallback(monkeypatch, tmp_path):
    ...
    def fake_run_single_dataset(
        ...
        _ = (model, config, dataset_name, test_npz, runtime_contract_out)
