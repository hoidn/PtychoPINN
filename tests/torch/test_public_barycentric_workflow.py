...
def _write_flat_npz(
    ...
    *,
    ...
    measurement_pair: tuple[str, str] | None = None,
) -> None:
    ...
    if measurement_pair is not None:
        payload.update(
            scale_contract_version=np.asarray(measurement_pair[0]),
            measurement_domain=np.asarray(measurement_pair[1]),
        )
...
def _model_stub(
    *,
    ...
    scale_contract_version: str = "legacy_v1",
    ...
    return SimpleNamespace(
        ...
        inference_config=InferenceConfig(
            ...
            varpro_scaling=(measurement_domain == "count_intensity"),
            middle_trim=4,
            batch_size=1,
        ),
        ...
        eval=MagicMock(),
        to=MagicMock(),
        get_ci_statistics=MagicMock(
            return_value={
                "rms_input_scale": torch.tensor([0.25]),
                "mean_measured_intensity": torch.tensor([12.5]),
            }
        ),
    )
...
def _install_stubs(monkeypatch, model, *, reconstruct=None, loader_params=None):
    ...
    def build_dataset(ptycho_dir, _model_config, data_config, *_args, **kwargs):
        ...
        dataset.creation_kwargs = kwargs
        return dataset
...
def test_public_name_replaces_cryptic_symbol():
    import ptycho_torch.inference as inference

    assert callable(inference.reconstruct)
    assert not hasattr(inference, "reconstruct_npz_" + "barycentric")


@pytest.mark.parametrize("parent_kind", ["none", "path", "string"])
def test_workspace_accepts_pathlikes_and_cleans_owned_child(
    tmp_path, monkeypatch, parent_kind
):
    from ptycho_torch.inference import reconstruct

    bundle, test_npz, run_root = _paths(tmp_path)
    model = _model_stub()
    _, dataset_paths, _, _ = _install_stubs(monkeypatch, model)
    parent = None if parent_kind == "none" else run_root
    if parent_kind == "string":
        parent = os.fspath(parent)

    model_path = bundle / "wts.h5.zip" if parent_kind == "string" else bundle
    result = reconstruct(os.fspath(model_path), test_npz, work_dir=parent, quiet=True)
...
@pytest.mark.parametrize(
    ("source_pair", "digest_matches", "supplied_nphotons", "expected_scale"),
    [
        (("ci_intensity_v2", "count_intensity"), False, 1e9, None),
        (("legacy_v1", "normalized_amplitude"), False, None, 1e9),
        (None, True, None, 1e9),
        (None, False, 1e9, 1e9),
        (None, False, None, None),
    ],
)
def test_rescale_source_choice_matrix(
    tmp_path,
    monkeypatch,
    source_pair,
    digest_matches,
    ...
    bundle, test_npz, run_root = _paths(tmp_path)
    _write_flat_npz(test_npz, measurement_pair=source_pair)
    digest = hashlib.sha256(test_npz.read_bytes()).hexdigest()
    model = _model_stub(
    ...
    loader_params = {
        "amplitude_physics_gain_record": None,
        "rescaled_source_sha256": digest if digest_matches else "0" * 64,
    }
    _, _, _, reassembly = _install_stubs(
        monkeypatch, model, loader_params=loader_params
    )
    ...
    reconstruct(
        ...
        test_npz,
        work_dir=run_root,
        nphotons=supplied_nphotons,
        quiet=True,
    )

    dataset = reassembly.call_args.args[1]
    assert dataset.creation_kwargs["rescale_to_nphotons"] == expected_scale
    assert dataset.creation_kwargs["defer_ci_statistics"] is True
    assert dataset.data_dict["ci_statistics"] is model.get_ci_statistics.return_value


def test_reconstruction_nphotons_must_match_bundle_even_without_scaling(
    ...
    _write_flat_npz(
        test_npz,
        measurement_pair=("ci_intensity_v2", "count_intensity"),
    )
    model = _model_stub(
        physics_forward_mode="rectangular_scaled",
        scale_contract_version="ci_intensity_v2",
    ...
    _, _, _, reassembly = _install_stubs(monkeypatch, model)

    with pytest.raises(ValueError, match="nphotons.*bundle"):
        reconstruct(
            bundle,
            test_npz,
            work_dir=run_root,
            nphotons=model.data_config.nphotons + 1,
        )
    reassembly.assert_not_called()
...
def test_partial_source_measurement_pair_fails_before_mmap(
    ...
    with pytest.raises(ValueError, match="unsupported.*pair"):
        reconstruct(bundle, test_npz, work_dir=run_root)
    reassembly.assert_not_called()


def test_declared_count_source_rejects_legacy_bundle(tmp_path, monkeypatch):
    from ptycho_torch.inference import reconstruct
    ...
    with pytest.raises(ValueError, match="count.*legacy|legacy.*count"):
        reconstruct(bundle, test_npz, work_dir=run_root)
    reassembly.assert_not_called()
...
@pytest.mark.parametrize(
    "source_pair",
    [("legacy_v1", "normalized_amplitude"), None],
)
def test_legacy_amplitude_and_metadata_free_sources_pass_through(
    tmp_path, monkeypatch, source_pair
):
    from ptycho_torch.inference import reconstruct
    ...
    reconstruct(
        ...
        test_npz,
        work_dir=run_root,
        nphotons=model.data_config.nphotons,
        quiet=True,
    )

    dataset = reassembly.call_args.args[1]
    assert dataset.creation_kwargs["rescale_to_nphotons"] is None
...
def test_public_workflow_strict_loads_and_returns_frozen_snapshots(
    ...
    from ptycho_torch.inference import (
        ...
        reconstruct as reconstruct_npz,
    )
...
@pytest.mark.parametrize("supplied_parent", [False, True])
def test_workspace_cleanup_preserves_sentinel_when_reassembly_fails(
    tmp_path, monkeypatch, supplied_parent
):
    from ptycho_torch.inference import reconstruct as reconstruct_npz
    ...
    with pytest.raises(RuntimeError, match="reassembly exploded"):
        reconstruct_npz(
            bundle,
            test_npz,
            work_dir=run_root if supplied_parent else None,
            quiet=True,
        )
...
def test_bundle_modelspec_must_agree_with_dual_written_model_config(
    ...
    bundle, test_npz, run_root = _paths(tmp_path)
    model = _model_stub()
    model.model_config = replace(model.model_config, physics_forward_mode="rectangular_scaled")
    _, _, _, reconstruct = _install_stubs(monkeypatch, model)

    with pytest.raises(ValueError, match=r"ModelSpec.*physics_forward_mode"):
        reconstruct_npz(bundle, test_npz, work_dir=run_root)
...
def test_training_groups_alias_must_match_loaded_training_config(tmp_path, monkeypatch):
    ...
    with pytest.raises(
        ValueError, match=r"training_groups mismatch"
    ):
        reconstruct_npz(
            bundle,
            test_npz,
...
def test_manifest_without_default_frame_order_remains_inference_compatible(tmp_path):
    from ptycho.workflows.synthetic_config import (
        materialize_data_config,
        resolve_synthetic_workflow,
    )
    ...
    manifest["simulation"].pop("frame_order_recipe")
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
...
def test_flat_npz_validation_accepts_source_backed_manifest_v4(tmp_path):
    ...
    _validate_flat_npz(
        ...
        materialize_data_config(resolved),
...
def test_historical_rectangular_legacy_bundle_is_supported(tmp_path, monkeypatch):
    ...
    reconstruct_npz(bundle, test_npz, work_dir=run_root, quiet=True)
