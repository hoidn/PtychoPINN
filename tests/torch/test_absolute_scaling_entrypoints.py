...
def test_ci_bundle_recovers_profile_configs_and_frozen_statistics(tmp_path):
    ...
    assert manifest["artifact_schema_version"] == "torch-artifact-v2"
    assert persisted_identity["schema_version"] == "torch-artifact-v2"
    assert persisted_identity["model_spec"]["schema_version"] == "torch-model-spec-v2"
