...
def test_rawdata_container_exposes_coords_relative():
    ...
    assert hasattr(container, "coords_relative"), "coords_relative must be exposed on container"
    assert np.allclose(
        container.coords_relative.cpu().numpy(),
        container.coords_nominal.cpu().numpy(),
    )
