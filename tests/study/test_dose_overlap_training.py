...
pytest.importorskip("studies.fly64_dose_overlap.training", reason="branch-local studies/ package not present (gitignored; see TESTING_GUIDE CI section)")
...
def test_execute_training_job_delegates_to_pytorch_trainer(tmp_path, monkeypatch):
    ...
    config = TrainingConfig(
        data=DataConfig(train_data_file=str(train_npz), test_data_file=str(test_npz), nphotons=1e3),
        output_dir=str(artifact_dir),
        model=model_config,
    )
