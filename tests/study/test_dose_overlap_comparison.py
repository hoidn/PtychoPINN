...
def test_pinn_reconstruction_reassembles_full_train_split():
    ...
    try:
        ...
        # For complex output from reassemble_patches, shape should be (1, padded_size, padded_size, 1)
        assert result.shape[0] == 1, f"Expected batch dimension 1, got {result.shape[0]}"
        assert result.shape[1] == padded_size, f"Expected height {padded_size}, got {result.shape[1]}"
        assert result.shape[2] == padded_size, f"Expected width {padded_size}, got {result.shape[2]}"
        assert result.shape[3] == 1, f"Expected channel dimension 1, got {result.shape[3]}"

        # Verify result dtype is complex (reassemble_patches returns complex output)
        assert result.dtype in [tf.complex64, tf.complex128], \
            f"Expected complex dtype, got {result.dtype}"

    except ValueError as e:
        # If we get a Translation layer error, the fix didn't work
        if "Translation" in str(e) or "inputs" in str(e):
            raise AssertionError(
                f"ReassemblePatchesLayer failed with Translation error on {B*C} patches: {e}\n"
                f"This indicates the batched reassembly path is not working correctly."
            )
        else:
