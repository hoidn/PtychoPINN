...
- Re-restored the 16-command verification ledger at
  ...
  to the on-disk working tree (23rd iteration of this recurring restore). The
  file had regressed once more to the 3-command workflow-default `json.tool`
  skeleton between the prior fix commit (`c756b02a docs(cdi-final-refresh):
  record 22nd-iteration on-disk ledger restore`) and the start of this
...
- Workflow-infrastructure follow-up (out of implementation authority for this
  ...
  `c756b02a`) restored the published 16-command ledger and each was subsequently
