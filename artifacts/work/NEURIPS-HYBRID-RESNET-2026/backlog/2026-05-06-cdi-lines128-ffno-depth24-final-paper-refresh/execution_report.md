...
- Re-restored the 16-command verification ledger at
  ...
  to the on-disk working tree (16th iteration of this recurring restore). The
  file had regressed once more to the 3-command workflow-default `json.tool`
  skeleton between the prior fix commit (`158d410d docs(cdi-final-refresh):
  record 15th-iteration on-disk ledger restore`) and the start of this
...
- Workflow-infrastructure follow-up (out of implementation authority for this
  ...
  restored the published 16-command ledger and each was subsequently
