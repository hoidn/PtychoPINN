...
- Re-restored the 16-command verification ledger at
  ...
  to the on-disk working tree (24th iteration of this recurring restore). The
  file had regressed once more to the 3-command workflow-default `json.tool`
  skeleton between the prior fix commit (`29d85f2e docs(cdi-final-refresh):
  record 23rd-iteration on-disk ledger restore`) and the start of this
...
- Workflow-infrastructure follow-up (out of implementation authority for this
  ...
  `c756b02a`, `29d85f2e`) restored the published 16-command ledger and each was subsequently
