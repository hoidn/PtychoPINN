...
- Re-restored the 16-command verification ledger at
  ...
  to the on-disk working tree (11th iteration of this recurring restore). The
  file had regressed once more to the 3-command workflow-default `json.tool`
  skeleton between the prior fix commit (`f4c2834c docs(cdi-final-refresh):
  re-restore on-disk 16-cmd checks ledger (10th iter)`) and the start of this
...
- Workflow-infrastructure follow-up (out of implementation authority for this
  ...
  `f4c2834c`) restored the published 16-command ledger and each was
