...
def process_subset(description: str, answers_file: str = None, system: str = None, system_file: str = None):
    ...
    def format_files_section(files_data):
        ...
        for file in files_data:
            # Handle spec_of_changes that could be string or list
            spec_changes = file['spec_of_changes']
            if isinstance(spec_changes, str):
                # Split string on newlines and clean up
                changes = [line.strip() for line in spec_changes.split('\n') if line.strip()]
            else:
                changes = spec_changes

            # Handle dependencies that could be under either key
            deps = file.get('dependencies_affected', file.get('dependencies', []))
