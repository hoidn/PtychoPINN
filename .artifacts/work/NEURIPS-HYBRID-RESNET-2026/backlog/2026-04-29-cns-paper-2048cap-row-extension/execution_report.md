...
- Refreshed `cns_paper_locked_rows_2048cap.json`, `bundle_2048cap/`, and `pdebench_cns_paper_2048cap_extension_summary.md` so the 2048cap companion bundle now reflects the verified replacement `unet_strong` metrics (`err_nRMSE=0.64315`) and no longer relies on reconstructed PID/exit-code evidence.
...
- Preserved the approved scope boundary: the 512cap bundle remains the durable paper bundle, while the 2048cap bundle remains a capped decision-support companion.
